...
REM ==============================================================================
REM SMART BUILD (EMBED MODE) - Builds with --embed flag for debugging
REM ==============================================================================
...
REM First, try to find .uplugin in root directory
...
:FoundPlugin
if "%PLUGIN_FILE%"=="" (
    echo [ERROR] No .uplugin file found in root or nested folders!
    echo Searched: %SCRIPT_DIR%
    exit /b 1
)
...
echo.
echo ============================================================================
echo [STEP 2/3] Regenerating C++ from KAIN source (WITH EMBED MARKERS)...
echo ============================================================================
kain build --ue5 --embed > "%TEMP_LOG%" 2>&1
set "KAIN_RESULT=%ERRORLEVEL%"

if !KAIN_RESULT! neq 0 (
    echo [ERROR] KAIN compilation failed
    type "%TEMP_LOG%"
    call :LogErrors "KAIN COMPILATION (EMBED)" "%TEMP_LOG%"
    del "%TEMP_LOG%" 2>nul
    exit /b 1
)

echo [SUCCESS] KAIN compilation complete with embedded markers
del "%TEMP_LOG%" 2>nul
...
if !BUILD_RESULT! neq 0 (
    ...
    type "%TEMP_LOG%"
    call :LogErrors "UE5 BUILD (EMBED)" "%TEMP_LOG%"
    del "%TEMP_LOG%" 2>nul
    exit /b 1
)
...
echo.
echo ============================================================================
echo [SUCCESS] Build complete with embedded KAIN markers!
echo ============================================================================
echo Plugin: %PLUGIN_NAME%
echo Build: %OUTPUT_ROOT%\%PLUGIN_NAME%_5.4
echo.
echo Generated C++ contains embedded KAIN source as comments.
