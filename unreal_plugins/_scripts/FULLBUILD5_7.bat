...
if !KAIN_RESULT! neq 0 (
    ...
    call :LogToLocal "KAIN COMPILATION" "%TEMP_LOG%"
    del "%TEMP_LOG%" 2>nul
    exit /b 1
)
...
if not exist "%UE_5_7%\Engine\Build\BatchFiles\RunUAT.bat" (
    echo [ERROR] Could not find UE 5.4 at: %UE_5_7%
    exit /b 1
)

REM BuildPlugin is smart - it does incremental builds automatically
call "%UE_5_7%\Engine\Build\BatchFiles\RunUAT.bat" BuildPlugin -Plugin="%PLUGIN_FILE%" -Package="%OUTPUT_ROOT%\%PLUGIN_NAME%_5.4" -Rocket -TargetPlatforms=Win64 > "%TEMP_LOG%" 2>&1
