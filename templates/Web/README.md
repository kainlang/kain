...
- website systems for docs links, search, FAQ, pricing, testimonials, prompt decks, capability matrices, blueprint grids, app modules, UI kit (components + layouts + tokens), auth strategy, commerce offers, data collections, realtime channels, local form capture, actor routes, actor policies + metrics, status boards, roadmaps, team/career panels, press kits, support lanes, legal policies, security controls, RSS, robots, and sitemap output
- growth, experiments, service catalog, success playbooks, and notification channels modeled alongside the core web systems
