...
- runtime reflection catalogs
  ...
  - committed launch-profile, build-graph, distribution-receipt, GPU, and runtime-contract reflection catalogs under `generated/runtime-reflection/{launch-profiles,build-graphs,distribution,gpu,contracts}`
  - launch-profile/build-graph/distribution catalogs include query-ready indexes and graph-channel cross-links generated by `tools/reflection/generate_runtime_reflection_catalogs.ps1`
  ...
  - the committed jobs examples now cover completed, running, and failed receipt states plus a multi-entry retry ledger so the reflection catalogs bind more than the single happy-path sample
