...
- On Linux, use `scripts/materialize_session_state.py` and `scripts/materialize_shell.py` instead of the PowerShell scripts. `pwsh` is not assumed to exist on the machine.
- `scripts/patch_native_app_bridge.py` is part of the Linux build path. `kain build native-ui --bundle-only` regenerates `native-app/src/main.rs` and `native-app/Cargo.toml`, so the launcher patch must be reapplied after each bundle materialization if the file-backed bridge is required.
