...
- `fabric/intents/*.fabric.toml` owns reusable per-lane graphs for ingest, sculpt, rig, sim, material, render, compositor, publish, and tensor-oriented work. The render room now includes delegated preview, lighting review, AOV packing, review capture, visibility telemetry, pathtrace accumulation, and frame scheduling as separate authored seams. The material lane now also carries a GPU paint-runtime preview seam alongside UV policy and deformation surfaces so the lookdev bench reads like a real multi-runtime control room.
