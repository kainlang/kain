...
void fill_src_buffer(VkDevice device, const Buffer& buffer, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        mapped[i * 4 + 0] = static_cast<float>((i * 3u) % 257u) * 0.25f + 1.0f;
        mapped[i * 4 + 1] = static_cast<float>((i * 5u) % 263u) * 0.125f + 2.0f;
        mapped[i * 4 + 2] = static_cast<float>((i * 7u) % 269u) * 0.0625f + 3.0f;
        mapped[i * 4 + 3] = -7777.0f;
    }
    vkUnmapMemory(device, buffer.memory);
}
...
VerifyResult verify_copy(VkDevice device, const Buffer& src, const Buffer& dst, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        for (uint32_t lane = 0; lane < 3; ++lane) {
            const float expected = src_map[i * 4 + lane];
            const float actual = dst_map[i * 4 + lane];
            result.checksum = fnv_mix(result.checksum, float_bits(actual));
...
int main() {
    ...
    try {
        ...
        const std::string case_id = env_string("KAIN_GPU_CASE_ID", "vec3_storage_copy");
        ...
        app.pApplicationName = "kain-gpu-benchmark-vec3-copy";
        ...
        const VkDeviceSize vec3_stride_bytes = 16;
        const VkDeviceSize storage_size = static_cast<VkDeviceSize>(work_items) * vec3_stride_bytes;
        ...
        const VerifyResult verify = verify_copy(device, src_buffer, dst_buffer, work_items);
