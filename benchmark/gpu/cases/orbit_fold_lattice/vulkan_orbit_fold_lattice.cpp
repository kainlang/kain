...
Vec4f transform_packet(const Vec4f& packet, uint32_t idx, uint32_t bias) {
    ...
    const float seed = static_cast<float>(((idx + (bias * 3u)) % 257u) + 5u);
    const float base0 = packet.x + seed;
    const float base1 = packet.y + step + 3.0f;
    const float base2 = packet.z + packet.x + 17.0f;
    const float base3 = packet.w + packet.y + 29.0f;
    if (lane == 0u) {
        return Vec4f{base0 + base1 + 5.0f, base1 + base2 + 7.0f, base2 + base3 + 11.0f, base3 + step + 13.0f};
    }
    if (lane == 1u) {
        return Vec4f{base0 + packet.w + 17.0f, base1 + packet.x + 19.0f, base2 + packet.y + 23.0f, base3 + packet.z + 29.0f};
    }
    if (lane == 2u) {
        return Vec4f{base0 + base2 + 31.0f, base1 + base3 + 37.0f, base2 + base0 + 41.0f, base3 + base1 + 43.0f};
    }
    if (lane == 3u) {
        return Vec4f{base0 + (step * 2.0f) + 47.0f, base1 + (seed * 2.0f) + 53.0f, base2 + packet.w + 59.0f, base3 + packet.x + 61.0f};
    }
    if (lane == 4u) {
        return Vec4f{base0 + packet.y + 67.0f, base1 + base0 + 71.0f, base2 + packet.x + 73.0f, base3 + seed + 79.0f};
    }
    return Vec4f{base0 + base3 + 83.0f, base1 + packet.w + 89.0f, base2 + step + 97.0f, base3 + packet.z + 101.0f};
}
...
int main() {
    ...
    try {
        ...
        const std::string case_id = env_string("KAIN_GPU_CASE_ID", "orbit_fold_lattice");
        ...
        const uint32_t bias = env_u32("KAIN_GPU_BIAS", 29u);
        ...
        app.pApplicationName = "kain-gpu-benchmark-orbit-fold-lattice";
