...
Vec4f transform_packet(const Vec4f& packet, uint32_t idx, uint32_t bias) {
    const uint32_t lane = (idx + bias) % 5u;
    const float bias_value = static_cast<float>(bias);
    const float step = static_cast<float>((idx % 113u) + 1u);
    const float base0 = packet.x + step + bias_value;
    const float base1 = packet.y + step + 13.0f;
    const float base2 = packet.z + packet.x + 21.0f;
    const float base3 = packet.w + packet.y + 29.0f;
    if (lane == 0u) {
        return Vec4f{base0 + base2, base1 + base0 + 7.0f, base2 + base3, base3 + step};
    }
    if (lane == 1u) {
        return Vec4f{base0 + packet.w + 11.0f, base1 + base2 + 17.0f, base2 + packet.y + 19.0f, base3 + packet.x + 23.0f};
    }
    if (lane == 2u) {
        return Vec4f{base0 + base3 + 31.0f, base1 + packet.z + 37.0f, base2 + base0 + 41.0f, base3 + packet.y + 43.0f};
    }
    if (lane == 3u) {
        return Vec4f{base0 + base1 + 47.0f, base1 + packet.x + 53.0f, base2 + packet.w + 59.0f, base3 + base2 + 61.0f};
    }
    return Vec4f{base0 + packet.y + 67.0f, base1 + base3 + 71.0f, base2 + packet.x + 73.0f, base3 + step + 79.0f};
}
...
int main() {
    ...
    try {
        ...
        const std::string case_id = env_string("KAIN_GPU_CASE_ID", "wave_interleave_fold");
        ...
        app.pApplicationName = "kain-gpu-benchmark-wave-interleave-fold";
