...
# Extensions (PI IS INSTALLED AT "F:\\Scoop\\apps\\pi-coding-agent\\0.78.0")
...
1. `input` event fires - can intercept, transform, or handle
1. If not handled: skill commands (`/skill:name`) expanded to skill content
1. If not handled: prompt templates (`/template`) expanded to template content
1. Agent processing begins (`before_agent_start`, etc.)
