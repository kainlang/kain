...
fn python_commands() -> Vec<(OsString, Vec<OsString>)> {
    ...
    if cfg!(windows) {
        // Prefer a real `python` first: the `py` launcher only knows about
        // registry-registered interpreters and fails with "No installed
        // Python found!" on Scoop/portable layouts even though a working
        // python.exe sits on PATH. `py -3` remains as a fallback.
