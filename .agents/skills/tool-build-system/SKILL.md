...
- On Windows, the managed launcher directory should also emit bare POSIX shims named `kain` and `kn` alongside the `.exe` and `.cmd` wrappers so WSL shells can resolve the command name directly when the repo launcher dir is on PATH.
