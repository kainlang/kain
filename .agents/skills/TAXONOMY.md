...
- Use `bootstrap-semantic-authoring` for intentionally broken Kain fixtures under `crates/semantic/error_corpus`, spec-driven batch authoring under `crates/semantic/batches` and `crates/semantic/templates`, and helper automation under `crates/semantic/scripts`; escalate to `bootstrap-semantic` when the oracle, pack, expert rules, or compiler diagnostic wiring changes.
