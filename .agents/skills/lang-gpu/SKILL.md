...
py -3 -m kaindev --module gpu --limit 80
py -3 -m kaindev --module graphics --contains shader --limit 40
py -3 -m kaindev --module graphics::shared --limit 40
...
## The `dispatch` Keyword — Full Pipeline (Backend-Agnostic)

`dispatch` is a **built-in Kain keyword**. It does NOT require `use std::cuda`,
`use std::gpu`, or any stdlib module. It lowers directly through the compiler to
the GPU runtime, which auto-selects Vulkan or CUDA at runtime.
...
- The compute key string MUST be a literal (not a variable): `"shader::FluidStep::compute"`
- The dispatch dimensions `[X, Y, Z]` are workgroup counts, NOT thread counts
...
Kain source:
  dispatch "shader::FluidStep::compute" [32, 32, 1]
      │
      ▼
Parser (crates/core/src/parser.rs:4513):
  Stmt::Dispatch { compute_key: "shader::FluidStep::compute", dispatch_size: [32, 32, 1] }
      │
      ▼
LLVM Codegen (crates/sys-codegen/src/codegen_llvm/mod.rs:16736):
  call i64 @abi_gpu_dispatch(i8* key_ptr, i64 32, i64 32, i64 1)
      │
      ▼
C Runtime (runtime/native/src/core/cuda_runtime.c:598):
  abi_gpu_dispatch(key, x, y, z)
    → cuda_dispatch_internal(NULL, NULL, key, x, y, z)
      → finds shader_bundle.json + residency sidecars on filesystem
      → loads kain-gpu-runtime.dll (Rust, from crates/gpu-runtime)
      → calls kain_gpu_runtime_dispatch_nvidia_ptx_primary_compute_persisted()
        → selects VulkanComputeExecutor OR NvidiaPtxExecutor at runtime
        → creates pipeline, binds descriptors, dispatches, waits, reads back
```

### Key Truths

- **"cuda" naming is legacy.** `cuda_runtime.c`, `cuda_dispatch_internal`, `abi_cuda_*`
  are all historical names. The actual dispatch handles Vulkan compute just fine.
  The Rust `kain-gpu-runtime` crate provides both `VulkanComputeExecutor` and
  `NvidiaPtxExecutor`.
- **Sidecars are auto-discovered.** The C runtime resolves `shader_bundle.json` and
  `kain_compute_residency.json` from the filesystem via env vars and directory
  search. These are generated by `kain gpu-artifacts` or `kain build`.
- **No `std::cuda` needed.** The `dispatch` keyword works standalone. `std::cuda`
  provides additional helpers (`cuda_dispatch_primary_compute`, `cuda_runtime_ready`,
  warp intrinsics, etc.) but is not required for basic compute dispatch.
- **Derived targets don't block SPIR-V.** WGSL/HLSL generation failures (e.g.,
  `if`-expressions unsupported in WGSL) are now graceful — they produce `None`
  instead of `Err`, so SPIR-V-only targets never fail on derived backend issues.

### Minimal Dispatch Example

```kn
// my_compute.kn — host code
use std::runtime
...
fn main() -> Int with GPU, Unsafe:
    ...
    // Dispatch compute shader: 32x32x1 workgroups
    dispatch "shader::MyKernel::compute" [32, 32, 1]

    let _ = win_MessageBoxA(0, "GPU compute dispatched!", "Kain GPU", 0)
...
```kn
// my_kernel.comp.kn — compute shader
...
## Full GPU Project Workflow (Start to Finish)

This is the end-to-end recipe for a GPU compute project. Follow this and you
won't need to rediscover anything. Reference blade: `blades/c/vulkan_v2/`.

### 1. Project Structure

```text
my_gpu_project/
  KAIN.toml          ← minimal: [project] name + version
  build.kn           ← build orchestration (see below)
  src/
    main.kn          ← host code with dispatch keyword
    kernel.comp.kn   ← compute shader(s)
```
...
That's it. Three lines. No `[c_ffi]` section needed — the libclang C-FFI
system handles `include <windows.h>` and `include <vulkan/vulkan.h>` through
env vars and the system header registry.
...
fn build(ctx: BuildContext) -> BuildGraph:
    let app = project("my-gpu-project")
    ...
    // GPU artifacts: compiles .comp.kn → SPIR-V + sidecars
    let gpu = gpu_suite("gpu-artifacts")
        .compute("src/kernel.comp.kn")
        .targets("spirv")          // spirv | cuda | hlsl | all
        .artifact_root(".kain/out/gpu")
        .requires("check-llvm")

    let exe = native_executable("my-exe")
        .project(app)
        .output("$blade/my_project.exe")
        .requires(check)
        .requires(gpu)
        .inputs(sources)
...
Key build.kn rules:
- `gpu_suite` compiles compute shaders to SPIR-V (or CUDA/PTX, HLSL)
- `.compute("path/to/kernel.comp.kn")` registers a compute shader file
- `.targets("spirv")` targets SPIR-V only (use `"cuda"` for CUDA, `"all"` for everything)
- `native_executable` depends on `gpu` so shaders are compiled before linking
- The build produces: `.spv`, `.gpu.rs`, `.reflect.json`, `.shader_bundle.json`,
  and `kain_compute_residency.json` + `.bin` staging files

### 4. Compute Shader (kernel.comp.kn)
...
Shader rules:
- Use `shader compute Name(id: UVec3) -> Void workgroup(X, Y, Z):`
- `StorageBuffer<Float>` for float buffers, `StorageBuffer<Vec4>` for vec4
- `comptime` block is metadata for the host-side dispatch system
- `if` expressions (`x if cond else y`) work in SPIR-V but NOT WGSL
- Types in shader code: `Float`, `Int`, `UInt`, `Vec2`, `Vec3`, `Vec4`, `UVec3`
- Built-ins: `max(a,b)`, `min(a,b)`, `abs(x)`
- Avoid Kain reserved keywords as variable names (`half`, `sampler`, `input`, `type`, etc.)
...
fn main() -> Int with GPU, Unsafe:
    ...
    let _ = win_MessageBoxA(0, "GPU compute done!", "Kain", 0)
...
Host rules:
- `with GPU, Unsafe` is REQUIRED on any function containing `dispatch`
- `dispatch` takes a string LITERAL (not a variable) for the compute key
- The compute key format is `"shader::KernelName::compute"`
- `runtime_init()` and `runtime_shutdown()` are required for GPU runtime lifecycle
- `include <windows.h> as win` uses libclang for zero-shim C-FFI

### 6. Build & Run

```powershell
# Set Vulkan SDK (Windows)
$env:VULKAN_SDK = "F:/Scoop/apps/vulkan/current"

# Full build (check → gpu-artifacts → executable)
kain build . --target llvm

# Run
kain run . --target llvm

# GPU artifacts only (for debugging shader compilation)
kain gpu-artifacts src/kernel.comp.kn --target spirv --output .kain/out/gpu

# Validate SPIR-V
spirv-val --target-env vulkan1.3 .kain/out/gpu.spv
```

### 7. Debugging Dispatch Failures

If `dispatch` fails silently or returns non-zero:

1. Check the residency sidecars exist: `ls .kain/out/gpu/*residency*`
2. Check the shader bundle exists: `ls .kain/out/gpu/*shader_bundle*`
3. Validate the SPIR-V: `spirv-val --target-env vulkan1.3 .kain/out/gpu/*.spv`
4. Check env vars: `KAIN_CUDA_SHADER_BUNDLE_ENV`, `KAIN_COMPUTE_RESIDENCY_ENV`
5. The C runtime logs paths to `g_cuda_last_*` globals — check with `cuda_last_status()`
6. If SPIR-V generation fails, check shader syntax against known-good examples
   (`smoketest/src/gpu/compute.kn`, `benchmark/cases_v2/gpu_cpu_pipeline.kn`)
7. Derived target failures (WGSL/HLSL) no longer block SPIR-V — they gracefully
   produce `None` since the `crates/driver/src/lib.rs` fix
