...
description: Use when adding, changing, debugging, validating, or reviewing Kain's source checking and testing pipeline, including crates/kain-check, crates/kain-test, the `kain check` and `kain test` CLI commands, compiletest-style `//@` directives, ignored/known-bug cases, JSON reports, runtime `test` execution, and smoketest/kain-test fixtures.
---
...
- `crates/kain-test/src/lib.rs`: directive parsing, pass/fail modes, skipped/ignored cases, suite reports, runtime test dispatch.
...
3. Preserve directive compatibility:
   - support `//@` and `#@`
   - current modes: `check-pass`, `check-fail`, `run-pass`, `run-fail`, `kain-test`
   - current metadata: `mode`, `target`, `error`, `expect-error`, `ignore`, `skip`, `known-bug`
...
5. Keep report schemas explicit:
   ...
   - report `target: "test"` for Kain `test` execution and `target: "run"` for run modes.
...
- Add snapshot comparison, revisions, target-conditional directives, and bless/update workflows inside `kain-test` first.
