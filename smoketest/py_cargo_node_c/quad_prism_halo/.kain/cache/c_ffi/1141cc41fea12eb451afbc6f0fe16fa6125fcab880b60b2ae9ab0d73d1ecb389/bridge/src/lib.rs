...
fn __kain_c_bridge_imagefx_checksum(_env: &mut Env, args: Vec<Value>) -> Result<Value, KainError> {
    ...
    let symbol: Symbol<unsafe extern "C" fn(*const u8, usize) -> i64> = unsafe { library.get(&[105, 109, 97, 103, 101, 102, 120, 95, 99, 104, 101, 99, 107, 115, 117, 109, 0]) }
...
fn __kain_c_bridge_imagefx_signature(_env: &mut Env, args: Vec<Value>) -> Result<Value, KainError> {
    ...
    let checksum = <i64 as FromKainValue>::from_kain_value(iter.next().expect("checked arg count"))? as i64;
    let accent = <i64 as FromKainValue>::from_kain_value(iter.next().expect("checked arg count"))? as std::os::raw::c_int;
    let symbol: Symbol<unsafe extern "C" fn(std::os::raw::c_int, std::os::raw::c_int, i64, std::os::raw::c_int) -> *const std::os::raw::c_char> = unsafe { library.get(&[105, 109, 97, 103, 101, 102, 120, 95, 115, 105, 103, 110, 97, 116, 117, 114, 101, 0]) }
