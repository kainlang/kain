...
`generic_scene_smoke` is the source-first 3D smoke for Kain's native runtime. The smoke source is embedded directly into the native launcher, the top bar is owned by Kain language code, and the shell treats `geometry_fixture` as a first-class runtime scene instead of a bundle example.
...
- a premium `viewport3d` scene bound to the authored `geometry_fixture` runtime profile
...
- a Windows native viewport profile and geometry branch for `geometry_fixture` in the runtime itself
...
- the native Win32 viewport now recognizes `geometry_fixture` as a real scene profile and has a dedicated geometry branch for it
