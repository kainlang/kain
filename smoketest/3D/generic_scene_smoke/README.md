# Generic Scene Showcase

`generic_scene_smoke` is the source-first 3D smoke for Kain's native runtime. The smoke source is embedded directly into the native launcher, the top bar is owned by Kain language code, and the shell treats `geometry_fixture` as authored scene data instead of a runtime-baked demo.
...
- a compact `viewport3d` scene bound to the authored `geometry_fixture` fixture
...
- the Windows native viewport fallback path through the neutral `default_viewport` runtime profile
...
```bash
./target/debug/kain smoketest/3D/generic_scene_smoke/smoke.kn --target llvm --output smoketest/3D/generic_scene_smoke/llvm-native/generic-scene-smoke.ll
```

Windows compatibility shell:

```powershell
powershell -ExecutionPolicy Bypass -File smoketest/3D/generic_scene_smoke/build_visual_exe.ps1
```
...
- the native Win32 viewport now uses one neutral `default_viewport` profile and expects authored scene data to come from Kain bundles or assets
- Linux still lacks a fully native viewport host, so the current smoke remains a compatibility-hosted proof there

Visual example:

- `generic_scene_visual_reference.png` is a deterministic preview artifact generated from the authored `geometry_fixture` scene
- it proves the scene composition and shell presentation path without claiming to be a live vendor-direct viewport capture
- `generated/generic_scene_runtime_report.json` records the backend labels, scene metadata, and frame stats used to produce it
