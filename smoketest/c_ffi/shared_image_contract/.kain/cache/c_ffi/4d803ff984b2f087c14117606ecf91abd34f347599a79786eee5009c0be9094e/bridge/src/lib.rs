...
const SHARED_LIB_PATH: &str = "M:\\Code\\Kain\\smoketest\\c_ffi\\shared_image_contract\\native/image_fx.dll";
...
fn __kain_c_bridge_imagefx_invert_rgba(_env: &mut Env, args: Vec<Value>) -> Result<Value, KainError> {
    if args.len() != 2 {
        return Err(KainError::runtime(format!("imagefx_invert_rgba expected 2 argument(s), got {}", args.len())));
    }
    let library = unsafe { Library::new(SHARED_LIB_PATH) }
        .map_err(|err| KainError::runtime(format!("Failed to load C shared library {}: {err}", SHARED_LIB_PATH)))?;
    let mut iter = args.into_iter();
    ...
    if __len_value < 0 { return Err(KainError::runtime("unsigned C ABI argument cannot be negative".to_string())); }
    let len = __len_value as usize;
    let symbol: Symbol<unsafe extern "C" fn(*mut u8, usize) -> ()> = unsafe { library.get(&[105, 109, 97, 103, 101, 102, 120, 95, 105, 110, 118, 101, 114, 116, 95, 114, 103, 98, 97, 0]) }
        .map_err(|err| KainError::runtime(format!("Missing C symbol {} in {}: {err}", "imagefx_invert_rgba", SHARED_LIB_PATH)))?;
    let result = unsafe { symbol(pixels, len) };
...
fn __kain_c_bridge_imagefx_signature(_env: &mut Env, args: Vec<Value>) -> Result<Value, KainError> {
    if args.len() != 3 {
        return Err(KainError::runtime(format!("imagefx_signature expected 3 argument(s), got {}", args.len())));
    }
    let library = unsafe { Library::new(SHARED_LIB_PATH) }
        .map_err(|err| KainError::runtime(format!("Failed to load C shared library {}: {err}", SHARED_LIB_PATH)))?;
    let mut iter = args.into_iter();
    ...
    let symbol: Symbol<unsafe extern "C" fn(std::os::raw::c_int, std::os::raw::c_int, u64) -> *const std::os::raw::c_char> = unsafe { library.get(&[105, 109, 97, 103, 101, 102, 120, 95, 115, 105, 103, 110, 97, 116, 117, 114, 101, 0]) }
        .map_err(|err| KainError::runtime(format!("Missing C symbol {} in {}: {err}", "imagefx_signature", SHARED_LIB_PATH)))?;
    let result = unsafe { symbol(width, height, checksum) };
...
fn register_all(env: &mut Env) {
    ...
    env.register_native_fn("imagefx_invert_rgba", __kain_c_bridge_imagefx_invert_rgba);
