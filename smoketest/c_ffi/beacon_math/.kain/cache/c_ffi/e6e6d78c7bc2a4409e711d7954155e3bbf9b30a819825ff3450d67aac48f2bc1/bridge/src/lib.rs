...
fn __kain_c_bridge_beacon_scale(_env: &mut Env, args: Vec<Value>) -> Result<Value, KainError> {
    ...
    let symbol: Symbol<unsafe extern "C" fn(std::os::raw::c_int, f64) -> f64> = unsafe { library.get(&[98, 101, 97, 99, 111, 110, 95, 115, 99, 97, 108, 101, 0]) }.map_err(|err| KainError::runtime(format!("Missing C symbol 'beacon_scale' in '{}': {err}", SHARED_LIB_PATH)))?;
