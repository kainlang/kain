...
**CRITICAL**: This is a SOLO DEV project built entirely by one person. When docs mention "other devs", that means OTHER LLMs helping with development, NOT human developers. This is a private tool that will never be public.

**Architecture**: KAIN compiler (Rust) → generates UE5 C++ → Python post-processing for edge cases

**Your Role**: You are an LLM assistant helping the solo dev iterate quickly. Focus on speed and what works NOW, BUT ALSO DO NOT TAKE SHORTCUTS OR ""SIMPLIFY"". FULL IMPLEMENTATIONS ARE THE MOST IMPORTANT. not enterprise best practices for teams.

## Core Principles

1
2. **Type-safe** - Let compiler catch errors
3. **Attribute-driven** - Use `@datatable`, `@component`, `@blueprint`, `@replicated`
4. **Convention-based** - Naming triggers behavior (Server_*, Client_*, Multicast_*)
5. **Production-ready** - Generated code needs zero manual edits
