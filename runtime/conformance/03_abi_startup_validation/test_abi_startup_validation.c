...
int test_runtime_version_exposure(void) {
    ...
    if (info.abi_version_encoded != KAIN_RUNTIME_ABI_VERSION_CURRENT) {
        TEST_FAIL("ABI version mismatch: expected 0x%08X, got 0x%08X",
                  KAIN_RUNTIME_ABI_VERSION_CURRENT, info.abi_version_encoded);
    }
    ...
    if (info.runtime_version_encoded != KAIN_RUNTIME_VERSION_CURRENT) {
        TEST_FAIL("Runtime version mismatch: expected 0x%08X, got 0x%08X",
                  KAIN_RUNTIME_VERSION_CURRENT, info.runtime_version_encoded);
    }
...
int test_abi_compatibility_checking(void) {
    ...
    same_version = KAIN_RUNTIME_ABI_VERSION_CURRENT;
    compatible_version = KAIN_RUNTIME_ABI_VERSION_ENCODE(0, 0, 0);
    incompatible_major = KAIN_RUNTIME_ABI_VERSION_ENCODE(1, 0, 0);
    incompatible_minor = KAIN_RUNTIME_ABI_VERSION_ENCODE(0, 2, 0);
...
int test_startup_mismatch_detection(void) {
    ...
    kain_service_registry_register(
        ...
        KAIN_SERVICE_STATUS_AVAILABLE,
        KAIN_SERVICE_REQUIREMENT_REQUIRED,
        KAIN_RUNTIME_ABI_VERSION_ENCODE(0, 0, 0),  /* Old ABI */
        NULL
    );
