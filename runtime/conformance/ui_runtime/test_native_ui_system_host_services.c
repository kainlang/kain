...
int main(void) {
...
#ifdef _WIN32
    {
        ...
        if (!test_true(kain_native_ui_host_attach(live_session, "win32-gl") == KAIN_NATIVE_UI_INVALID_ARGUMENT, "legacy runtime-owned win32-gl presenter should be rejected")) {
            return 1;
        }
        if (!test_true(strcmp(kain_native_ui_host_backend(live_session), "memory") == 0, "rejected legacy presenter should leave the runtime session on the memory backend")) {
            return 1;
        }
