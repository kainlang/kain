...
int main(void) {
    ...
    printf("Test 1: version_get_info()\n");
    result = version_get_info(&info);
    if (result != 0) {
        printf("  ❌ FAIL: version_get_info() returned %d\n", result);
        return 1;
    }
    ...
    if (info.abi_version_major != RUNTIME_ABI_VERSION_MAJOR ||
    ...
    if (info.runtime_version_major != VERSION_MAJOR ||
    ...
    printf("Test 5: Version Formatting Functions\n");
    {
        ...
        if (written < 0) {
            printf("  ❌ FAIL: version_format_abi() failed\n");
            return 1;
        }

        printf("  Formatted ABI: '%s'\n", buffer);

        written = version_format_runtime(
            VERSION_CURRENT,
            buffer,
            sizeof(buffer)
        );
        ...
        if (written < 0) {
            printf("  ❌ FAIL: version_format_runtime() failed\n");
            return 1;
        }
    ...
    {
        unsigned int same_version = RUNTIME_ABI_VERSION_CURRENT;
        unsigned int compatible_version = RUNTIME_ABI_VERSION_ENCODE(0, 0, 0);
        unsigned int incompatible_major = RUNTIME_ABI_VERSION_ENCODE(1, 0, 0);
        unsigned int incompatible_minor = RUNTIME_ABI_VERSION_ENCODE(0, 2, 0);
