...
BACKEND="all"
COMPILE_TIMEOUT_SEC="${HOT_RELOAD_COMPILE_TIMEOUT_SEC:-300}"
TEST_TIMEOUT_SEC="${HOT_RELOAD_TEST_TIMEOUT_SEC:-20}"
VERBOSE=0
...
Run hot reload and compatibility conformance tests with hard per-step timeouts.
...
TEST_BINARIES=(
    "test_hot_reload_compatibility"
    "test_hot_reload_lifecycle"
)
...
for binary in "${TEST_BINARIES[@]}"; do
    set +e
    run_with_timeout "$COMPILE_TIMEOUT_SEC" "hot reload compilation" \
        "$C_COMPILER" \
        -I"$NATIVE_INCLUDE" \
