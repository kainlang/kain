### This is the template for CLI in kain and this folder will be used instead of a python or c framework for testing as kain has both in its natural interop  
--Wanna know how to write kain code easily ? You have two options my friend 

1. Read the entire x:/docs-tsv folder - its pretty short but all kain docs in the repo, and its 20000 + lines of kain documentation translated to tsv // or you have option 2

2. Literally just resort to grepping , rg, and analyzing `THE_MESSIAH.kn` in this CWD. IT IS 34MB OF KAIN CODE AND CONTAINS EVERY FILE IN THE REPO. It will not guarantee that every example or reference is perfect as some code may be off but it will get you 95% of the way there. GODSPEED MY FRIEND

3. FOR INTEROP references and how kain interacts with python and C - there is roughly 2mb worth of code on interop in "X:\runtime\native\src\ui_v2\tests\reference" -- that is all you need honestly, it shows exactly how it works and the natural incldue and import system.
...
├── README.md         # This file
├── THE_MESSIAH.KN    # 34MB AND 800,000 LOC OF PURE RAW KAIN CODE AND 7000+ FILES AMALGAMATED INTO ONE 
└── src/
