...
int reflection_query_matches_item(
    ...
    if (query->selector_kind == REFLECTION_SELECTOR_NONE ||
        query->selector_kind == REFLECTION_SELECTOR_PRIMARY) {
        return 1;
    }

    if (query->selector_kind == REFLECTION_SELECTOR_ITEM_ID) {
        return query->item_id != 0ull && query->item_id == metadata->item_id;
    }

    if (query->selector_kind == REFLECTION_SELECTOR_TYPE_ID) {
        return query->type_id != 0ull && query->type_id == metadata->type_id;
    }

    if (query->selector_kind == REFLECTION_SELECTOR_NAME) {
        return query->subject_name[0] && strcmp(query->subject_name, metadata->name) == 0;
    }

    if (query->selector_kind == REFLECTION_SELECTOR_HANDLE) {
...
static void reflection_set_diag(
    ...
    kain_diagnostic_create(
        ...
        message,
        detail,
        "runtime/native/src/core/reflection.c"
    );
}
