#include "../../include/kain_runtime_compatibility.h"
...
static int kain_runtime_version_is_compatible(unsigned int required_runtime_version) {
    unsigned int current_major = KAIN_RUNTIME_VERSION_GET_MAJOR(KAIN_RUNTIME_VERSION_CURRENT);
    unsigned int current_minor = KAIN_RUNTIME_VERSION_GET_MINOR(KAIN_RUNTIME_VERSION_CURRENT);
    unsigned int current_patch = KAIN_RUNTIME_VERSION_GET_PATCH(KAIN_RUNTIME_VERSION_CURRENT);
    unsigned int required_major = KAIN_RUNTIME_VERSION_GET_MAJOR(required_runtime_version);
    unsigned int required_minor = KAIN_RUNTIME_VERSION_GET_MINOR(required_runtime_version);
    unsigned int required_patch = KAIN_RUNTIME_VERSION_GET_PATCH(required_runtime_version);
...
void kain_bundle_compat_metadata_init(KainBundleCompatibilityMetadata* metadata) {
    ...
    metadata->required_abi_version = KAIN_RUNTIME_ABI_VERSION_CURRENT;
    metadata->required_runtime_version = KAIN_RUNTIME_VERSION_CURRENT;
...
int kain_bundle_check_abi_compatibility(unsigned int required_abi_version) {
    ...
    return kain_runtime_version_check_abi_compatibility(required_abi_version);
}

int kain_bundle_check_runtime_compatibility(unsigned int required_runtime_version) {
    return kain_runtime_version_is_compatible(required_runtime_version);
}
