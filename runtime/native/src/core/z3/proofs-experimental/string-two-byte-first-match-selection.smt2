; Experimental proof for a packed two-byte substring fast path over the current
; benchmark shape used by benchmark/cases/string_ops/main.kn.
;
; Domain:
...
;
; Candidate:
; - compare each 16-bit little-endian window against a packed 16-bit needle
...
; left-to-right substring scan for every possible 12-byte text and 2-byte needle.
(set-logic QF_BV)
