#include "../../include/kain_runtime_scene.h"

static unsigned long long kain_runtime_scene_pack_handle(
...
static int kain_runtime_append_text(char* out, size_t out_cap, const char* text) {
    size_t offset;
    size_t text_len;
...
static int kain_runtime_append_feature_name(
    ...
    if (emitted_any && *emitted_any) {
        kain_runtime_append_text(out, out_cap, ",");
    }
    if (kain_runtime_append_text(out, out_cap, name) && emitted_any) {
        *emitted_any = 1;
    }
    return 1;
}
...
KainSceneHandle kain_scene_handle_make(
    ...
    handle.value = kain_runtime_scene_pack_handle(kind, slot, generation);
    return handle;
}
...
void kain_scene_query_request_init(KainSceneQueryRequest* request) {
    ...
    ZeroMemory(request, sizeof(*request));
    request->max_hits = KAIN_RUNTIME_SCENE_QUERY_MAX_HITS;
}
...
int kain_scene_query_result_append_hit(
    ...
    if (result->hit_count < 0 || result->hit_count >= KAIN_RUNTIME_SCENE_QUERY_MAX_HITS) {
        return 0;
    }
...
const char* kain_runtime_backend_kind_name(KainRuntimeBackendKind kind) {
    switch (kind) {
        ...
        case KAIN_RUNTIME_BACKEND_SOFTWARE: return "software";
        case KAIN_RUNTIME_BACKEND_UNKNOWN:
        default:
            return "unknown";
    }
}
...
int kain_runtime_format_feature_mask(
    ...
    out[0] = '\0';
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_GRAPHICS,
        "graphics",
        &emitted_any,
        out,
        out_cap
    ...
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_VIEWPORT_INPUT,
        "viewport-input",
        &emitted_any,
        out,
        out_cap
    ...
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_SCENE_QUERY,
        "scene-query",
        &emitted_any,
        out,
        out_cap
    ...
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_SCENE_MUTATION,
        "scene-mutation",
        &emitted_any,
        out,
        out_cap
    ...
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_RUNTIME_REFLECTION,
        "runtime-reflection",
        &emitted_any,
        out,
        out_cap
    ...
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_INGESTION,
        "ingestion",
        &emitted_any,
        out,
        out_cap
    ...
    kain_runtime_append_feature_name(
        feature_mask,
        KAIN_RUNTIME_FEATURE_PACKAGING,
        "packaging",
        &emitted_any,
        out,
        out_cap
    ...
    if (!emitted_any) {
        kain_runtime_append_text(out, out_cap, "none");
    }
