/*
 * KAIN Crash Forensics — macOS signal handler (sigaction)
 *
 * Registered by __kain_crash_platform_register_handlers().
 * Included via macos_sources in the runtime manifest.
 */
...
static void crash_signal_handler(int sig, siginfo_t *info, void *ucontext) {
...
#if defined(__x86_64__)
    {
        ...
        /* macOS: uc->uc_mcontext->__ss.__rip / __rbp */
        fault_ip = (const void *)uc->uc_mcontext->__ss.__rip;
        rbp_val  = (void *)uc->uc_mcontext->__ss.__rbp;
    }
#endif
