 ...
 * VALIDITY — just non-nullness. To give pointers provenance (the
...
/* Static buffer the arena will "manage" — CBMC knows it's a real object */
...
static KainArena* create_valid_arena(void) {
    ...
    /* ── Pointer provenance: all arena pointers point into static buffer ── */
    ...
    /* ── Constrain offsets — low <= high, both within [start, end] ── */
    ...
    /* Marker offsets — constrained to be within the buffer so that
...
void check_arena_available(void) {
    ...
    __CPROVER_assert(avail <= total,
                     "available >= 0 (unsigned) — always true by type");
}
