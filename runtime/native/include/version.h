...
#define RUNTIME_ABI_VERSION_MAJOR 0
#define RUNTIME_ABI_VERSION_MINOR 1
#define RUNTIME_ABI_VERSION_PATCH 0
...
/* Build Metadata */
#define RUNTIME_BUILD_DATE __DATE__
#define RUNTIME_BUILD_TIME __TIME__

/* Version String Formatting */
#define VERSION_STRING_MAX 64
#define RUNTIME_BUILD_INFO_STRING_MAX 128

/* Composite Version Macros */
#define RUNTIME_ABI_VERSION_ENCODE(major, minor, patch) \
    (((major) << 16) | ((minor) << 8) | (patch))

#define RUNTIME_ABI_VERSION_CURRENT \
    RUNTIME_ABI_VERSION_ENCODE( \
...
#define VERSION_GET_MAJOR(version) (((version) >> 16) & 0xFF)
#define VERSION_GET_MINOR(version) (((version) >> 8) & 0xFF)
#define VERSION_GET_PATCH(version) ((version) & 0xFF)

/* Compatibility Check Macros */
#define RUNTIME_ABI_COMPATIBLE(required_major, required_minor) \
    (RUNTIME_ABI_VERSION_MAJOR == (required_major) && \
     RUNTIME_ABI_VERSION_MINOR >= (required_minor))

#define RUNTIME_ABI_EXACT_MATCH(major, minor, patch) \
    (RUNTIME_ABI_VERSION_MAJOR == (major) && \
     RUNTIME_ABI_VERSION_MINOR == (minor) && \
     RUNTIME_ABI_VERSION_PATCH == (patch))

/*
 * Runtime Version Information Structure
 *
 * This structure provides programmatic access to runtime version and build
 * information. It is populated by version_get_info() and used
...
typedef struct {
    ...
    /* Formatted Strings */
    char abi_version_string[VERSION_STRING_MAX];
    char runtime_version_string[VERSION_STRING_MAX];
    char build_info_string[RUNTIME_BUILD_INFO_STRING_MAX];
 ...
 *
 * Parameters:
 *   abi_version_encoded - Encoded ABI version (from RUNTIME_ABI_VERSION_ENCODE)
 *   out - Output buffer
