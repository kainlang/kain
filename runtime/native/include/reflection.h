...
typedef enum {
    REFLECTION_SCOPE_UNKNOWN = 0,
    REFLECTION_SCOPE_SCENE,
    REFLECTION_SCOPE_RESOURCE,
    REFLECTION_SCOPE_BINDING,
    REFLECTION_SCOPE_DEVICE,
    REFLECTION_SCOPE_BUNDLE,
} KainRuntimeReflectionScope;

typedef enum {
    REFLECTION_SELECTOR_NONE = 0,
    REFLECTION_SELECTOR_PRIMARY,
    REFLECTION_SELECTOR_HANDLE,
    REFLECTION_SELECTOR_NAME,
    REFLECTION_SELECTOR_TYPE_ID,
    REFLECTION_SELECTOR_ITEM_ID,
...
void reflection_query_init(KainRuntimeReflectionQuery* query);
void reflection_record_init(KainRuntimeReflectionRecord* record);
int reflection_query_matches_item(
    const KainRuntimeReflectionQuery* query,
    const KainItemMetadata* metadata
);
void reflection_record_from_item(
    const KainRuntimeReflectionQuery* query,
    const KainItemMetadata* metadata,
    KainRuntimeReflectionRecord* record
);
int reflection_format_record(
