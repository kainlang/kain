...
| Actor model | `Partial` | `Broken` | `kain-core` models actors conceptually, and LLVM emits actor-related code, but the raw native layer only has thread spawn plus FIFO queue primitives. Missing supervision, monitors, typed mailboxes, selective receive, actor registry, lifecycle hooks, crash semantics, and remote/distributed actor support. Current LLVM spawn wiring appears to route through `default_actor_run` rather than the emitted actor loop. |
| Actor scheduling/runtime policy | `Scaffold` | `Missing` | Need mailbox ownership rules, scheduler policy, fairness/backpressure, actor sleep/wake integration, crash containment, supervisor restart policy, and metrics/observability. |
| Async/await and futures | `Partial` | `Missing` | Interpreter/runtime-side value handling is richer than the native lane. Native runtime lacks a full async executor, poll/wake machinery, cancellation model, timer integration, and effect-aware task scheduling. |
...
| Component runtime | `Partial` | `Scaffold` | Kain models richer component concepts, but native currently consumes compiled UI projections rather than running a general component model with lifecycle, state, properties, and reactive invalidation. |
| UI runtime | `Partial` | `Partial` | Native UI overlay and compiled-bundle support exist, but a full widget/runtime system does not. Missing diffing, focus, input routing, text editing, accessibility, event bubbling, semantic controls, lifecycle hooks, and state propagation parity. |
...
| Shader system | `Scaffold` | `Missing` | The native runtime reads some bundle metadata, but does not yet provide a full shader artifact loading and execution pipeline. Missing modern pipeline creation, reflection-based bindings, backend abstraction, and artifact lifecycle management. |
| Material system | `Scaffold` | `Missing` | Need material graph/runtime representation, parameter reflection, resource binding, shader specialization, caching, and editor/runtime parity. |
| Compute runtime | `Scaffold` | `Missing` | No actual compute dispatch/runtime layer is present in the C lane. Need pipeline creation, resource binding, dispatch contracts, synchronization, and validation/debug tooling. |
...
| Host extension/plugin bridge | `Partial` | `Missing` | Need dynamic runtime service registration, host plugin loading, capability declaration, stable extension ABI, and parity with richer host integrations used elsewhere. |
| Python/foreign bridge parity | `Partial` | `Missing` | `kain-core` has richer host-runtime ideas than the C runtime currently exposes. Native lacks equivalent bridge contracts, marshaling, lifetime rules, and failure handling. |
| Hot reload | `Scaffold` | `Missing` | Need reload manifests, compatibility classes, migration hooks, state transfer, install/uninstall/update lifecycle APIs, and contract compatibility validation. |
...
| Testing and conformance | `Partial` | `Missing` | Need backend/runtime parity suites, ABI conformance tests, actor semantics tests, reflection/schema tests, hot reload compatibility tests, shader/runtime parity tests, and feature matrices enforced by CI. |
...
| Cross-platform native runtime | `Partial` | `Missing` | The current native host path is strongly Win32-oriented. Need Linux and macOS app host, input, windowing, viewport, timing, sockets, and graphics parity. |
