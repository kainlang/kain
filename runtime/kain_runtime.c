...
char* string_new(char* src) {
    ...
    char* buf = kain_alloc_rc(len + 1, 1);
    strcpy(buf, src);
    return buf;
}
...
char* str_concat(char* s1, char* s2) {
    ...
    char* res = kain_alloc_rc(l1 + l2 + 1, 1);
...
KainArray* array_new(long long cap) {
    KainArray* arr = kain_alloc_rc(sizeof(KainArray), 2);
    arr->len = 0;
    arr->cap = cap < 4 ? 4 : cap;
...
long long array_len(KainArray* arr) { return arr->len; }
...
char* file_read(char* path) {
    ...
    char* buf = kain_alloc_rc(size + 1, 1);
    fread(buf, 1, size, f);
    buf[size] = 0;
...
KainMap* map_new() {
    KainMap* map = kain_alloc_rc(sizeof(KainMap), 3);
    map->capacity = 16;
    map->count = 0;
