...
- New Kain 3D pass (2026-04-15): `SceneCompositionSummary::brief_label()` now names the viewport shape (`portrait`, `square`, or `landscape`) from the aspect ratio, so 3D diagnostics expose both the numerical aspect and the human-readable framing shape. Added a regression assertion that the default square path reports `square`, and that a wide summary reports `landscape`. Validation was blocked by the local Windows GNU linker gap while compiling test dependencies (`x86_64-w64-mingw32-gcc` missing `-lgcc_eh` and `-lgcc`).
...
- New Kain 3D pass (2026-04-14): `material_atrium_smoke` now lifts `scene_shape` and `composition_summary` to the top-level JSON report, so consumers can learn the scene's framing profile without drilling into the first tile entry. This keeps the smoke artifact more machine-friendly for 3D composition tooling and future report diffing. Validation still hits the same Windows GNU linker blocker before the crate can finish building.
