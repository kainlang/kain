...
- Preserve the row's observable result: `194810730`.
...
- Mechanism: Factor the affine lane bias out of the repeated dot product:
  ...
  Compute `base_dot` and `sum_right` once with native SIMD, then fold the 256 phases in scalar integer math.
- Expected upside: Huge. The benchmark currently performs `32,768 * 256 = 8,388,608` lane products; the factored kernel performs `32,768` lane products plus 256 scalar phase updates. Arithmetic work drops by about `254x`, and memory traffic drops from 256 full right/left scans to one full scan.
...
- Local:
  - `benchmark/cases/simd_lane_mix/main.kn`: current Kain row calls `simd_lane_mix_dot(...)` once per phase through `converge`.
  - `benchmark/cases/simd_lane_mix/main.cpp`: C++ baseline repeats the full vectorizable dot 256 times.
  - `benchmark/cases/simd_lane_mix/main.rs`: Rust baseline repeats an explicit AVX2 dot 256 times.
  - `runtime/native/src/core/simd.c`: current native ABI computes exactly one biased dot per call.
