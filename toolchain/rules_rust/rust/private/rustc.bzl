...
def _stage_proc_macro_dependency_symlinks(actions, crate_info, dep_info):
    """Create a compact proc-macro search root for rustc on Windows-heavy graphs."""

    proc_macro_symlinks = []
    seen_proc_macro_basenames = {}
    for dep_crate in dep_info.transitive_crates.to_list():
        dep_crate_info = dep_crate.dep if hasattr(dep_crate, "dep") else dep_crate
        if not _is_proc_macro(dep_crate_info):
            continue
        dylib = dep_crate_info.output
        if dylib.basename in seen_proc_macro_basenames:
            continue
        seen_proc_macro_basenames[dylib.basename] = True
        staged_dylib = actions.declare_file(
            "_proc_macro_dylibs/{}/{}".format(crate_info.output.basename, dylib.basename),
        )
        actions.symlink(
            output = staged_dylib,
            target_file = dylib,
            progress_message = "Staging proc-macro dylib: {}".format(dylib.path),
        )
...
def add_crate_link_flags(
    ...
    args.add_all(extra_dependency_search_paths, uniquify = True, format_each = "-Ldependency=%s")
