...
1. `rules_rust` converts that to a dependent build-script env var:
   - `DEP_PRODUCER_MODERN_VERSION_1_10_0=1`
   - `DEP_PRODUCER_LEGACY_VERSION_1_10_0=1`
1. `consumer_build_rs` reads both and exports:
   - `cargo:rustc-env=METADATA_MODERN_VALUE=1`
   - `cargo:rustc-env=METADATA_LEGACY_VALUE=2`
1. Two `rust_test` targets assert each value independently:
