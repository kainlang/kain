...
pub fn import_usf_file(path: &Path, config: UsfImportConfig) -> Result<Program, UsfImportError> {
    // Check if this is an engine shader
    let is_engine_shader = path
        .to_str()
    ...
    // Read source file
    let source = std::fs::read_to_string(path).map_err(|_| UsfImportError::FileNotFound {
        path: path.to_path_buf(),
    })?;
    ...
    let tree = parser
        .parse(&preprocess_result.output, None)
...
pub fn import_for_research(
    path: &Path,
    engine_shaders_path: &Path,
