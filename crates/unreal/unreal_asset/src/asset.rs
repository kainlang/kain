...
impl Asset<std::io::Cursor<Vec<u8>>> {
    /// Create a new empty asset from scratch, ready for programmatic population.
    ///
    /// This is the key constructor for generating .uasset files without reading
    /// an existing file first. After calling this, use `add_fname()`, `add_import()`,
    /// and push exports to `asset_data.exports`, then call `write_data()`.
    pub fn new_empty(engine_version: EngineVersion) -> Self {
        let name_map = NameMap::new();
        let use_event_driven_loader = false;

        let chain = Chain::new(std::io::Cursor::new(Vec::new()), None);
        let (object_version, object_version_ue5) =
            unreal_asset_base::engine_version::get_object_versions(engine_version);
        ...
        asset_data.set_engine_version(engine_version);

        Asset {
            raw_reader,
            info: String::from("Serialized with KAIN"),
            asset_data,
            ...
            engine_version_recorded: FEngineVersion::new(5, 2, 0, 0, None),
            engine_version_compatible: FEngineVersion::new(5, 2, 0, 0, None),
            chunk_ids: Vec::new(),
            package_source: 0,
