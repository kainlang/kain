...
//! SPIR-V remains the canonical native GPU payload; HLSL is emitted as a backend materialization.
...
fn emit_shader(shader: &TypedShader) -> KainResult<String> {
    ...
    for uniform in &shader.ast.uniforms {
        ...
        ctx.uniform_bindings
            .push((uniform.name.clone(), hlsl_type, uniform.binding));
    }
    ...
    for (name, ty, binding) in &ctx.uniform_bindings {
        ...
        } else if ty.contains("Buffer")
            || ty.contains("RWBuffer")
            || ty.contains("StructuredBuffer")
        {
    ...
    match shader.ast.stage {
        ShaderStage::Compute => {
            ...
            ctx.vars.insert(
                ...
                "dispatchThreadID".to_string(),
            );
            ctx.vars
                .insert("group_thread_id".to_string(), "groupThreadID".to_string());
            ctx.vars
                .insert("group_id".to_string(), "groupID".to_string());
            ctx.vars
                .insert("group_index".to_string(), "groupIndex".to_string());
        ...
        ShaderStage::Vertex => {
            ...
            for param in &shader.ast.inputs {
                ctx.vars
                    .insert(param.name.clone(), format!("input.{}", param.name));
            }
...
fn emit_expr(ctx: &mut HLSLContext, expr: &Expr) -> KainResult<(String, String)> {
    match expr {
        ...
        Expr::If {
            ...
            Err(KainError::codegen(
                "Complex if expressions not yet supported in HLSL backend",
                expr.span(),
            ))
        }
        Expr::Paren(inner, _) => {
...
fn emit_function_call(
    ...
    match name {
        ...
        "sample" => {
            ...
            Ok((
                format!("{}.Sample({}_sampler, {})", sampler, sampler, coords),
                "float4".to_string(),
            ))
        }
        "sample_lod" => {
            ...
            Ok((
                format!(
                    "{}.SampleLevel({}_sampler, {}, {})",
                    sampler, sampler, coords, lod
                ),
                "float4".to_string(),
            ))
        }
        "sample_grad" => {
            ...
            Ok((
                format!(
                    "{}.SampleGrad({}_sampler, {}, {}, {})",
                    sampler, sampler, coords, ddx, ddy
                ),
                "float4".to_string(),
            ))
        }
        ...
        "sample_cmp" => {
            ...
            Ok((
                format!(
                    "{}.SampleCmp({}_sampler, {}, {})",
                    sampler, sampler, coords, compare
                ),
                "float".to_string(),
            ))
        }
        ...
        "noise" => {
            ...
            Ok((
                format!(
                    "frac(sin(dot({}, float2(12.9898, 78.233))) * 43758.5453)",
                    arg_code
                ),
                "float".to_string(),
            ))
        ...
        "noise3d" => {
            ...
            Ok((
                format!(
                    "frac(sin(dot({}, float3(12.9898, 78.233, 37.719))) * 43758.5453)",
                    arg_code
                ),
                "float".to_string(),
            ))
        ...
        "unpack_half_2x16" => {
            ...
            Ok((
                format!(
                    "float2(f16tof32({} & 0xFFFF), f16tof32({} >> 16))",
                    arg_code, arg_code
                ),
                "float2".to_string(),
            ))
        ...
        "texture_size" => {
            ...
            Ok((
                format!("{}.GetDimensions({})", texture, lod),
                "int2".to_string(),
            ))
        }
        "texture_query_lod" => {
            ...
            Ok((
                format!(
                    "{}.CalculateLevelOfDetail({}_sampler, {})",
        ...
        "texture_gather" => {
            ...
            Ok((
                format!(
                    "{}.Gather({}_sampler, {}, {})",
                    texture, texture, coords, component
                ),
                "float4".to_string(),
            ))
        }
        ...
        "mat2" | "Mat2" => {
            ...
            Ok((
                format!("float2x2({})", arg_codes.join(", ")),
                "float2x2".to_string(),
            ))
        }
        "mat3" | "Mat3" => {
            ...
            Ok((
                format!("float3x3({})", arg_codes.join(", ")),
                "float3x3".to_string(),
            ))
        }
        "mat4" | "Mat4" => {
            ...
            Ok((
                format!("float4x4({})", arg_codes.join(", ")),
                "float4x4".to_string(),
            ))
        }
        ...
        "flat" | "noperspective" | "centroid" => {
            ...
            Err(KainError::codegen(
                format!("{} is an interpolation modifier, not a function", name),
                kain_core::span::Span::new(0, 0),
            ))
        }

        // Atomic operations (for compute shaders)
        "atomic_add" | "atomic_sub" | "atomic_min" | "atomic_max" | "atomic_and" | "atomic_or"
        | "atomic_xor" | "atomic_exchange" | "atomic_cas" => {
            ...
            Ok((
                format!("{}({})", hlsl_name, arg_codes.join(", ")),
                "void".to_string(),
            ))
        }
