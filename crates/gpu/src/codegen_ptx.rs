...
use crate::ptx_module::{
    ...
    PtxScalarKind as ModuleScalarKind, DEFAULT_PTX_VERSION,
};
use kain_core::ast::{
...
const PTX_TARGET_ARCH: ModuleArch = ModuleArch::Sm50;
...
fn emit_shader(shader: &TypedShader) -> KainResult<String> {
    ...
    kernel_plan.validate(PTX_TARGET_ARCH).map_err(|err| {
        KainError::codegen(
            format!(
...
mod tests {
    ...
    fn ptx_rejects_non_compute_shader() {
        ...
        let err = emit_shader(&shader).expect_err("fragment shader must fail");
    ...
    fn ptx_rejects_duplicate_runtime_bindings() {
        ...
        let err = emit_shader(&shader).expect_err("duplicate binding must fail");
