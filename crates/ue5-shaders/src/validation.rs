...
impl ShaderValidator {
    ...
    fn load_hlsl_keywords(&mut self, json_data: &str) -> Result<(), String> {
        ...
        if let Some(keywords_obj) = value.get("hlsl_keywords") {
            ...
            if let Some(type_quals) = keywords_obj
                .get("type_qualifiers")
            ...
            if let Some(param_quals) = keywords_obj
                .get("parameter_qualifiers")
                .and_then(|v| v.as_array())
            {
            ...
            if let Some(func_quals) = keywords_obj
                .get("function_qualifiers")
                .and_then(|v| v.as_array())
            {
...
Documentation: https://kain.dev/docs/shaders/types",
                ...
                let is_user_struct = program.map_or(false, |p| {
                    p.items
                        .iter()
                        .any(|item| matches!(item, TypedItem::Struct(s) if s.ast.name == type_name))
                });
    ...
    fn validate_binding_range(
        ...
        if let Type::Named { name, .. } = ty {
            ...
            if type_name.starts_with("Texture")
                || type_name == "Buffer"
                || type_name == "StructuredBuffer"
                || type_name == "ByteAddressBuffer"
            {
                if binding > 127 {
                    errors.push(format!(
    ...
    fn validate_permutation_naming(
        &self,
        shader_name: &str,
        uniform_name: &str,
        ty: &Type,
        ...
        if is_float_type && !is_permutation_name {
            ...
            if uppercase_name.contains("CONFIG")
                || uppercase_name.contains("ENABLE")
                || uppercase_name.contains("DISABLE")
                || uppercase_name.contains("USE")
                || uppercase_name.contains("FEATURE")
                || uppercase_name.contains("QUALITY")
            {
                errors.push(format!(
    ...
    fn validate_pod_struct_definition(
        &self,
        shader_name: &str,
    ...
    fn is_hlsl_compatible_type(&self, ty: &Type) -> bool {
        match ty {
            Type::Named { name, .. } => {
                ...
                let additional_hlsl_types = [
                    // Lowercase variants (HLSL native)
                    "float", "float2", "float3", "float4", "int", "int2", "int3", "int4", "uint",
                    "uint2", "uint3", "uint4", "bool", "bool2", "bool3", "bool4", "half", "half2",
                    "half3", "half4", "double", "double2", "double3", "double4",
                    // Matrix types
                    "float2x2", "float3x3", "float4x4", "float3x4", "float4x3", "matrix", "Matrix",
                ];
        ...
        match ty {
            Type::Named { name, .. } => {
                ...
                match type_name {
                    ...
                    "float" | "Float" | "int" | "Int" | "uint" | "Uint" | "UInt" | "bool"
                    | "Bool" => 4,
    ...
    fn check_pod_struct_redefinitions(
        &self,
        shader_name: &str,
    ...
    fn validate_hlsl_syntax(&self, shader: &TypedShader, errors: &mut Vec<String>) {
        ...
        for uniform in &shader.ast.uniforms {
            ...
            self.validate_hlsl_type_compatibility(
                shader_name,
        ...
        for input in &shader.ast.inputs {
            ...
            self.validate_hlsl_type_compatibility(
                shader_name,
                &format!("input parameter '{}'", input.name),
                &input.ty,
                errors,
            );
        }
    ...
    fn validate_hlsl_type_compatibility(
        &self,
        shader_name: &str,
        context: &str,
        ty: &Type,
...
mod tests {
    use super::*;
    use kain_core::ast::{Block, Param, Shader, Uniform};
    ...
    fn test_validator_creation() {
        ...
        assert!(
            !validator.hlsl_keywords.is_empty(),
            "HLSL keywords should be loaded"
        );
    }
    ...
    fn test_binding_conflict_detection() {
        ...
        assert!(
            errors.iter().any(|e| e.contains("already used")),
            "Should report binding conflict"
        );
    }
    ...
    fn test_hlsl_keyword_conflict() {
        ...
        let errors = result.unwrap_err();
        assert!(
            errors.iter().any(|e| e.contains("reserved keyword")),
            "Should report keyword conflict"
        );
    }
    ...
    fn test_cpp_keyword_conflict_in_shader_name() {
        ...
        assert!(
            result.is_err(),
            "Should detect C++ keyword conflict in shader name"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("C++ keyword") && e.contains("class")),
            "Should report C++ keyword conflict: {:?}",
            errors
        );
    }
    ...
    fn test_cpp_keyword_conflict_in_uniform_name() {
        ...
        assert!(
            result.is_err(),
            "Should detect C++ keyword conflict in uniform name"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("C++ keyword") && e.contains("template")),
            "Should report C++ keyword conflict: {:?}",
            errors
        );
    }
    ...
    fn test_cpp_keyword_conflict_in_input_name() {
        ...
        assert!(
            result.is_err(),
            "Should detect C++ keyword conflict in input parameter name"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("C++ keyword") && e.contains("namespace")),
            "Should report C++ keyword conflict: {:?}",
            errors
        );
    }
    ...
    fn test_multiple_cpp_keyword_conflicts() {
        ...
        assert!(
            result.is_err(),
            "Should detect multiple C++ keyword conflicts"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("C++ keyword") && e.contains("namespace")),
            "Should report 'namespace' conflict: {:?}",
            errors
        );
    }
    ...
    fn test_ue5_macro_name_conflict() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("C++ keyword") && e.to_lowercase().contains("uproperty")),
            "Should report UE5 macro conflict: {:?}",
            errors
        );
    }
    ...
    fn test_valid_names_pass_cpp_check() {
        ...
        if let Err(errors) = result {
            assert!(
                !errors.iter().any(|e| e.contains("C++ keyword")),
                "Should not report C++ keyword conflicts for valid names: {:?}",
                errors
            );
        }
    }
    ...
    fn test_function_signature_validation_return_type() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("return type") && e.contains("String")),
            "Should report invalid return type: {:?}",
            errors
        );
    }
    ...
    fn test_function_signature_validation_parameter_type() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("input parameter") && e.contains("array")),
            "Should report invalid parameter type: {:?}",
            errors
        );
    }
    ...
    fn test_permutation_naming_valid_cfg() {
        ...
        assert!(
            result.is_ok(),
            "Valid CFG_* permutation should pass: {:?}",
            result
        );
    }
    ...
    fn test_permutation_naming_valid_enable() {
        ...
        assert!(
            result.is_ok(),
            "Valid ENABLE_* permutation should pass: {:?}",
            result
        );
    }
    ...
    fn test_permutation_naming_invalid_type() {
        ...
        let errors = result.unwrap_err();
        assert!(
            errors.iter().any(|e| e.contains("should have Float type")),
            "Should report type error for permutation: {:?}",
            errors
        );
    }
    ...
    fn test_permutation_naming_invalid_case() {
        ...
        let errors = result.unwrap_err();
        assert!(
            errors.iter().any(|e| e.contains("should be all uppercase")),
            "Should report case error for permutation: {:?}",
            errors
        );
    }
    ...
    fn test_permutation_naming_suggestion() {
        ...
        assert!(
            result.is_err(),
            "Float uniform with config-like name should get suggestion"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("Consider renaming to CFG_")
                    || e.contains("Consider renaming to ENABLE_")),
            "Should suggest permutation naming: {:?}",
            errors
        );
    }
    ...
    fn test_permutation_naming_multiple_permutations() {
        ...
        assert!(
            result.is_ok(),
            "Multiple valid permutations should pass: {:?}",
            result
        );
    }
    ...
    fn make_test_program_with_struct(struct_name: &str, fields: Vec<Field>) -> TypedProgram {
        TypedProgram {
            items: vec![TypedItem::Struct(kain_core::types::TypedStruct {
                ast: Struct {
    ...
    fn test_uniform_user_struct_type_is_accepted() {
        ...
        let program =
            make_test_program_with_struct("GpuParams", vec![make_field("albedo", "Vec4")]);
        ...
        if let Err(errors) = result {
            assert!(
                !errors.iter().any(|e| e
                    .contains("Uniform 'params' has potentially invalid HLSL type 'GpuParams'")),
    ...
    fn test_pod_struct_valid_hlsl_types() {
        ...
        let program = make_test_program_with_struct(
            "ShaderParams",
            vec![
        ...
        if let Err(errors) = result {
            ...
            assert!(
                !errors
                    .iter()
                    .any(|e| e.contains("non-HLSL-compatible type")),
                "Should not report type errors for valid HLSL types: {:?}",
                errors
            );
        }
    }
    ...
    fn test_pod_struct_invalid_string_type() {
        ...
        let program = make_test_program_with_struct(
            "InvalidParams",
            vec![make_field("name", "String"), make_field("value", "Float")],
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("String") && e.contains("not supported")),
            "Should report String type error: {:?}",
            errors
        );
    }
    ...
    fn test_pod_struct_alignment_check() {
        ...
        let program_aligned = make_test_program_with_struct(
            "AlignedParams",
            vec![
                make_field("color", "Vec3"),
                make_field("intensity", "Float"),
        ...
        if let Err(errors) = &result {
            assert!(
                !errors.iter().any(|e| e.contains("not 16-byte aligned")),
                "Should not report alignment error for 16-byte aligned struct: {:?}",
                errors
            );
        }
        ...
        let program_unaligned =
            make_test_program_with_struct("UnalignedParams", vec![make_field("color", "Vec3")]);

        let shader2 = make_test_shader(
            "TestShader2",
            vec![Uniform {
                name: "params".to_string(),
                ty: Type::Named {
    ...
    fn test_pod_struct_array_type() {
        ...
        let errors = result.unwrap_err();
        assert!(
            errors.iter().any(|e| e.contains("array type")),
            "Should report array type error: {:?}",
            errors
        );
    }
    ...
    fn test_pod_struct_redefinition_warning() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("may conflict") || e.contains("redefinition")),
            "Should warn about redefinition: {:?}",
            errors
        );
    }
    ...
    fn test_pod_struct_no_program_provided() {
        ...
        assert!(
            result.is_ok(),
            "Should handle None program gracefully: {:?}",
            result
        );
    }
    ...
    fn test_binding_texture_conflict() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("t0") && e.contains("already used")),
            "Should report texture binding conflict: {:?}",
            errors
        );
    }
    ...
    fn test_binding_sampler_conflict() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("t0") && e.contains("already used")),
            "Should report sampler binding conflict (samplers use t-register): {:?}",
            errors
        );
    }
    ...
    fn test_binding_uav_conflict() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("u0") && e.contains("already used")),
            "Should report UAV binding conflict: {:?}",
            errors
        );
    }
    ...
    fn test_binding_cbuffer_conflict() {
        ...
        assert!(
            result.is_err(),
            "Should detect scalar ordering index conflict"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("ordering index @1") && e.contains("already used")),
            "Should report scalar ordering index conflict: {:?}",
            errors
        );
    }
    ...
    fn test_binding_texture_exceeds_limit() {
        ...
        assert!(
            result.is_err(),
            "Should detect texture binding exceeds limit"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("exceeds D3D11 texture register limit")),
            "Should report texture binding limit exceeded: {:?}",
            errors
        );
    }
    ...
    fn test_binding_sampler_exceeds_limit() {
        ...
        assert!(
            result.is_err(),
            "Should detect sampler binding exceeds limit"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("exceeds D3D11 texture register limit")),
            "Should report sampler binding limit exceeded (samplers use t-register): {:?}",
            errors
        );
    }
    ...
    fn test_binding_uav_exceeds_limit() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("exceeds D3D11 UAV register limit")),
            "Should report UAV binding limit exceeded: {:?}",
            errors
        );
    }
    ...
    fn test_binding_cbuffer_b0_not_reserved_for_shader_params() {
        ...
        if let Err(errors) = result {
            assert!(
                !errors
                    .iter()
                    .any(|e| e.contains("b0") && e.contains("reserved for UE5 View")),
                "Should NOT report b0 reserved error for SHADER_PARAMETER_STRUCT members: {:?}",
                errors
            );
        }
    }
    ...
    fn test_binding_valid_separate_resource_types() {
        ...
        assert!(
            result.is_ok(),
            "Valid separate bindings should pass: {:?}",
            result
        );
    }
    ...
    fn test_binding_buffer_types() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("t0") && e.contains("already used")),
            "Should report buffer binding conflict: {:?}",
            errors
        );
    }
    ...
    fn test_binding_multiple_conflicts() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("t0") && e.contains("already used")),
            "Should report texture conflict: {:?}",
            errors
        );
        assert!(
            errors
                .iter()
                .any(|e| e.contains("exceeds maximum sampler slot")),
            "Should report sampler limit: {:?}",
            errors
        );
    ...
    fn test_invalid_type_error_lists_valid_types() {
        ...
        assert!(
            error_msg.contains("Valid KAIN types:"),
            "Error should list valid types: {}",
            error_msg
        );
        assert!(
            error_msg.contains("Float"),
            "Error should list Float: {}",
            error_msg
        );
        assert!(
            error_msg.contains("Vec3"),
            "Error should list Vec3: {}",
            error_msg
        );
        assert!(
            error_msg.contains("UVec2"),
            "Error should list UVec2: {}",
            error_msg
        );
        assert!(
            error_msg.contains("Mat4"),
            "Error should list Mat4: {}",
            error_msg
        );
    }
    ...
    fn test_classify_uniform_texture_types() {
        ...
        assert_eq!(
            classify_uniform_type("Texture1DArray"),
            UniformClass::Texture
        );
        assert_eq!(
            classify_uniform_type("Texture2DArray"),
            UniformClass::Texture
        );
        assert_eq!(
            classify_uniform_type("TextureCubeArray"),
            UniformClass::Texture
        );
        ...
        assert_eq!(
            classify_uniform_type("Texture2DMSArray"),
            UniformClass::Texture
        );
    }
    ...
    fn test_classify_uniform_sampler_types() {
        ...
        assert_eq!(
            classify_uniform_type("SamplerComparisonState"),
            UniformClass::Texture
        );
    ...
    fn test_classify_uniform_buffer_types() {
        ...
        assert_eq!(
            classify_uniform_type("StructuredBuffer"),
            UniformClass::Texture
        );
        assert_eq!(
            classify_uniform_type("ByteAddressBuffer"),
            UniformClass::Texture
        );
    }
    ...
    fn test_classify_uniform_uav_types() {
        ...
        assert_eq!(
            classify_uniform_type("RWStructuredBuffer"),
            UniformClass::UAV
        );
        assert_eq!(
            classify_uniform_type("RWByteAddressBuffer"),
            UniformClass::UAV
        );
        ...
        assert_eq!(
            classify_uniform_type("RWTexture2D_Float"),
            UniformClass::UAV
        );
        assert_eq!(
            classify_uniform_type("RWTexture2D_Float2"),
            UniformClass::UAV
        );
        assert_eq!(
            classify_uniform_type("RWTexture2D_Float3"),
            UniformClass::UAV
        );
        assert_eq!(
            classify_uniform_type("RWTexture2D_Float4"),
            UniformClass::UAV
        );
    ...
    fn test_classify_uniform_user_struct_types() {
        ...
        assert_eq!(
            classify_uniform_type("MyCustomStruct"),
            UniformClass::Scalar
        );
        ...
        assert_eq!(
            classify_uniform_type("LightingParams"),
            UniformClass::Scalar
        );
    }
    ...
    fn test_classify_uniform_edge_cases() {
        ...
        assert_eq!(
            classify_uniform_type("SamplerConfig"),
            UniformClass::Texture
        ); // Still classified as Texture due to substring
    ...
    fn test_uniform_classification_semantics_texture() {
        ...
        let shader_valid = make_test_shader(
            "TestShader",
            vec![Uniform {
                name: "albedo_map".to_string(),
                ty: Type::Named {
        ...
        let shader_invalid = make_test_shader(
            "TestShader",
            vec![Uniform {
                name: "albedo_map".to_string(),
                ty: Type::Named {
        ...
        assert!(
            result.is_err(),
            "Texture uniform with binding 128 should be invalid"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("exceeds maximum texture slot t127")),
            "Should report texture binding limit exceeded: {:?}",
            errors
        );
    }
    ...
    fn test_uniform_classification_semantics_uav() {
        ...
        assert!(
            result.is_err(),
            "UAV uniform with binding 64 should be invalid"
        );
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.contains("exceeds maximum UAV slot u63")),
            "Should report UAV binding limit exceeded: {:?}",
            errors
        );
    }
    ...
    fn test_uniform_classification_mixed_types() {
        ...
        if let Err(errors) = &result {
            ...
            assert!(
                errors.iter().all(
                    |e| e.contains("multiple resource types") || e.contains("can be confusing")
                ),
                "Should only have cross-resource-type warnings: {:?}",
                errors
            );
        }
    }
