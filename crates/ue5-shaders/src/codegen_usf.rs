...
fn detect_needed_helpers(shader: &TypedShader) -> Vec<&'static str> {
    ...
    let has_input_texture = shader.ast.uniforms.iter().any(|u| {
        matches!(&u.ty, Type::Named { name, .. } if name == "Sampler2D" || name == "Texture2D")
    });
...
pub fn generate_shared_shader_library(program: &TypedProgram, plugin_name: &str) -> Option<String> {
    // Collect all shaders
    let shaders: Vec<&TypedShader> = program.items.iter().filter_map(|item| {
        if let TypedItem::Shader(s) = item { Some(s) } else { None }
    }).collect();
    ...
    let emitted_helpers: Vec<&SharedHelper> = SHARED_HELPERS.iter()
    ...
    out.push_str(&format!("// Common helpers for {} shaders in {}\n", shaders.len(), plugin_name));
...
pub fn shader_needs_shared_library(shader: &TypedShader, program: &TypedProgram) -> bool {
    ...
    let shader_count = program.items.iter().filter(|i| matches!(i, TypedItem::Shader(_))).count();
    if shader_count < 2 {
        return false;
    }
...
mod tests {
    ...
    fn test_scalar_binding_zero_is_valid() {
        ...
        assert!(result.is_ok(), "Scalar binding 0 should be valid: {:?}", result);
    }
    ...
    fn test_validation_error_formatting() {
        ...
        assert!(err_msg.contains("❌ Shader validation failed"), 
            "Should have formatted header, got: {}", err_msg);
        assert!(err_msg.contains("1."), "Should number errors, got: {}", err_msg);
        assert!(err_msg.contains("2."), "Should have multiple errors, got: {}", err_msg);
        assert!(err_msg.contains("💡 Suggestion"), "Should include suggestions, got: {}", err_msg);
        assert!(err_msg.contains("Fix these issues and rebuild"), 
            "Should include helpful footer, got: {}", err_msg);
    }
    ...
    fn test_validation_single_usf_generation() {
        ...
        let err_msg = result.unwrap_err().to_string();
        assert!(err_msg.contains("HLSL reserved keyword"), 
            "Error should mention HLSL keyword conflict, got: {}", err_msg);
    }
    ...
    fn test_shared_library_not_generated_for_single_shader() {
        let program = make_compute_shader("SingleShader", vec![
            make_uniform("texture_size", "UVec2", 1),
        ]);

        let result = generate_shared_shader_library(&program, "TestPlugin");
        assert!(result.is_none(), "Single shader should not generate shared library");
    }
    ...
    fn test_shared_library_generated_for_multi_shader() {
        ...
        let shader1 = make_compute_shader_with_uniforms("GenerateRadial", vec![
            make_uniform("texture_size", "UVec2", 0),
            make_uniform("falloff", "Float", 1),
        ]);
        let shader2 = make_compute_shader_with_uniforms("GenerateCircle", vec![
            make_uniform("texture_size", "UVec2", 0),
        ...
        assert!(result.is_some(), "Multi-shader program should generate shared library");

        let ush = result.unwrap();
        assert!(ush.contains("#pragma once"), "Should have pragma once");
        assert!(ush.contains("IsInBounds"), "Should contain bounds check helper");
        assert!(ush.contains("PixelToUV"), "Should contain UV computation helper");
    ...
    fn test_shared_library_hash_noise_detection() {
        ...
        let shader1 = make_compute_shader_with_uniforms("GenPerlin", vec![
            make_uniform("texture_size", "UVec2", 0),
            make_uniform("scale", "Float", 1),
            make_uniform("seed", "Float", 2),
        ]);
        let shader2 = make_compute_shader_with_uniforms("GenVoronoi", vec![
            make_uniform("texture_size", "UVec2", 0),
            make_uniform("scale", "Float", 1),
            make_uniform("seed", "Float", 2),
        ]);
        ...
        let ush = result.unwrap();
        assert!(ush.contains("HashNoise"), "Should contain hash noise helper");
        assert!(ush.contains("GrayscaleOutput") || ush.contains("Grayscale"),
