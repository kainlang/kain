...
fn main():
    ...
    assert!(
        names.iter().any(|n| n.contains("identity")),
        "Should have identity functions"
    );
...
fn test_max():
    ...
    assert!(
        names.iter().any(|n| n.contains("max")),
        "Should have max functions"
    );
}
...
fn test_nested():
    ...
    assert!(
        names.iter().any(|n| n.contains("double_identity")),
        "Should have double_identity functions"
    );
}
...
fn make_float_box() -> Box<Float>:
    ...
    assert!(
        struct_names.iter().any(|n| n.contains("Box")),
        "Should have Box structs"
    );
    assert!(
        struct_count >= 2,
        "Expected at least 2 struct instantiations, got {}",
        struct_count
    );
}
...
fn make_pair() -> Pair<Int, String>:
    ...
    assert!(
        struct_names.iter().any(|n| n.contains("Pair")),
        "Should have Pair struct"
    );
}
...
fn make_container() -> Container<String>:
    ...
    assert!(
        struct_names.iter().any(|n| n.contains("Container")),
        "Should have Container structs"
    );
    assert!(
        struct_names.len() >= 2,
        "Should have at least 2 struct instantiations"
    );
}
...
fn use_box():
    ...
    assert!(
        func_names.iter().any(|n| n.contains("Box_Int_get")),
        "Should have Box_Int_get method"
    );
    assert!(
        func_names.iter().any(|n| n.contains("Box_Int_set")),
        "Should have Box_Int_set method"
    );
    ...
    assert!(
        struct_names.iter().any(|n| n.contains("Box_Int")),
        "Should have Box_Int struct"
    );
}
...
fn use_pair():
    ...
    assert!(
        func_names
            .iter()
            .any(|n| n.contains("Pair_Int_String_get_first")),
        "Should have Pair_Int_String_get_first method"
    );
    assert!(
        func_names
            .iter()
            .any(|n| n.contains("Pair_Int_String_get_second")),
        "Should have Pair_Int_String_get_second method"
    );
}
...
fn process_float_container(c: Container<Float>) -> Float:
    ...
    assert!(
        func_names.iter().any(|n| n.contains("Container_Int_get")),
        "Should have Container_Int_get method"
    );
    assert!(
        func_names.iter().any(|n| n.contains("Container_Float_get")),
        "Should have Container_Float_get method"
    );

    // Should have the process functions
    assert!(
        func_names.contains(&"process_int_container"),
        "Should have process_int_container function"
    );
    assert!(
        func_names.contains(&"process_float_container"),
        "Should have process_float_container function"
    );
}
