...
pub fn import_asm(
    ...
    let raw = fs::read_to_string(input).map_err(AsmError::Io)?;
    ...
    if !validate_only {
        ...
        let map_json = serde_json::to_string_pretty(&furby_map)
            .map_err(|e| AsmError::runtime(format!("Failed to serialize furby map: {}", e)))?;
        fs::write(&map_json_path, map_json).map_err(AsmError::Io)?;
    }

    let report_json = serde_json::to_string_pretty(&report)
        .map_err(|e| AsmError::runtime(format!("Failed to serialize recovery report: {}", e)))?;
...
fn parse_data_line(line: &str) -> Option<(String, Vec<String>)> {
    let upper = line.to_ascii_uppercase();
    let (marker, pos) = if let Some(p) = upper.find(" DB ") {
        ("DB", p + 1)
    } else if upper.starts_with("DB ") {
        ("DB", 0)
    } else if let Some(p) = upper.find(" DW ") {
        ("DW", p + 1)
...
fn expand_compound_line(line: &str) -> Vec<String> {
    ...
    while i < tokens.len() {
        if i + 1 < tokens.len() && tokens[i + 1] == ":" {
            out.push(format!("{}:", sanitize_token(&tokens[i])));
            i += 2;
            continue;
        }
