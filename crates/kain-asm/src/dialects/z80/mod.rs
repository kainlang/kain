...
const SUPPORTED_FORMATS: &[&str] = &["z80", "z80-arcade", "z80-spectrum", "z80-msx"];
...
fn parse_asm_program(lines: &[CanonLine]) -> (AsmProgram, Vec<SourceProvenance>) {
    ...
    for l in lines {
        ...
        if cur_label.is_empty() {
            cur_label = format!("z80_entry_{}", l.canon);
            cur_start = l.canon;
            cur_file = l.file.clone();
            cur_src_start = l.line;
            cur_src_end = l.line;
...
fn build_translit_units(program: &AsmProgram) -> Vec<TranslitUnit> {
    program
        ...
        .map(|b| TranslitUnit {
            ...
            target_item: format!("z80_{}", normalize_identifier(&b.label)),
...
fn render_kain_firmware(program: &AsmProgram, units: &[TranslitUnit]) -> String {
    let mut out = String::new();
    out.push_str("# Generated by kain import-asm --format z80\n");
    out.push_str("# Z80 transliteration seed\n");
...
fn build_rom_model(lines: &[CanonLine], program: &AsmProgram) -> Z80RomModel {
    let mut pc = 0u16;
...
mod tests {
    ...
    fn z80_registers_in_cpu_state() {
        ...
        assert!(firmware.contains("iy: Int"));
        assert!(firmware.contains("iff1: Int"));
        assert!(firmware.contains("iff2: Int"));
    }

    #[test]
    fn parse_hex_literals() {
        assert_eq!(parse_num("$42"), Some(0x42));
        assert_eq!(parse_num("0x42"), Some(0x42));
        assert_eq!(parse_num("42H"), Some(0x42));
        assert_eq!(parse_num("42h"), Some(0x42));
    }

    #[test]
    fn parse_binary_literals() {
        assert_eq!(parse_num("%10101010"), Some(0b10101010));
        assert_eq!(parse_num("10101010B"), Some(0b10101010));
        assert_eq!(parse_num("10101010b"), Some(0b10101010));
    }
    ...
    fn import_writes_outputs() {
        ...
        let result = import_asm(&input, "z80", None, false).expect("import");
    ...
    #[test]
    fn parity_harness_builds_call_graph() {
        ...
        let harness = build_parity_harness(&program);
        assert_eq!(harness.graph_nodes, 2);
        assert_eq!(harness.graph_edges, 1);
    }
}
