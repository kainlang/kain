...
mod tests {
    ...
    fn command_run_captures_status_stdout_and_stderr() {
        let dir = tempdir().unwrap();
        let (command, args, stdout_needle, stderr_needle) = if cfg!(windows) {
            (
                "cmd.exe",
                vec![
                    "/d".to_string(),
                    "/c".to_string(),
                    "(echo codebase-stdout)&(echo codebase-stderr 1>&2)".to_string(),
                ],
                "codebase-stdout",
                "codebase-stderr",
            )
            ...
            (
                ...
                vec![
                    "-c".to_string(),
                    "printf 'codebase-stdout\\n'; printf 'codebase-stderr\\n' >&2".to_string(),
                ],
                "codebase-stdout",
                "codebase-stderr",
            )
        };
        let result = run_command(dir.path(), command, &args).unwrap();
        assert!(result.success);
        assert!(result.stdout.contains(stdout_needle));
        assert!(result.stderr.contains(stderr_needle));
