...
fn find_kain_launcher() -> PathBuf {
    ...
    if let Ok(current_exe) = std::env::current_exe() {
        ...
        if let Some(candidate) = find_repo_local_kain_launcher(&current_exe) {
            return candidate;
        }
    ...
    if let Ok(current_dir) = std::env::current_dir() {
        if let Some(candidate) = find_repo_local_kain_launcher(&current_dir) {
            return candidate;
        }
    }
...
fn find_repo_local_kain_launcher(start: &Path) -> Option<PathBuf> {
    ...
    for ancestor in search_root.ancestors() {
        if let Some(candidate) = find_latest_repo_local_staged_kain_binary(ancestor) {
            return Some(candidate);
        }
    }
    let repo_local_relatives = if cfg!(target_os = "windows") {
        [
            ".kain/bin/kain.exe",
            ".kain/bin/kn.exe",
    ...
    for ancestor in search_root.ancestors() {
        for relative in repo_local_relatives {
...
fn find_latest_repo_local_staged_kain_binary(root: &Path) -> Option<PathBuf> {
    let staged_dir = root.join(".kain").join("state").join("bin");
    ...
    for entry in entries.flatten() {
        ...
        if !name.starts_with("kain-") && !name.starts_with("kn-") {
            continue;
        }
        if cfg!(target_os = "windows") && path.extension() != Some(OsStr::new("exe")) {
            continue;
        }
        let modified = entry
            .metadata()
...
mod tests {
    ...
    fn lock_process_context() -> MutexGuard<'static, ()> {
...
"#,
        ...
        .unwrap();
        kfs::write_text(
            fmt_root.join("src").join("workspace_fmt.kn"),
            r#"
pub fn workspace_join_strings(items: Array<String>, separator: String) -> String:
    let output = ""
    let index = 0
...
"#,
        ...
        kfs::write_text(
            ...
            r#"
use workspace_fmt::workspace_join_strings

fn main() -> String:
    return workspace_join_strings(["one", "two"], ",")
"#,
        )
        .unwrap();
