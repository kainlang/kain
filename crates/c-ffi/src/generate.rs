...
pub fn write_generated_artifacts(
    ...
    kfs::atomic_write_text(&manifest_json_path, &manifest_json).map_err(fs_to_kain_error)?;
    kfs::atomic_write_text(
        &packaged_bridge_manifest_path,
        &packaged_bridge_manifest_json,
    )
    .map_err(fs_to_kain_error)?;
    kfs::atomic_write_text(&bridge_manifest_path, &render_bridge_manifest(resolved))
        .map_err(fs_to_kain_error)?;
...
fn render_bridge_manifest(resolved: &ResolvedCLibrary) -> String {
    ...
    format!(
        "[package]\nname = {:?}\nversion = \"0.1.0\"\nedition = \"2021\"\n\n[lib]\ncrate-type = [\"cdylib\"]\n\n[dependencies]\nkain-core = {{ path = {:?} }}\nkain-interop = {{ path = {:?} }}\nlibloading = \"0.8\"\n\n[profile.dev]\ndebug = 0\npanic = \"unwind\"\n\n[workspace]\nresolver = \"2\"\n",
        crate_name,
        repo_root.join("crates").join("core").display().to_string(),
        repo_root
            .join("crates")
            .join("interop")
            .display()
