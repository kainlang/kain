...
impl LlvmGenerator {
    ...
    fn normalize_windows_tls_section_name_for_coff(authored: &str) -> String {
        ...
        // Proof: crates/sys-codegen/z3/proofs/control-windows-custom-tls-prefix-sorts-before-crt-terminator.yaml
    ...
    fn expr_needs_shared_value_retain(&self, expr: &Expr, llvm_ty: &str) -> bool {
        ...
        match Self::expr_strip_parens(expr) {
            // Proof: crates/sys-codegen/z3/proofs/memory-rc-escape-transfer-balance.yaml
    ...
    fn cast_numeric_value(
        ...
        match (src_ty, dst_ty) {
            ...
            ("double", "i1") => {
                // Proof: crates/sys-codegen/z3/proofs/casts-double-to-bool-unordered-nonzero-aligns-with-runtime-truthiness.yaml
    ...
    // Proof: crates/sys-codegen/z3/proofs/casts-double-to-int-saturating-intrinsic-preserves-in-range-raw-cast.yaml
    ...
    fn emit_externs(&mut self) {
        ...
        self.emit("declare i64 @kain_shared_buffer_element_size(i64)");
        self.emit("declare i64 @kain_shared_buffer_zero_copy_flag(i64)");
        self.emit("declare i64 @kain_shared_buffer_shared_ownership(i64)");
        self.emit("declare void @kain_shared_buffer_release(i64)");
            ...
            Stmt::Return(expr, _) => {
                ...
                if let Some(e) = expr {
                    ...
                    if ty == "i64" && self.expr_owns_json_value(e) && !self.is_new_object(e) {
                        // Proof: crates/sys-codegen/z3/proofs/json_owned_return_transfer.smt2
                        self.emit(&format!("  call void @json_retain(i64 {})", val));
                    }
    ...
    fn compile_direct_call(
        ...
        for (index, arg) in args.iter().enumerate() {
            ...
            if expects_json_value {
                // Proof: crates/sys-codegen/z3/proofs/json_any_tag_partition.smt2
