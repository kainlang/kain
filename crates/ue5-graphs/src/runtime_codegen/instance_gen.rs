...
impl InstanceGenerator {
    ...
    pub fn generate_instance_header(&self) -> Result<String> {
        ...
        lines.push(format!("#include \"{}Instance.generated.h\"", self.graph.name));
        lines.push(String::new());
        ...
        lines.push(format!("enum class E{}ResetReason : uint8", self.graph.name));
        lines.push(format!("{{"));
        ...
        lines.push(format!("class {}_API {} : public UObject", 
            self.plugin_name.to_uppercase(), class_name));
        lines.push(format!("{{"));
        ...
        lines.push(format!("\tUFUNCTION(BlueprintPure, Category = \"{}|Instance\")", self.graph.name));
        lines.push(format!("\tconst {}* GetCurrentNode() const;", node_data_class));
        lines.push(String::new());
        
        // Protected interface
        lines.push(format!("protected:"));
        lines.push(format!("\t// Construction and destruction"));
        lines.push(format!("\tvirtual bool ConstructInstance({}* InGraphAsset);", asset_class));
        ...
        lines.push(format!("\tvirtual bool SetCurrentNode(const {}* ToNode);", node_data_class));
        lines.push(String::new());
        
        lines.push(format!("\t// Blueprint events"));
        lines.push(format!("\tUFUNCTION(BlueprintNativeEvent, Category = \"{}|Instance\")", self.graph.name));
        ...
        lines.push(format!("\tF{}InstanceResetDelegate OnInstanceReset;", self.graph.name));
        lines.push(String::new());
        
        lines.push(format!("\t// State flags"));
        lines.push(format!("\tUPROPERTY(Transient, BlueprintReadOnly, Category = \"{}|Instance\")", self.graph.name));
        ...
        lines.push(format!("\tTWeakObjectPtr<const {}> CurrentNode;", node_data_class));
        lines.push(format!("}};"));
    ...
    pub fn generate_instance_source(&self) -> Result<String> {
        ...
        // ResetInstance
        lines.push(format!("bool {}::ResetInstance(E{}ResetReason ResetReason)",
        ...
        lines.push(format!("{{"));
        lines.push(format!("\treturn GraphAsset != nullptr && bInstanceActive;"));
        lines.push(format!("}}"));
        lines.push(String::new());
        
        // GetGraphAsset
        lines.push(format!("const {}* {}::GetGraphAsset() const",
        ...
        // GetCurrentNode
        lines.push(format!("const {}* {}::GetCurrentNode() const",
        ...
        // ConstructInstance
        lines.push(format!("bool {}::ConstructInstance({}* InGraphAsset)",
        ...
        // SetInstanceActive
        lines.push(format!("void {}::SetInstanceActive(bool bActive)", class_name));
        lines.push(format!("{{"));
        ...
        lines.push(format!("\t// This should find the next node based on current node and connections"));
        lines.push(String::new());
        lines.push(format!("\tbNeedProceed = false;"));
        lines.push(format!("\treturn true;"));
        lines.push(format!("}}"));
        ...
        // SetCurrentNode
        lines.push(format!("bool {}::SetCurrentNode(const {}* ToNode)",
        ...
        lines.push(format!("bool {}::IsProceedBlocked_Implementation() const", class_name));
        lines.push(format!("{{"));
        lines.push(format!("\t// Default: never blocked"));
        lines.push(format!("\t// Override in Blueprint or C++ subclass for custom logic"));
        lines.push(format!("\treturn false;"));
        lines.push(format!("}}"));
    ...
    pub fn generate_node_data_header(&self) -> Result<String> {
        ...
        lines.push(format!("\tUFUNCTION(BlueprintPure, Category = \"{}|NodeData\")", self.graph.name));
        ...
        lines.push(format!("\tUPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = \"{}|NodeData\")", self.graph.name));
    ...
    pub fn generate_node_data_source(&self) -> Result<String> {
        ...
        lines.push(format!("{{"));
        lines.push(format!("\treturn DisplayName.IsEmpty() ? FText::FromName(NodeName) : DisplayName;"));
        lines.push(format!("}}"));
...
pub fn generate_graph_instance(
    graph: &GraphEditor,
    plugin_name: &str,
...
mod tests {
    ...
    fn test_instance_header_generation() {
        ...
        assert!(header_content.contains("class UTestGraphInstance") || header_content.contains("UTestGraphInstance"), 
    ...
    fn test_instance_source_generation() {
        ...
        assert!(source_content.contains("const UTestGraphNodeData* UTestGraphInstance::GetCurrentNode"));
    ...
    fn test_node_data_header_generation() {
        ...
        // Check for essential elements
        assert!(header_content.contains("class UTestGraphNodeData") || header_content.contains("UTestGraphNodeData"),
