...
impl FactoryGenerator {
    ...
    fn generate_base_node_source(&self) -> Result<String> {
        ...
        lines.push(format!(
            "\t// Context menu entries are optional and module-dependent."
        ));
        lines.push(format!("}}"));
        lines.push(String::new());
        ...
        lines.push(format!(
            "bool {}::CanCreateUnderSpecifiedSchema(const UEdGraphSchema* Schema) const",
            class_name
        ));
        lines.push(format!("{{"));
    ...
    fn generate_node_source(&self, node: &NodeType) -> Result<String> {
        ...
        lines.push(String::new());
        lines.push(format!(
            "#define LOCTEXT_NAMESPACE \"{}Schema\"",
            self.graph.name
        ));
        lines.push(String::new());
        ...
        lines.push(format!("{{"));
        lines.push(format!(
            "\treturn LOCTEXT(\"Node.{}.Title\", \"{}\");",
            node.name, node_title
        ));
        lines.push(format!("}}"));
        lines.push(String::new());
        ...
        lines.push(format!("{{"));
        lines.push(format!(
            "\treturn FLinearColor({:.3}f, {:.3}f, {:.3}f, {:.3}f);",
    ...
    fn generate_schema_source(&self) -> Result<String> {
        ...
        for node_type in &self.graph.node_types {
            lines.push(format!(
                "#include \"{}{}Node.h\"",
                self.graph.name, node_type.name
            ));
        }
        ...
        lines.push(format!("{{"));
        lines.push(format!(
            "\t// TODO: Add context menu actions for node creation"
        ));
        lines.push(format!("}}"));
        lines.push(String::new());
        ...
        if !self.graph.schema.allowed_connections.is_empty() {
            ...
            lines.push(format!(
                "\t// TODO: Implement connection validation based on pin types"
            ));
        }
        ...
        lines.push(format!(
            "void {}::CreateDefaultNodesForGraph(UEdGraph& Graph) const",
            class_name
        ));
        lines.push(format!("{{"));
        ...
        // BreakPinLinks
        lines.push(format!(
            "void {}::BreakPinLinks(UEdGraphPin& TargetPin, bool bSendsNodeNotification) const",
            class_name
        ));
        lines.push(format!("{{"));
...
pub fn generate_graph_factory(graph: &GraphEditor, plugin_name: &str) -> Result<FactoryOutput> {
...
mod tests {
    ...
    fn test_pin_type_conversion() {
        ...
        assert_eq!(
            generator.pin_type_to_category(&PinType::Object("AActor".to_string())),
            "object:AActor"
        );
    }
}
