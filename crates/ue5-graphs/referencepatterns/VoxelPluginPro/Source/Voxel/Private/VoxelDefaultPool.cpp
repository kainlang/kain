...
static TStaticArray<uint32, 256> InitializePriorityArray(const TMap<EVoxelTaskType, int32>& InPriorityMap)
{
	TStaticArray<uint32, 256> Array;
	for (int32 Index = 0; Index < 256; Index++)
	{
		Array[Index] = InPriorityMap.FindRef(EVoxelTaskType(Index));
	}
	return Array;
}
...
FVoxelDefaultPool::FVoxelDefaultPool(
	...
	: Pool(FVoxelQueuedThreadPool::Create(FVoxelQueuedThreadPoolSettings(
		...
		bConstantPriorities))),
	PriorityCategories(InitializePriorityArray(InPriorityCategories)),
	PriorityOffsets(InitializePriorityArray(InPriorityOffsets))
{
}
