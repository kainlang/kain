...
FText UDialogGraphNode::GetNodeTitle(ENodeTitleType::Type titalType) const { 
    ...
    if (nodeInfo->isBranchingNode) return FText::FromString("Branching");

    FActorInfo * actor = GetAsset()->getActorInfoFromIndex(nodeInfo->ActorIdx);
    if(actor != nullptr)
        nodeInfo->ActorName = actor->ActorIdentifier;
    else
        nodeInfo->ActorName = FText::FromString("");
    
    if (nodeInfo->ActorName.IsEmpty()) {
        FString dialogTextStr = nodeInfo->Replique.SpokenText.ToString();
        if (dialogTextStr.Len() > 15) {
            dialogTextStr = dialogTextStr.Left(15) + TEXT("...");
        }
        return FText::FromString(dialogTextStr);
    }
    return nodeInfo->ActorName;
}
...
UEdGraphPin* UDialogGraphNode::CreateDefaultInputPin() { 
    return CreateDialogPin(EEdGraphPinDirection::EGPD_Input, TEXT(""));
}
