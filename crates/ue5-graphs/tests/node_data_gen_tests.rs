...
fn test_generate_node_data_creates_all_files() {
    ...
    assert!(output
        .node_data_headers
        .iter()
        .any(|(name, _)| name == "RootNodeData.h"));
    assert!(output
        .node_data_headers
        .iter()
        .any(|(name, _)| name == "ConditionNodeData.h"));
    assert!(output
        .node_data_headers
        .iter()
        .any(|(name, _)| name == "ActionNodeData.h"));
}
...
fn test_base_node_data_source_implementation() {
    ...
    assert!(source
        .contains("UTestGraphGraphNodeData* UTestGraphGraphNodeData::GetNextOutputNodeByPinIndex"));
...
fn test_pin_type_conversions() {
    ...
    assert_eq!(
        generator.pin_type_to_cpp_type(&PinType::Object("UStaticMeshComponent".to_string())),
    ...
    assert_eq!(
        generator.pin_type_to_cpp_type(&PinType::Wildcard),
        "UObject*"
    );
}
...
fn test_property_name_sanitization() {
    ...
    assert_eq!(
        generator.sanitize_property_name("special!@#chars"),
        "SpecialChars"
    );
}
...
fn test_execution_logic_preservation() {
    ...
    let source = generator
        .generate_node_data_subclass_source(condition_node)
        .unwrap();
    ...
    let source = generator
        .generate_node_data_subclass_source(action_node)
        .unwrap();
...
fn test_multiple_properties_generation() {
    ...
    let header = generator
        .generate_node_data_subclass_header(action_node)
        .unwrap();

    // Should have both Target and Damage properties
    let property_count = header
        .matches("UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = \"Node Data\")")
        .count();
