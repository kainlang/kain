...
fn test_graph_data_header_generation() {
    ...
    assert!(
        header.contains("#include \"CoreMinimal.h\""),
        "Should include CoreMinimal"
    );
    assert!(
        header.contains("CombatGraphData.generated.h"),
        "Should include generated header"
    );

    // Verify forward declarations
    assert!(
        header.contains("class UCombatGraphInstance"),
        "Should forward declare GraphInstance"
    );
    assert!(
        header.contains("class UCombatGraphNodeData"),
        "Should forward declare NodeData"
    );

    // Verify PinData class
    assert!(header.contains("UCLASS()"), "Should have UCLASS macro");
    assert!(
        header.contains("class COMBATPLUGIN_API UCombatPinData"),
        "Should have correct class name and API macro"
    );
    assert!(
        header.contains("GENERATED_BODY()"),
        "Should have GENERATED_BODY macro"
    );
    assert!(
        header.contains("FName PinName"),
        "Should have PinName field"
    );
    assert!(header.contains("FGuid PinId"), "Should have PinId field");
    assert!(
        header.contains("TArray<UCombatPinData*> ConnectToPins"),
        "Should have ConnectToPins array"
    );
    assert!(
        header.contains("UCombatGraphNodeData* Parent"),
        "Should have Parent pointer"
    );

    // Verify NodeData class
    assert!(
        header.contains("class COMBATPLUGIN_API UCombatGraphNodeData"),
        "Should have NodeData class"
    );
    assert!(
        header.contains("GetNextOutputNodeByPinIndex"),
        "Should have GetNextOutputNodeByPinIndex method"
    );
    assert!(
        header.contains("ExecuteNode"),
        "Should have ExecuteNode method"
    );
    assert!(
        header.contains("TArray<UCombatPinData*> InputPins"),
        "Should have InputPins array"
    );
    assert!(
        header.contains("TArray<UCombatPinData*> OutputPins"),
        "Should have OutputPins array"
    );
    assert!(
        header.contains("FIntPoint NodePosition"),
        "Should have NodePosition field"
    );
    assert!(
        header.contains("FGuid NodeGuid"),
        "Should have NodeGuid field"
    );

    // Verify GraphData class
    assert!(
        header.contains("class COMBATPLUGIN_API UCombatGraphData"),
        "Should have GraphData class"
    );
    assert!(
        header.contains("FindPinById"),
        "Should have FindPinById method"
    );
    assert!(
        header.contains("TArray<UCombatGraphNodeData*> Nodes"),
...
fn test_graph_data_source_generation() {
    ...
    // Verify includes
    assert!(
        source.contains("#include \"GraphData/CombatGraphData.h\""),
        "Should include header"
    );

    // Verify FindPinById implementation
    assert!(
        source.contains("UCombatPinData* UCombatGraphData::FindPinById(FGuid PinId)"),
        "Should have FindPinById implementation"
    );
    assert!(
        source.contains("for (UCombatGraphNodeData* Node : Nodes)"),
        "Should iterate over nodes"
    );
    assert!(
        source.contains("for (UCombatPinData* Pin : Node->InputPins)"),
        "Should check input pins"
    );
    assert!(
        source.contains("for (UCombatPinData* Pin : Node->OutputPins)"),
        "Should check output pins"
    );
    assert!(
        source.contains("if (Pin->PinId == PinId)"),
        "Should compare pin IDs"
    );
    assert!(source.contains("return Pin;"), "Should return found pin");
    assert!(
        source.contains("return nullptr;"),
    ...
    assert!(
        source.contains("if (!OutputPins.IsValidIndex(OutputPinIndex))"),
        "Should validate index"
    );
    assert!(
        source.contains("if (!IsValid(OutputPin))"),
        "Should validate pin"
    );
    assert!(
        source.contains("if (OutputPin->ConnectToPins.Num() == 0)"),
        "Should check for connections"
    );
    assert!(
        source.contains("return OutputPin->ConnectToPins[0]->Parent;"),
    ...
    assert!(
        source.contains("return GetNextOutputNodeByPinIndex(0);"),
...
fn test_template_methods_in_header() {
    ...
    assert!(
        header.contains("template <typename T>"),
        "Should have template declaration"
    );
    assert!(
        header.contains("T* GetNodeData()"),
        "Should have GetNodeData method"
    );
    assert!(
        header.contains("static_assert(TIsDerivedFrom<T, UCombatGraphNodeData>::IsDerived"),
        "Should have static_assert for type checking"
    );
    assert!(
        header.contains("if (!IsValid(Nodes[i]))"),
        "Should validate nodes"
    );
    assert!(
        header.contains("if (Nodes[i]->IsA<T>())"),
        "Should check node type"
    );
    assert!(
        header.contains("return Cast<T>(Nodes[i]);"),
        "Should cast and return"
    );

    // Verify GetListNodeData template
    assert!(
        header.contains("TArray<T*> GetListNodeData()"),
        "Should have GetListNodeData method"
    );
    assert!(
        header.contains("TArray<T*> Result;"),
        "Should declare result array"
    );
    assert!(
        header.contains("Result.Add(Cast<T>(Nodes[i]));"),
        "Should add to result"
    );
    assert!(
        header.contains("return Result;"),
...
fn test_api_macro_formatting() {
    ...
    for (plugin_name, expected_macro) in test_cases {
        ...
        let header = result.unwrap();
        assert!(
            header.contains(expected_macro),
            "Should contain {} for plugin {}",
            expected_macro,
            plugin_name
        );
    }
...
fn test_documentation_comments() {
    ...
    assert!(
        header.contains("Pin connection data"),
        "Should document PinData"
    );
    assert!(
        header.contains("Base class for"),
        "Should document NodeData"
    );
    assert!(
        header.contains("Container for"),
        "Should document GraphData"
    );
...
fn test_different_graph_names() {
    ...
    for graph_name in test_cases {
        ...
        assert!(
            header_result.is_ok(),
            "Header generation should succeed for {}",
            graph_name
        );
        ...
        assert!(
            header.contains(&format!("U{}PinData", graph_name)),
            "Should have correct PinData name"
        );
        assert!(
            header.contains(&format!("U{}GraphNodeData", graph_name)),
            "Should have correct NodeData name"
        );
        assert!(
            header.contains(&format!("U{}GraphData", graph_name)),
        ...
        assert!(
            source_result.is_ok(),
            "Source generation should succeed for {}",
            graph_name
        );
        ...
        assert!(
            source.contains(&format!("GraphData/{}GraphData.h", graph_name)),
...
fn test_generated_code_compiles_conceptually() {
    ...
    assert!(
        header.contains("#include \"CoreMinimal.h\""),
        "Missing CoreMinimal include"
    );
    assert!(
        header.contains(".generated.h"),
        "Missing generated header include"
    );

    // Verify source includes header
    assert!(
        source.contains(&format!("#include \"GraphData/{}GraphData.h\"", graph.name)),
