...
pub fn inject_into_plugin(
    ...
    for input in inputs {
        ...
        let source = fs::read_to_string(input).map_err(|e| {
            KainError::runtime(format!("Failed to read {}: {}", input.display(), e))
        })?;
    ...
    for (path, source) in &all_sources {
        let tokens = kain_core::Lexer::new(source).tokenize().map_err(|e| {
            KainError::parse_error(format!("Lexer error in {}: {}", path.display(), e))
        })?;
        ...
        let ast = kain_core::Parser::new(&tokens, &span_mapper, file_name)
            .parse()
            .map_err(|e| {
                KainError::parse_error(format!("Parse error in {}: {}", path.display(), e))
            })?;
    ...
    if !conflicts.is_empty() {
        println!(
            "   ⚠️  {} file(s) will be overwritten (--force enabled)",
            conflicts.len()
        );
    ...
    if dry_run {
        ...
        let new_includes: Vec<String> = generated_files
            .keys()
...
fn update_master_header(
    ...
    let source_dir = plugin_root.join("Source");
    let master_header_path = source_dir.join("Public").join(format!("{}.h", plugin_name));

    if !master_header_path.exists() {
        println!(
            "   ⚠️  Master header not found: {}",
            master_header_path.display()
        );
        return Ok(());
    }
    ...
    if dry_run {
        println!(
            "   [DRY RUN] Would update master header: {}",
            master_header_path.display()
        );
    ...
    let mut content = fs::read_to_string(&master_header_path).map_err(|e| KainError::Io(e))?;
    ...
    fs::write(&master_header_path, content).map_err(|e| KainError::Io(e))?;

    println!(
        "   ✓ Updated master header: {}",
        master_header_path.display()
    );
    ...
    for item in &program.items {
        let (item_name, output_name) = match item {
            kain_core::types::TypedItem::Actor(a) => {
                (&a.ast.name, ue5::naming::to_actor_name(&a.ast.name))
            }
            kain_core::types::TypedItem::Component(c) => {
                (&c.ast.name, ue5::naming::to_component_name(&c.ast.name))
            }
            kain_core::types::TypedItem::Struct(s) => {
                (&s.ast.name, ue5::naming::to_struct_name(&s.ast.name))
            }
            kain_core::types::TypedItem::Enum(e) => {
                (&e.ast.name, ue5::naming::to_enum_name(&e.ast.name))
            }
            _ => continue,
        };
...
fn write_injection_files(plugin_root: &Path, files: &HashMap<String, String>) -> KainResult<()> {
    let source_dir = plugin_root.join("Source");
    ...
    for (filename, content) in files {
        ...
        fs::create_dir_all(&target_dir).map_err(|e| KainError::Io(e))?;

        let target_path = target_dir.join(filename);

        fs::write(&target_path, content).map_err(|e| KainError::Io(e))?;
