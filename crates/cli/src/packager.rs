...
"#, ue5_config.plugin_name, ue5_config.plugin_name, ue5_config.plugin_name, ue5_config.plugin_name);
    ...
    let uplugin_content = generate_uplugin_file(&ue5_config.plugin_name, &manifest.package.description);
    ...
    // Check if we have editor items to include editor dependencies
    let has_editor_items = typed_program.items.iter().any(|item| {
        match item {
            kain_core::types::TypedItem::Struct(s) => {
                s.ast.attributes.iter().any(|a| 
...
/// Generate a .uplugin file with correct shader loading phase
fn generate_uplugin_file(plugin_name: &str, description: &Option<String>) -> String {
