...
fn get_failing_plugins() -> Vec<PluginTestCase> {
    vec![
        ...
        PluginTestCase::new("BulkMatte", FailureCategory::NameCollision {
            types: vec!["EParameterType".to_string(), "FMaterialInstanceInfo".to_string()]
        }),
        PluginTestCase::new("Example", FailureCategory::NameCollision {
            types: vec!["EQuestStatus".to_string(), "UPhysicsComponent".to_string()]
        }),
        PluginTestCase::new("NarrativeGraph", FailureCategory::NameCollision {
            types: vec!["EDialogueNodeType".to_string()]
        }),
        
        // Missing .generated.h (3 plugins)
        PluginTestCase::new("Cosmos", FailureCategory::MissingGeneratedHeader {
            header: "FVec2.generated.h".to_string()
        }),
        PluginTestCase::new("MetaHumanVAT", FailureCategory::MissingGeneratedHeader {
            header: "FVec2.generated.h".to_string()
        }),
        PluginTestCase::new("TickOptimizer", FailureCategory::MissingGeneratedHeader {
            header: "FVec2.generated.h".to_string()
        }),
        
        // Function Conflicts (2 plugins)
        PluginTestCase::new("Cinema4DMograph", FailureCategory::FunctionConflict {
            function: "Remap".to_string()
        }),
        PluginTestCase::new("TemporalBlueprint", FailureCategory::FunctionConflict {
            function: "ease_in_out".to_string()
        }),
        
        // C++ Syntax Errors (1 plugin)
        PluginTestCase::new("UltimateVFX", FailureCategory::CppSyntaxError {
            error_count: 259
        }),
...
fn run_fullbuild(plugin: &PluginTestCase) -> Result<(), String> {
    ...
    if output.status.success() {
        ...
        Err(format!("FULLBUILD.bat failed:\nSTDOUT:\n{}\nSTDERR:\n{}", stdout, stderr))
    }
}
...
fn categorize_failure(error_output: &str) -> FailureCategory {
    ...
    } else if error_output.contains("Cannot open include file") && error_output.contains(".generated.h") {
        // Extract header name
    ...
    } else if error_output.contains("missing type specifier") || error_output.contains("syntax error") {
...
fn test_bug_condition_all_failing_plugins() {
    ...
    for plugin in &failing_plugins {
        println!("Testing plugin: {} (expected: {:?})", plugin.name, plugin.expected_failure);
    ...
    println!("\nMissing .generated.h ({} plugins):", missing_headers.len());
    ...
    println!("\nFunction Conflicts ({} plugins):", function_conflicts.len());
    ...
    if !unexpected_successes.is_empty() {
        println!("\n⚠️  WARNING: {} plugins compiled successfully when they should fail:", unexpected_successes.len());
    ...
    for plugin in failing_plugins {
        ...
        match result {
            ...
            Err(error) => {
                ...
                panic!("Bug condition exists: Plugin {} fails to compile", plugin.name);
            }
        }
    }
}
...
fn property_parse_errors_fail_at_kain_stage() {
    let parse_error_plugins = vec![
        "VoxelForgePro", "AeroTunnel", "AlphagenKain", "AutoInstancer",
        "CineMasterPro", "FluidFlow", "Materialize", "OmniCam",
        "PSOEliminator", "TitanGraph", "ToonShaderz", "UESculpt", "VRAMSniper"
    ];
...
fn property_name_collisions_caught_by_oracle() {
    ...
    for (plugin_name, expected_types) in collision_plugins {
        ...
        match run_fullbuild(&plugin) {
            Ok(()) => {
                println!("✓ Plugin {} compiles successfully (Oracle catches collisions)", plugin_name);
            }
            Err(error) => {
...
fn property_missing_generated_headers_fixed() {
    let missing_header_plugins = vec![
        "Cosmos", "MetaHumanVAT", "TickOptimizer"
    ];
    ...
    for plugin_name in missing_header_plugins {
        ...
        match run_fullbuild(&plugin) {
            Ok(()) => {
                println!("✓ Plugin {} compiles successfully (USTRUCT macros present)", plugin_name);
            }
            Err(error) => {
