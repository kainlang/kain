...
pub fn generate_stdlib_map(options: &StdlibMapOptions) -> Result<StdlibMap> {
    ...
    Ok(StdlibMap {
        ...
        source_roots: vec![
            ...
            "crates/core/src/stdlib.rs".to_string(),
...
pub fn render_llm_markdown(map: &StdlibMap) -> String {
    ...
    out.push_str("Scope: LLVM/native root profile only. Indexed source is top-level `stdlib/*.kn`; overlay/helper trees outside the root profile are intentionally excluded.\n\n");
    out.push_str(&format!(
    ...
    let mut builtins: Vec<_> = lib
        ...
        .map(|function| {
            ...
            BuiltinSymbol {
                ...
                origin: "crates/core/src/stdlib.rs".to_string(),
            }
        })
        .collect();
