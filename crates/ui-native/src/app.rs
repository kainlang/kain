...
use crate::qt_host::launch_qt_quick_host;
...
impl Default for KainUiNativeBackendPlan {
    fn default() -> Self {
        Self {
            shell_host_backend: UiHostBackendKind::Auto,
            document_host_backend: UiHostBackendKind::Auto,
            devtools_host_backend: UiHostBackendKind::Auto,
            layout_engine: UiLayoutEngineKind::Auto,
            render_engine: UiRenderEngineKind::Auto,
            compatibility_host_backend: UiHostBackendKind::Auto,
            mixed_backend_session: false,
        }
    }
}

impl KainUiNativeBackendPlan {
    pub fn from_runtime_metadata(metadata: &UiRuntimeMetadata) -> Self {
        Self {
...
impl Default for KainUiNativeAppConfig {
    fn default() -> Self {
        Self {
            window_title: "Kain UI Native Host".to_string(),
            root_component: "App".to_string(),
            source: String::new(),
            initial_window_size: [1280.0, 800.0],
...
impl fmt::Display for UnsupportedNativeHostError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "kain-ui-native can launch Auto/Native through the Qt shell adapter today; `{}` requires a registered host adapter",
...
pub fn run_bundled_app(bundle: KainUiNativeRuntimeBundle) -> Result<(), Box<dyn Error>> {
    ...
    match launch_backend(&backend_plan) {
...
fn host_launch_label(backend_plan: &KainUiNativeBackendPlan) -> &'static str {
    host_backend_label(launch_backend(backend_plan))
}

fn launch_backend(backend_plan: &KainUiNativeBackendPlan) -> UiHostBackendKind {
...
mod tests {
    ...
    #[test]
    fn default_backend_plan_does_not_pick_document_or_devtools_catalogs() {
        let plan = KainUiNativeBackendPlan::default();
        assert_eq!(plan.shell_host_backend, UiHostBackendKind::Auto);
        assert_eq!(plan.document_host_backend, UiHostBackendKind::Auto);
        assert_eq!(plan.devtools_host_backend, UiHostBackendKind::Auto);
        assert_eq!(plan.layout_engine, UiLayoutEngineKind::Auto);
        assert_eq!(plan.render_engine, UiRenderEngineKind::Auto);
        assert!(!plan.mixed_backend_session);
    }
}
