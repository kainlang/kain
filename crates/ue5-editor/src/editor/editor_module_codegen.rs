...
use crate::editor::editor_module_ir::{EditorModuleIR, MenuEntryIR, ToolbarButtonIR, ToolbarPositionIR};
...
fn generate_header(
    ...
    // Module class
    output.push_str(&format!("class {} {} : public IModuleInterface\n", api_macro, class_name));
    output.push_str("{\n");
...
fn generate_commands_class(ir: &EditorModuleIR, output: &mut String) {
    ...
    output.push_str(&format!("class {} : public TCommands<{}>\n", commands_class_name, commands_class_name));
    output.push_str("{\n");
    output.push_str("public:\n");
    
    // Constructor
    output.push_str(&format!("    {}()\n", commands_class_name));
    output.push_str(&format!("        : TCommands<{}>(TEXT(\"{}\"),\n", commands_class_name, ir.name));
    output.push_str(&format!("            NSLOCTEXT(\"{}\", \"ContextDescription\", \"{} commands\"),\n", ir.name, ir.name));
    ...
    for menu_entry in &ir.menu_entries {
        ...
        output.push_str(&format!("    TSharedPtr<FUICommandInfo> {};\n", command_name));
    }
...
fn generate_commands_implementation(ir: &EditorModuleIR, output: &mut String) {
    ...
    output.push_str(&format!("void {}::RegisterCommands()\n", commands_class_name));
    output.push_str("{\n");
...
fn generate_menu_entry_callback(ir: &EditorModuleIR, class_name: &str, menu_entry: &MenuEntryIR, output: &mut String) {
    output.push_str(&format!("void {}::{}()\n", class_name, menu_entry.method_name));
    output.push_str("{\n");
    output.push_str(&format!("    // TODO: Implement {} callback\n", menu_entry.label));
    output.push_str(&format!("    UE_LOG(LogTemp, Log, TEXT(\"Menu entry clicked: {}\"));\n", menu_entry.label));
    output.push_str("}\n\n");
}

/// Generate toolbar button callback implementation
fn generate_toolbar_button_callback(ir: &EditorModuleIR, class_name: &str, toolbar_button: &ToolbarButtonIR, output: &mut String) {
    output.push_str(&format!("void {}::{}()\n", class_name, toolbar_button.method_name));
    output.push_str("{\n");
    let label = toolbar_button.label.as_deref().unwrap_or("Toolbar button");
    output.push_str(&format!("    // TODO: Implement {} callback\n", label));
    output.push_str(&format!("    UE_LOG(LogTemp, Log, TEXT(\"Toolbar button clicked: {}\"));\n", label));
    output.push_str("}\n\n");
}

/// Generate menu extension registration
fn generate_menu_extension_registration(ir: &EditorModuleIR, class_name: &str, output: &mut String) {
    let commands_class_name = format!("F{}Commands", ir.name);
    ...
    let mut paths: Vec<String> = ir.menu_entries.iter()
        .map(|e| e.path.clone())
        .collect();
    paths.sort();
    paths.dedup();
    ...
    for path in &paths {
        ...
        output.push_str(&format!("        {}::Get().GetCommandList(),\n", commands_class_name));
        output.push_str(&format!("        FMenuExtensionDelegate::CreateRaw(this, &{}::Extend{}Menu));\n\n", class_name, path.replace("/", "")));
    }
    ...
    for path in &paths {
        let entries: Vec<&MenuEntryIR> = ir.menu_entries.iter()
            .filter(|e| &e.path == path)
            .collect();
        
        let method_name = format!("Extend{}Menu", path.replace("/", ""));
        output.push_str(&format!("void {}::{}(FMenuBuilder& MenuBuilder)\n", class_name, method_name));
        output.push_str("{\n");
        
        for entry in entries {
            let command_name = format!("{}Command", entry.method_name);
            output.push_str(&format!("    MenuBuilder.AddMenuEntry(F{}Commands::Get().{});\n", ir.name, command_name));
        }
...
fn generate_toolbar_extension_registration(ir: &EditorModuleIR, class_name: &str, output: &mut String) {
    output.push_str(&format!("void {}::RegisterToolbarExtensions()\n", class_name));
    output.push_str("{\n");
    output.push_str("    ToolbarExtender = MakeShareable(new FExtender);\n\n");
    
    // Group toolbar buttons by section
    let mut sections: Vec<String> = ir.toolbar_buttons.iter()
    ...
    for section in &sections {
        ...
        output.push_str(&format!("        FToolBarExtensionDelegate::CreateRaw(this, &{}::Extend{}Toolbar));\n\n", class_name, section));
    }
    ...
    for section in &sections {
        let buttons: Vec<&ToolbarButtonIR> = ir.toolbar_buttons.iter()
            .filter(|b| &b.section == section)
            .collect();
        
        let method_name = format!("Extend{}Toolbar", section);
        output.push_str(&format!("void {}::{}(FToolBarBuilder& ToolbarBuilder)\n", class_name, method_name));
        output.push_str("{\n");
        ...
        for button in buttons {
            ...
            output.push_str(&format!("        FUIAction(FExecuteAction::CreateRaw(this, &{}::{})),\n", class_name, button.method_name));
            output.push_str(&format!("        NAME_None,\n"));
            output.push_str(&format!("        LOCTEXT(\"{}_Label\", \"{}\"),\n", button.method_name, label));
            output.push_str(&format!("        LOCTEXT(\"{}_Tooltip\", \"{}\"),\n", button.method_name, tooltip));
            output.push_str(&format!("        FSlateIcon(FEditorStyle::GetStyleSetName(), TEXT(\"{}\")));\n", button.icon));
        }
...
mod tests {
    ...
    fn test_generate_editor_module_header() {
        ...
        assert!(output.header.contains("class TESTPLUGIN_API FWeaponEditorModule"));
        assert!(output.header.contains("public IModuleInterface"));
        assert!(output.header.contains("virtual void StartupModule() override"));
        assert!(output.header.contains("virtual void ShutdownModule() override"));
        ...
        assert!(output.header.contains("TSharedPtr<FExtender> ToolbarExtender"));
    }
    ...
    fn test_generate_commands_class() {
        ...
        assert!(output.header.contains("class FWeaponEditorModuleCommands : public TCommands<FWeaponEditorModuleCommands>"));
        assert!(output.header.contains("virtual void RegisterCommands() override"));
        assert!(output.header.contains("TSharedPtr<FUICommandInfo> on_open_editorCommand"));
    }
    ...
    fn test_generate_startup_module() {
        ...
        assert!(output.source.contains("void FWeaponEditorModule::StartupModule()"));
        assert!(output.source.contains("FWeaponEditorModuleCommands::Register()"));
    ...
    fn test_generate_shutdown_module() {
        ...
        assert!(output.source.contains("void FWeaponEditorModule::ShutdownModule()"));
        assert!(output.source.contains("FWeaponEditorModuleCommands::Unregister()"));
    ...
    fn test_generate_implement_module() {
        ...
        assert!(output.source.contains("IMPLEMENT_MODULE(FWeaponEditorModule, TestPlugin)"));
    }
    ...
    fn test_generate_menu_entry_callback() {
        ...
        assert!(output.source.contains("void FWeaponEditorModule::on_open_editor()"));
        assert!(output.source.contains("Menu entry clicked: Open Weapon Editor"));
    }
    ...
    fn test_generate_toolbar_button_callback() {
        ...
        assert!(output.source.contains("void FWeaponEditorModule::on_quick_create()"));
        assert!(output.source.contains("Toolbar button clicked: Quick Create"));
    }
}
