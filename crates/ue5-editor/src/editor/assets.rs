...
use crate::editor::asset_editor_ir::{AssetEditorIR, convert_to_asset_editor_ir};
...
impl AssetEditorGenerator {
    ...
    fn generate_header(&self, ir: &AssetEditorIR) -> Result<String, String> {
        ...
        // Class declaration
        header.push_str(&format!("class {} : public FAssetEditorToolkit\n", class_name));
        header.push_str("{\n");
        ...
        for widget in &ir.custom_widgets {
            ...
            header.push_str(&format!("\tTSharedRef<SDockTab> {}(const FSpawnTabArgs& Args);\n", method_name));
        }
        ...
        for method in &ir.custom_methods {
            let return_type = method.return_type.as_ref().map(|s| s.as_str()).unwrap_or("void");
        ...
        for widget in &ir.custom_widgets {
            header.push_str(&format!("\tstatic const FName {}TabId;\n", widget.field_name));
        }
        if ir.viewport.is_some() || ir.details.is_some() || !ir.custom_widgets.is_empty() {
            header.push_str("\n");
        }
        ...
        for widget in &ir.custom_widgets {
            header.push_str(&format!("\tTSharedPtr<S{}> {}Widget;\n", widget.widget_type, widget.field_name));
        }
    ...
    fn generate_source(&self, ir: &AssetEditorIR) -> Result<String, String> {
        ...
        // Tab ID definitions
        if let Some(ref viewport) = ir.viewport {
            source.push_str(&format!("const FName {}::ViewportTabId(TEXT(\"{}\"));\n", 
                class_name, viewport.tab_id));
        }
        if let Some(ref details) = ir.details {
            source.push_str(&format!("const FName {}::DetailsTabId(TEXT(\"{}\"));\n", 
                class_name, details.tab_id));
        }
        for widget in &ir.custom_widgets {
            source.push_str(&format!("const FName {}::{}TabId(TEXT(\"{}\"));\n", 
        ...
        source.push_str(&format!("FText {}::GetBaseToolkitName() const\n", class_name));
        source.push_str("{\n");
        source.push_str(&format!("\treturn FText::FromString(TEXT(\"{}\"));\n", ir.name));
        source.push_str("}\n\n");
        
        source.push_str(&format!("FString {}::GetWorldCentricTabPrefix() const\n", class_name));
        source.push_str("{\n");
        source.push_str(&format!("\treturn TEXT(\"{}\");\n", ir.name));
        source.push_str("}\n\n");
        
        source.push_str(&format!("FLinearColor {}::GetWorldCentricTabColorScale() const\n", class_name));
        source.push_str("{\n");
    ...
    fn generate_init_editor(&self, ir: &AssetEditorIR, class_name: &str) -> Result<String, String> {
        ...
        for widget in &ir.custom_widgets {
            let method_name = format!("Spawn{}Tab", widget.field_name);
            code.push_str(&format!("\tTabManager->RegisterTabSpawner({}TabId, FOnSpawnTab::CreateSP(this, &{}::{}))\n", 
                widget.field_name, class_name, method_name));
            code.push_str(&format!("\t\t.SetDisplayName(FText::FromString(TEXT(\"{}\")))\n", widget.tab_name));
            code.push_str("\t\t.SetGroup(WorkspaceMenuCategory.ToSharedRef());\n");
        }
    ...
    fn generate_viewport_spawner(&self, _ir: &AssetEditorIR, class_name: &str, viewport: &crate::editor::asset_editor_ir::ViewportPanelIR) -> Result<String, String> {
        let mut code = String::new();
        
        code.push_str(&format!("TSharedRef<SDockTab> {}::SpawnViewportTab(const FSpawnTabArgs& Args)\n", class_name));
        code.push_str("{\n");
        code.push_str(&format!("\tViewportWidget = SNew(S{});\n\n", viewport.viewport_type));
        code.push_str("\treturn SNew(SDockTab)\n");
        code.push_str(&format!("\t\t.Label(FText::FromString(TEXT(\"{}\")))\n", viewport.tab_name));
        code.push_str("\t\t[\n");
    ...
    fn generate_details_spawner(&self, _ir: &AssetEditorIR, class_name: &str, details: &crate::editor::asset_editor_ir::DetailsPanelIR) -> Result<String, String> {
        let mut code = String::new();
        
        code.push_str(&format!("TSharedRef<SDockTab> {}::SpawnDetailsTab(const FSpawnTabArgs& Args)\n", class_name));
        code.push_str("{\n");
        ...
        code.push_str(&format!("\tDetailsViewArgs.bLockable = {};\n", if details.lockable { "true" } else { "false" }));
        code.push_str(&format!("\tDetailsViewArgs.NameAreaSettings = {};\n", 
            if details.show_name_area { "FDetailsViewArgs::ObjectsUseNameArea" } else { "FDetailsViewArgs::HideNameArea" }));
        code.push_str("\n");
    ...
    fn generate_custom_widget_spawner(&self, _ir: &AssetEditorIR, class_name: &str, widget: &crate::editor::asset_editor_ir::CustomWidgetIR) -> Result<String, String> {
        let mut code = String::new();
        let method_name = format!("Spawn{}Tab", widget.field_name);
        
        code.push_str(&format!("TSharedRef<SDockTab> {}::{}(const FSpawnTabArgs& Args)\n", class_name, method_name));
        code.push_str("{\n");
        code.push_str(&format!("\t{}Widget = SNew(S{});\n\n", widget.field_name, widget.widget_type));
        code.push_str("\treturn SNew(SDockTab)\n");
        code.push_str(&format!("\t\t.Label(FText::FromString(TEXT(\"{}\")))\n", widget.tab_name));
        code.push_str("\t\t[\n");
        code.push_str(&format!("\t\t\t{}Widget.ToSharedRef()\n", widget.field_name));
        code.push_str("\t\t];\n");
        code.push_str("}\n\n");
