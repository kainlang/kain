...
impl LayoutOptimizer {
    ...
    fn expr_references_property(&self, expr: &Expr, prop_name: &str) -> bool {
        match expr {
            ...
            Expr::Call { args, .. } => args
                .iter()
                .any(|arg| self.expr_references_property(&arg.value, prop_name)),
    ...
    pub fn generate_report(&self, analyses: &[PropertyAnalysis]) -> String {
        ...
        let static_count = analyses
            .iter()
            .filter(|a| a.reactivity == PropertyReactivity::Static)
            .count();
        let reactive_count = analyses
            .iter()
            .filter(|a| a.reactivity == PropertyReactivity::Reactive)
            .count();
        let event_count = analyses
            .iter()
            .filter(|a| a.reactivity == PropertyReactivity::Event)
            .count();

        report.push_str(&format!("// Layout Optimization Report\n"));
        report.push_str(&format!(
            "// Static Properties: {} (no Tick overhead)\n",
            static_count
        ));
        report.push_str(&format!(
            "// Reactive Properties: {} (requires Tick)\n",
            reactive_count
        ));
        report.push_str(&format!("// Event Handlers: {}\n", event_count));
        report.push_str(&format!(
            "// Estimated Tick Reduction: {:.1}%\n",
