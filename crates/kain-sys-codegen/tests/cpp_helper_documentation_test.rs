...
fn test_cpp_backend_includes_helper_documentation() {
    ...
    assert!(
        cpp_code.contains("template<typename T> T* __kain_ptr_offset(T* ptr, int64_t offset, int64_t stride);"),
...
fn test_cpp_backend_helper_organization() {
    ...
    let allocation_pos = cpp_code.find("Allocation helpers").expect("Should have allocation section");
    let union_pos = cpp_code.find("Union operations (SUPPORTED)").expect("Should have union section");
    let bitfield_pos = cpp_code.find("Bitfield operations (SUPPORTED)").expect("Should have bitfield section");
