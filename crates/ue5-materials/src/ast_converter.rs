...
use kain_core::ast::{
    BinaryOp, CallArg, Expr, MaterialGraphDef, MaterialInput, MaterialOutput, MaterialStatement,
    Type,
};
...
impl MaterialGraphConverter {
    ...
    fn create_input_node(
        ...
        let node = match &input.ty {
            ...
            Type::Named { name, .. } if name == "Sampler2D" => MaterialNode {
                id: node_id.clone(),
    ...
    fn convert_expr(&mut self, graph: &mut MaterialGraph, expr: &Expr) -> Result<String, String> {
        match expr {
                ...
                if let Expr::Ident(name, _) = &**callee {
                    match name.as_str() {
                        ...
                        "lerp" => {
                            if args.len() != 3 {
                                return Err(format!(
                                    "lerp() expects 3 arguments (a, b, alpha), got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "clamp" => {
                            if args.len() != 3 {
                                return Err(format!(
                                    "clamp() expects 3 arguments (input, min, max), got {}",
                                    args.len()
                                ));
                            }
                            let input_id = self.convert_expr(graph, &args[0].value)?;
                            ...
                            graph.nodes.push(MaterialNode {
                                id: node_id.clone(),
                                node_type: MaterialNodeType::Clamp {
                                    input: input_id,
                                    min: min_id,
                                    max: max_id,
                                },
                        ...
                        "pow" => {
                            if args.len() != 2 {
                                return Err(format!(
                                    "pow() expects 2 arguments (base, exponent), got {}",
                                    args.len()
                                ));
                            }
                            ...
                            graph.nodes.push(MaterialNode {
                                id: node_id.clone(),
                                node_type: MaterialNodeType::Power {
                                    base: base_id,
                                    exponent: exp_id,
                                },
                                position: (x, y),
                        ...
                        "distance" => {
                            if args.len() != 2 {
                                return Err(format!(
                                    "distance() expects 2 arguments, got {}",
                                    args.len()
                                ));
                            }
                            let a_id = self.convert_expr(graph, &args[0].value)?;
                        ...
                        "saturate" => {
                            if args.len() != 1 {
                                return Err(format!(
                                    "saturate() expects 1 argument, got {}",
                                    args.len()
                                ));
                            }
                            let input_id = self.convert_expr(graph, &args[0].value)?;
                        ...
                        "sample" | "texture_sample" => {
                            ...
                            if args.is_empty() || args.len() > 2 {
                                return Err(format!(
                                    "sample() expects 1 or 2 arguments (texture, [uv]), got {}",
                                    args.len()
                                ));
                            }
                            ...
                            let texture_name =
                                match texture_arg {
                                    ...
                                    _ => return Err(
                                        "sample() first argument must be a texture input variable"
                                            .to_string(),
                                    ),
                                };
                        ...
                        "vec4" => {
                            ...
                            if args.len() == 4 {
                                ...
                                graph.nodes.push(MaterialNode {
                                    id: node_id.clone(),
                                    node_type: MaterialNodeType::ConstantVec4 {
                                        value: [x, y, z, w],
                                    },
                                    position: (pos_x, pos_y),
                                });
                        ...
                        "call_shader" => {
                            ...
                            if args.is_empty() {
                                return Err(
                                    "call_shader() requires at least 1 argument (shader name)"
                                        .to_string(),
                                );
                            }
                        ...
                        "custom_hlsl" => {
                            ...
                            if args.is_empty() {
                                return Err(
                                    "custom_hlsl() requires at least 1 argument (HLSL code)"
                                        .to_string(),
                                );
                            }
                            ...
                            let code = match &args[0].value {
                                ...
                                _ => {
                                    return Err(
                                        "custom_hlsl() first argument must be a string literal"
                                            .to_string(),
                                    )
                                }
                            };
                                ...
                                match arg.name.as_deref() {
                                    Some("output_type") => {
                                        ...
                                        let type_str = match &arg.value {
                                            ...
                                            _ => {
                                                return Err("output_type must be a string literal"
                                                    .to_string())
                                            }
                                        };
                                    ...
                                    Some("inputs") => {
                                        ...
                                        let input_array = match &arg.value {
                                            ...
                                            _ => {
                                                return Err(
                                                    "inputs must be an array literal".to_string()
                                                )
                                            }
                                        };
                                            ...
                                            let (input_name, input_type_str) = match item {
                                                Expr::Tuple(elements, _) if elements.len() == 2 => {
                                                    ...
                                                    let type_str = match &elements[1] {
                                                        Expr::String(s, _) => s.as_str(),
                                                        _ => return Err(
                                                            "Input type must be a string literal"
                                                                .to_string(),
                                                        ),
                                                    };
                                                    (name, type_str)
                                                ...
                                                _ => {
                                                    return Err(
                                                        "Each input must be a tuple (name, type)"
                                                            .to_string(),
                                                    )
                                                }
                                            };
                                    ...
                                    Some(other) => {
                                        return Err(format!(
                                            "Unknown named argument for custom_hlsl(): '{}'",
                                            other
                                        ));
                                    }
                                    None => {
                        ...
                        "uv_scale" => {
                            ...
                            if args.len() != 3 {
                                return Err(format!(
                                    "uv_scale() expects 3 arguments (uv, scale_x, scale_y), got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "world_position" => {
                            ...
                            if !args.is_empty() {
                                return Err(format!(
                                    "world_position() expects 0 arguments, got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "world_normal" => {
                            ...
                            if !args.is_empty() {
                                return Err(format!(
                                    "world_normal() expects 0 arguments, got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "absolute_world_position" => {
                            ...
                            if !args.is_empty() {
                                return Err(format!(
                                    "absolute_world_position() expects 0 arguments, got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "camera_position" => {
                            ...
                            if !args.is_empty() {
                                return Err(format!(
                                    "camera_position() expects 0 arguments, got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "object_position" => {
                            ...
                            if !args.is_empty() {
                                return Err(format!(
                                    "object_position() expects 0 arguments, got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "object_orientation" => {
                            ...
                            if !args.is_empty() {
                                return Err(format!(
                                    "object_orientation() expects 0 arguments, got {}",
                                    args.len()
                                ));
                            }
                        ...
                        "triplanar_sample" => {
                            ...
                            let texture_id = self
                                ...
                                .cloned()
                                .ok_or_else(|| {
                                    format!("Undefined texture variable: {}", texture_name)
                                })?;
            ...
            Expr::Float(value, _) => {
                ...
                graph.nodes.push(MaterialNode {
                    id: node_id.clone(),
                    node_type: MaterialNodeType::ConstantFloat {
                        value: *value as f32,
                    },
                    position: (x, y),
                });
    ...
    ) -> Result<MaterialProperties, String> {
...
mod tests {
    ...
    fn test_custom_hlsl_parsing() {
        ...
        let def = MaterialGraphDef {
            ...
            attributes: vec![],
            inputs: vec![],
            body: vec![MaterialStatement::Let {
    ...
    fn test_call_shader_parsing() {
        ...
        let function_call_node = graph
            .nodes
            .iter()
            .find(|n| matches!(n.node_type, MaterialNodeType::MaterialFunctionCall { .. }));

        assert!(
            function_call_node.is_some(),
            "MaterialFunctionCall node not found"
        );
    ...
    fn test_resolve_shader_path() {
        ...
        assert_eq!(
            result.unwrap(),
            "/Game/Materials/Functions/My_Custom_Shader"
        );
    ...
    fn test_uv_scroll() {
                ...
                MaterialStatement::Let {
                    ...
                    value: Expr::Call {
                        ...
                        args: vec![
                            CallArg {
                                name: None,
                                value: Expr::Ident("uv".to_string(), Span::default()),
                                span: Span::default(),
                            },
                            CallArg {
                                name: None,
                                value: Expr::Float(0.1, Span::default()),
                                span: Span::default(),
                            },
                            CallArg {
                                name: None,
                                value: Expr::Float(0.2, Span::default()),
                                span: Span::default(),
                            },
                        ],
                        span: Span::default(),
        ...
        let uv_scroll_node = graph
            .nodes
            .iter()
            .find(|n| matches!(n.node_type, MaterialNodeType::UVScroll { .. }));
    ...
    fn test_uv_scale() {
        ...
        let uv_scale_node = graph
            .nodes
            .iter()
            .find(|n| matches!(n.node_type, MaterialNodeType::UVScale { .. }));
    ...
    fn test_uv_rotate() {
                ...
                MaterialStatement::Let {
                    ...
                    value: Expr::Call {
                        ...
                        args: vec![
                            ...
                            CallArg {
                                name: None,
                                value: Expr::Float(45.0, Span::default()),
                                span: Span::default(),
                            },
                        ],
                        span: Span::default(),
        ...
        let uv_rotate_node = graph
            .nodes
            .iter()
            .find(|n| matches!(n.node_type, MaterialNodeType::UVRotate { .. }));
    ...
    fn test_uv_operation_chaining() {
        ...
        let has_scale = graph
            .nodes
            .iter()
            .any(|n| matches!(n.node_type, MaterialNodeType::UVScale { .. }));
        let has_scroll = graph
            .nodes
            .iter()
            .any(|n| matches!(n.node_type, MaterialNodeType::UVScroll { .. }));
        ...
        let scroll_node = graph
            .nodes
            .iter()
            .find(|n| matches!(n.node_type, MaterialNodeType::UVScroll { .. }))
            .unwrap();
        let scale_node = graph
            .nodes
            .iter()
            .find(|n| matches!(n.node_type, MaterialNodeType::UVScale { .. }))
            .unwrap();
        ...
        match &scroll_node.node_type {
            MaterialNodeType::UVScroll { uv_input, .. } => {
                assert_eq!(
                    uv_input, &scale_node.id,
                    "UVScroll should use UVScale output as input"
                );
            }
    ...
    fn test_time_based_effects() {
            ...
            body: vec![
                ...
                MaterialStatement::Let {
                    ...
                    value: Expr::Call {
                        ...
                        args: vec![CallArg {
                            name: None,
                            value: Expr::Ident("t_scaled".to_string(), Span::default()),
                            span: Span::default(),
                        }],
                        span: Span::default(),
                    },
        ...
        assert!(
            graph.is_dynamic,
            "Material should be marked as dynamic when using time()"
        );

        // Verify nodes: Time, speed constant, Multiply, Sine
        assert!(
            graph.nodes.len() >= 4,
            "Expected at least 4 nodes (Time, Constant, Multiply, Sine), got {}",
            graph.nodes.len()
        );

        // Verify Time node exists
        let has_time_node = graph
            .nodes
            .iter()
            .any(|n| matches!(n.node_type, MaterialNodeType::Time));
        assert!(has_time_node, "Expected Time node in graph");

        // Verify Sine node exists
        let has_sine_node = graph
            .nodes
            .iter()
            .any(|n| matches!(n.node_type, MaterialNodeType::Sine { .. }));
        assert!(has_sine_node, "Expected Sine node in graph");

        // Verify emissive output is connected
        assert!(
            graph.outputs.emissive.is_some(),
            "Expected emissive output to be connected"
        );
    }
    ...
    fn test_time_node_deduplication() {
        ...
        let time_node_count = graph
            .nodes
            .iter()
            .filter(|n| matches!(n.node_type, MaterialNodeType::Time))
            .count();
        assert_eq!(
            time_node_count, 1,
            "Expected exactly 1 Time node (deduplication), got {}",
            time_node_count
        );
