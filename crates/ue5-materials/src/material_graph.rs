...
pub enum MaterialNodeType {
    TextureSample {
        texture_input: Option<String>,
        uv_input: Option<String>,
    },
    ...
    ConstantVector3 {
        value: [f32; 3],
    }, // Alias for ConstantVec3
    ConstantVector4 {
        value: [f32; 4],
    }, // Alias for ConstantVec4
    TextureCoordinate {
        index: u32,
        tiling: [f32; 2],
    },
    ...
    pub fn mark_parameter_dynamic(&mut self, param_name: &str) -> Result<(), String> {
        ...
        let (param_type, default_value) = match input.input_type {
            ...
            _ => {
                ...
                return Err(format!(
                    "Parameter type '{}' cannot be made dynamic",
                    type_name
                ));
            }
        };
...
mod tests {
    ...
    fn test_mark_parameter_dynamic_scalar() {
        ...
        assert!(matches!(
            graph.dynamic_parameters[0].param_type,
            DynamicParameterType::Scalar
        ));
        assert!(matches!(
            graph.dynamic_parameters[0].default_value,
            DynamicParameterValue::Scalar(0.5)
        ));
    }
