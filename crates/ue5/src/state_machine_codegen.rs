...
fn generate_header(ir: &StateMachineIR, class_name: &str, api_macro: &str, output: &mut String) {
    ...
    output.push_str(&format!(
        "    void TransitionToState(E{}State NewState);\n\n",
        ir.name
    ));
...
fn generate_source(ir: &StateMachineIR, class_name: &str, output: &mut String) {
    // Include header
...
fn generate_transition_method_implementations(
    ir: &StateMachineIR,
    class_name: &str,
    output: &mut String,
) {
...
fn generate_state_callback_implementations(
    ir: &StateMachineIR,
    class_name: &str,
    output: &mut String,
) {
...
mod tests {
    ...
    fn test_generate_update_method() {
        ...
        assert!(output.source.contains(
            "void UCharacterAnimationsStateMachine::UpdateStateMachine(float DeltaTime)"
        ));
    ...
    fn test_generate_get_current_animation() {
        ...
        assert!(output.source.contains(
            "UAnimSequence* UCharacterAnimationsStateMachine::GetCurrentAnimation() const"
        ));
        assert!(output
            .source
            .contains("case ECharacterAnimationsState::Idle:"));
    ...
    fn test_generate_with_callbacks() {
        ...
        assert!(output
            .source
            .contains("UE_LOG(LogTemp, Log, TEXT(\"Entering Idle\"))"));
    }
}
