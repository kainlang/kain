...
impl UhtRules {
    ...
    pub fn are_incompatible(&self, spec_a: &str, spec_b: &str) -> Option<&str> {
        ...
        for combo in &self.incompatible {
            ...
            let cb = combo.specifier_b.as_ref().map(|s| s.to_lowercase()).unwrap_or_default();
    ...
    pub fn incompatible_with(&self, specifier: &str) -> Vec<(&str, &str)> {
        ...
        for combo in &self.incompatible {
            ...
            if ca == s_lower {
                results.push((combo.specifier_b.as_deref().unwrap_or(""), combo.message.as_str()));
            } else if cb == s_lower {
    ...
    pub fn property_type_constraints(&self, type_name: &str) -> &[String] {
        self.property_constraints.get(type_name).map(|v| v.as_slice()).unwrap_or(&[])
    }
    ...
    pub fn rules_for_kain_construct(&self, construct: &str) -> &[UhtValidationRule] {
        self.kain_rules.get(construct).map(|v| v.as_slice()).unwrap_or(&[])
    }
