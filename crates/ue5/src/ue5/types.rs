...
pub fn map_type_with_knowledge(
    ty: &Type,
    config: &TypeMapConfig,
    kb: Option<&EngineKnowledge>,
) -> String {
    match ty {
        ...
        Type::Array(inner, size, _) => {
            format!(
                "TStaticArray<{}, {}>",
                map_type_with_knowledge(inner, config, kb),
                size
            )
        }
...
impl TypeMapper {
        ...
        match ty {
            Type::Named { name, generics, .. } => {
                ...
                let ue_name = match name.as_str() {
                    ...
                    // Vectors - configurable precision
                    "Vec2" => {
                        return if self.config.use_float_precision {
                            "FVector2f".to_string()
                        } else {
                            "FVector2D".to_string()
                        }
                    ...
                    "MaterialParameterCollection" => {
                        return "UMaterialParameterCollection*".to_string()
                    }
                    ...
                    _ => {
                            ...
                            .open("C:/temp/kain_debug.txt")
                        {
                            ...
                            if let Some(knowledge) = &self.knowledge {
                                let _ = writeln!(
                                    file,
                                    "[DEBUG TypeMapper] EngineKnowledge available for: {}",
                                    name
                                );
                                // Check if it's a known engine type with automatic C++ mapping
                                ...
                                let _ = writeln!(
                                    file,
                                    "[DEBUG TypeMapper] No cpp_type from knowledge for: {}",
                                    name
                                );
                            } else {
                                let _ = writeln!(
                                    file,
                                    "[DEBUG TypeMapper] EngineKnowledge NOT available!"
                                );
                            }
                        }
                ...
                if generics.is_empty() {
                    ...
                    let gen_strs: Vec<String> =
                        generics.iter().map(|g| self.map_type_string(g)).collect();
                    format!("{}<{}>", ue_name, gen_strs.join(", "))
                }
            }
            Type::Tuple(types, _) => {
                let type_strs: Vec<String> =
                    types.iter().map(|t| self.map_type_string(t)).collect();
                format!("TTuple<{}>", type_strs.join(", "))
            }
            Type::Array(inner, size, _) => {
            ...
            Type::Function {
                ...
                let param_strs: Vec<String> =
                    params.iter().map(|p| self.map_type_string(p)).collect();
                format!(
                    "TFunction<{}({})>",
    ...
    pub fn is_pointer_type(&self, ty: &Type) -> bool {
        match ty {
            Type::Named { name, .. } => self.is_pointer_type_by_name(name),
            _ => false,
        }
    }
