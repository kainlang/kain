...
use kain_core::ast::{Trait, Impl, Function};
...
fn generate_trait_method_signature(method: &kain_core::ast::TraitMethod, trait_name: &str) -> String {
...
pub fn generate_trait_impl_methods(
    ...
    for method in &impl_block.methods {
        let trait_method = trait_def.methods.iter()
            .find(|tm| tm.name == method.name);
...
pub fn generate_class_interface_list(
    ...
    let interfaces: Vec<String> = impls.iter()
        .filter_map(|impl_block| {
            impl_block.trait_name.as_ref().map(|trait_name| {
                format!(", public I{}", trait_name)
            })
        })
        .collect();
