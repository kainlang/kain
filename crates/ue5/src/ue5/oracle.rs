...
fn validate_name_collisions(ctx: &mut ValidationContext, program: &TypedProgram, kb: &EngineKnowledge) {
    ...
    const UE5_MACROS: &[&str] = &[
        ...
        "UPARAM", "UDELEGATE", "DECLARE_DYNAMIC_MULTICAST_DELEGATE",
    ];
...
fn validate_circular_dependencies(ctx: &mut ValidationContext, program: &TypedProgram) {
    ...
    for type_name in dependencies.keys() {
        if !visited.contains(type_name) {
            if detect_cycle(type_name, &dependencies, &mut visited, &mut rec_stack, &mut Vec::new(), ctx) {
