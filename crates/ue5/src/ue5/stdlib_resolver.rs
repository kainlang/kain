    ...
    pub fn new() -> Self {
        ...
        resolver.add(
            ...
            StdLibMapping {
                ...
                ue5_template:
                    "[&](){ TArray<FString> Parts; $0.ParseIntoArray(Parts, *$1); return Parts; }()"
                        .into(),
                param_count: 2,
                requires_include: None,
    ...
    fn gen_vector_componentwise(fn_name: &str, arg: &str) -> Option<String> {
        ...
        match fn_name {
            "floor" => {
                if is_vec2 {
                    Some(format!(
                        "FVector2D(FMath::FloorToFloat(({}).X), FMath::FloorToFloat(({}).Y))",
                        arg, arg
                    ))
                } else if is_vec4 {
            ...
            "frac" | "fract" => {
                if is_vec2 {
                    Some(format!(
                        "FVector2D(FMath::Frac(({}).X), FMath::Frac(({}).Y))",
                        arg, arg
                    ))
                } else if is_vec4 {
                    ...
                    Some(format!(
                        "FVector(FMath::Frac(({}).X), FMath::Frac(({}).Y), FMath::Frac(({}).Z))",
                        arg, arg, arg
                    ))
                }
            }
            "abs" => {
                if is_vec2 {
                    Some(format!(
                        "FVector2D(FMath::Abs(({}).X), FMath::Abs(({}).Y))",
                        arg, arg
                    ))
                } else if is_vec4 {
                    ...
                    Some(format!(
                        "FVector(FMath::Abs(({}).X), FMath::Abs(({}).Y), FMath::Abs(({}).Z))",
                        arg, arg, arg
                    ))
                }
            }
            "ceil" => {
                if is_vec2 {
                    Some(format!(
                        "FVector2D(FMath::CeilToFloat(({}).X), FMath::CeilToFloat(({}).Y))",
                        arg, arg
                    ))
                } else if is_vec4 {
            ...
            "round" => {
                if is_vec2 {
                    Some(format!(
                        "FVector2D(FMath::RoundToFloat(({}).X), FMath::RoundToFloat(({}).Y))",
                        arg, arg
                    ))
                } else if is_vec4 {
            ...
            "sqrt" => {
                if is_vec2 {
                    Some(format!(
                        "FVector2D(FMath::Sqrt(({}).X), FMath::Sqrt(({}).Y))",
                        arg, arg
                    ))
                } else if is_vec4 {
                    ...
                    Some(format!(
                        "FVector(FMath::Sqrt(({}).X), FMath::Sqrt(({}).Y), FMath::Sqrt(({}).Z))",
                        arg, arg, arg
                    ))
                }
            }
...
mod tests {
    ...
    fn test_min_max_clamp() {
        ...
        assert_eq!(
            resolver.resolve(
                "clamp",
                &["150.0".to_string(), "0.0".to_string(), "100.0".to_string()]
            ),
    ...
    fn test_interpolation() {
        ...
        assert_eq!(
            resolver.resolve(
                "lerp",
                &["0.0".to_string(), "100.0".to_string(), "0.5".to_string()]
            ),
        ...
        assert_eq!(
            resolver.resolve(
                "smoothstep",
                &["0.0".to_string(), "1.0".to_string(), "0.5".to_string()]
            ),
    ...
    fn test_error_handling() {
        ...
        assert!(resolver
            .resolve("sqrt", &["1.0".to_string(), "2.0".to_string()])
            .is_err());
    }
    ...
    fn test_all_20_functions() {
        ...
        assert!(
            functions.len() >= 20,
            "Expected at least 20 stdlib functions, got {}",
            functions.len()
        );
    ...
    fn test_all_32_functions() {
        ...
        assert!(
            functions.len() >= 32,
            "Expected at least 32 stdlib functions, got {}",
            functions.len()
        );

        // Verify all collection functions exist
        let collection_functions = vec![
            "len", "push", "pop", "first", "last", "reverse", "contains", "index_of", "remove",
            "clear", "is_empty", "reserve",
        ];
    ...
    fn test_string_functions() {
        ...
        assert_eq!(
            resolver.resolve(
                "str_contains",
                &["str".to_string(), "TEXT(\"sub\")".to_string()]
            ),
        ...
        assert_eq!(
            resolver.resolve(
                "starts_with",
                &["str".to_string(), "TEXT(\"prefix\")".to_string()]
            ),
        ...
        assert_eq!(
            resolver.resolve(
                "ends_with",
                &["str".to_string(), "TEXT(\"suffix\")".to_string()]
            ),
        ...
        assert_eq!(
            resolver.resolve(
                "substring",
                &["str".to_string(), "0".to_string(), "5".to_string()]
            ),
    ...
    fn test_all_47_functions() {
        ...
        assert!(
            functions.len() >= 47,
            "Expected at least 47 stdlib functions, got {}",
            functions.len()
        );
    }
