...
use crate::blueprint_ir::{
    ...
    AsyncBlueprintIR, AsyncOutputPinIR,
};
...
fn generate_event_header_declaration(
    ...
    // Generate parameters
    let params: Vec<String> = ir.params.iter()
...
fn generate_k2node_header(
    ...
    output.push_str(&format!("class {} {} : public UK2Node\n", api_macro, node_class_name));
    output.push_str("{\n");
...
fn generate_k2node_source(
    ...
    // Pin name constants
    for pin in &ir.input_pins {
        output.push_str(&format!("const FName {}::{}PinName(TEXT(\"{}\"));\n", 
    ...
    // AllocateDefaultPins
    output.push_str(&format!("void {}::AllocateDefaultPins()\n", node_class_name));
    output.push_str("{\n");
    ...
    // GetNodeTitle
    output.push_str(&format!("FText {}::GetNodeTitle(ENodeTitleType::Type TitleType) const\n", node_class_name));
    output.push_str("{\n");
    output.push_str(&format!("    return FText::FromString(TEXT(\"{}\"));\n", ir.node_title));
    output.push_str("}\n\n");
    
    // GetTooltipText
    output.push_str(&format!("FText {}::GetTooltipText() const\n", node_class_name));
    output.push_str("{\n");
    output.push_str(&format!("    return FText::FromString(TEXT(\"{}\"));\n", ir.node_title));
    output.push_str("}\n\n");
    
    // GetNodeTitleColor
    output.push_str(&format!("FLinearColor {}::GetNodeTitleColor() const\n", node_class_name));
    output.push_str("{\n");
    ...
    // GetIconAndTint
    output.push_str(&format!("FSlateIcon {}::GetIconAndTint(FLinearColor& OutColor) const\n", node_class_name));
    output.push_str("{\n");
...
fn generate_pin_creation(pin: &K2PinIR, is_input: bool, output: &mut String) {
    let direction = if is_input { "EGPD_Input" } else { "EGPD_Output" };
    let pin_category = map_pin_type_to_category(&pin.pin_type);
    
    output.push_str(&format!("    CreatePin({}, {}, {}PinName);\n", 
...
fn generate_async_action_header(
    ...
    output.push_str(" */\n");
    output.push_str("UCLASS()\n");
    output.push_str(&format!("class {} {} : public UBlueprintAsyncActionBase\n", api_macro, action_class_name));
    output.push_str("{\n");
    output.push_str("    GENERATED_BODY()\n\n");
    output.push_str("public:\n");
    ...
    output.push_str(&ir.category);
    output.push_str("\")\n");
    output.push_str(&format!("    static {}* {}(UObject* WorldContextObject", action_class_name, ir.action_name));
...
fn generate_async_action_source(
    ...
    // Factory method
    output.push_str(&format!("{}* {}::{}(UObject* WorldContextObject", 
    ...
    output.push_str(")\n");
    output.push_str("{\n");
    output.push_str(&format!("    {}* Action = NewObject<{}>();\n", action_class_name, action_class_name));
...
mod tests {
    ...
    fn make_simple_event() -> BlueprintEventIR {
        BlueprintEventIR {
            ...
            implementation_body: Some("UE_LOG(LogTemp, Log, TEXT(\"Event triggered\"));".to_string()),
        }
    }
    ...
    fn test_generate_blueprint_event_header() {
        ...
        assert!(output.header_declaration.contains("UFUNCTION(BlueprintNativeEvent"));
        assert!(output.header_declaration.contains("void OnCustomEvent(int32 value)"));
        assert!(output.header_declaration.contains("OnCustomEvent_Implementation"));
    }
    ...
    fn test_generate_blueprint_event_source() {
        ...
        assert!(output.source_implementation.contains("AMyActor::OnCustomEvent_Implementation"));
        assert!(output.source_implementation.contains("UE_LOG(LogTemp, Log, TEXT(\"Event triggered\"))"));
    }
    ...
    fn test_generate_k2node_header() {
        ...
        assert!(output.header.contains("class TESTPLUGIN_API UK2Node_MyCustomNode"));
    ...
    fn test_generate_async_blueprint_header() {
        ...
        assert!(output.header.contains("class TESTPLUGIN_API UMyAsyncAction"));
