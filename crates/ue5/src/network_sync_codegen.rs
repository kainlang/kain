...
use crate::network_sync_ir::{NetworkSyncIR, ReplicatedPropertyIR, ReplicationModeIR, NetworkConfigIR};
...
fn generate_state_buffers(ir: &NetworkSyncIR, output: &mut NetworkSyncCodegenOutput) {
    ...
    for prop in &ir.replicated_properties {
        if matches!(prop.mode, ReplicationModeIR::Interpolated { .. }) {
            output.header_declarations.push_str(&format!("        UPROPERTY()\n"));
            output.header_declarations.push_str(&format!("        {} {};\n\n", prop.cpp_type, prop.name));
        }
    }
...
fn generate_constructor_init(ir: &NetworkSyncIR, output: &mut NetworkSyncCodegenOutput) {
    ...
    for prop in &ir.replicated_properties {
        if let ReplicationModeIR::Interpolated { back_time, buffer_size } = prop.mode {
            output.constructor_body.push_str(&format!("    InterpolationBackTime = {}f;\n", back_time));
            output.constructor_body.push_str(&format!("    StateBuffer.Reserve({});\n", buffer_size));
...
fn generate_tick_logic(ir: &NetworkSyncIR, class_name: &str, output: &mut NetworkSyncCodegenOutput) {
    let mut has_interpolated = false;
...
fn generate_interpolation_logic(ir: &NetworkSyncIR, output: &mut NetworkSyncCodegenOutput) {
    ...
    for prop in &ir.replicated_properties {
        if matches!(prop.mode, ReplicationModeIR::Interpolated { .. }) {
            output.tick_body.push_str(&format!("                // Interpolate {}\n", prop.name));
...
fn generate_extrapolation_logic(ir: &NetworkSyncIR, output: &mut NetworkSyncCodegenOutput) {
    ...
    for prop in &ir.replicated_properties {
        if let ReplicationModeIR::Extrapolated { limit } = prop.mode {
            output.tick_body.push_str(&format!("        // Extrapolate {} with limit {}\n", prop.name, limit));
...
fn generate_snap_threshold_checks(ir: &NetworkSyncIR, output: &mut NetworkSyncCodegenOutput) {
    ...
    for prop in &ir.replicated_properties {
        if (prop.name.contains("position") || prop.name.contains("Position") || prop.name.contains("location") || prop.name.contains("Location"))
...
fn generate_replication_setup(ir: &NetworkSyncIR, class_name: &str, output: &mut NetworkSyncCodegenOutput) {
    ...
    for prop in &ir.replicated_properties {
        match &prop.mode {
            ...
            ReplicationModeIR::Compressed { threshold, use_half_float } => {
...
mod tests {
    use super::*;
    use crate::network_sync_ir::{ReplicatedPropertyIR, ReplicationModeIR, NetworkConfigIR};
    
    #[test]
    fn test_generate_simple_replication() {
        let ir = NetworkSyncIR {
            component_name: "TestComponent".to_string(),
            replicated_properties: vec![
                ReplicatedPropertyIR {
        ...
        assert!(output.replication_body.contains("DOREPLIFETIME(UTestComponent, Health)"));
        assert!(output.constructor_body.contains("SetIsReplicatedByDefault(true)"));
    }
    ...
    fn test_generate_interpolated_replication() {
        ...
        assert!(output.header_declarations.contains("TArray<FNetworkState> StateBuffer"));
        assert!(output.constructor_body.contains("InterpolationBackTime = 0.1f"));
