...
use kain_core::ast::{Attribute, Block, StateDef, StateMachineDef, TransitionDef};
...
fn test_state_machine_end_to_end() {
    ...
    let ir =
        convert_to_state_machine_ir(&state_machine_ast, &ctx).expect("Failed to convert to IR");
    ...
    assert!(output
        .header
        .contains("enum class ECharacterAnimationsState"));
    ...
    assert!(output
        .header
        .contains("class TESTPLUGIN_API UCharacterAnimationsStateMachine"));
    assert!(output
        .header
        .contains("ECharacterAnimationsState CurrentState"));
    ...
    assert!(output
        .header
        .contains("void UpdateStateMachine(float DeltaTime)"));
    assert!(output
        .header
        .contains("UAnimSequence* GetCurrentAnimation()"));
    ...
    assert!(output
        .source
        .contains("UCharacterAnimationsStateMachine::UCharacterAnimationsStateMachine()"));
    assert!(output
        .source
        .contains("CurrentState = ECharacterAnimationsState::Idle"));

    assert!(output
        .source
        .contains("void UCharacterAnimationsStateMachine::UpdateStateMachine(float DeltaTime)"));
    assert!(output.source.contains("switch (CurrentState)"));
    assert!(output
        .source
        .contains("case ECharacterAnimationsState::Idle:"));
    assert!(output.source.contains("if (CanTransitionToWalk())"));
    assert!(output
        .source
        .contains("TransitionToState(ECharacterAnimationsState::Walk)"));

    assert!(output
        .source
        .contains("UAnimSequence* UCharacterAnimationsStateMachine::GetCurrentAnimation() const"));
    ...
    assert!(output
        .source
        .contains("bool UCharacterAnimationsStateMachine::CanTransitionToWalk() const"));
    assert!(output
        .source
        .contains("bool UCharacterAnimationsStateMachine::CanTransitionToRun() const"));
    assert!(output
        .source
        .contains("bool UCharacterAnimationsStateMachine::CanTransitionToIdle() const"));
    ...
    assert!(output
        .includes
        .contains(&"Animation/AnimSequence.h".to_string()));
}
...
fn test_state_machine_with_callbacks() {
    ...
    assert!(output
        .source
        .contains("void UCharacterAnimationsStateMachine::OnEnterIdle()"));
    assert!(output
        .source
        .contains("void UCharacterAnimationsStateMachine::OnExitIdle()"));
...
fn test_state_machine_transition_priority() {
    ...
    let walk_case_start = output
        .source
        .find("case ECharacterAnimationsState::Walk:")
        .unwrap();
    let run_transition = output.source[walk_case_start..]
        .find("if (CanTransitionToRun())")
        .unwrap();
    let idle_transition = output.source[walk_case_start..]
        .find("if (CanTransitionToIdle())")
        .unwrap();
...
fn test_state_machine_no_transitions() {
    ...
    assert!(output
        .header
        .contains("enum class ESimpleStateMachineState"));
    assert!(output
        .header
        .contains("OnlyState UMETA(DisplayName = \"OnlyState\")"));
    assert!(output
        .source
        .contains("case ESimpleStateMachineState::OnlyState:"));
