...
fn test_tag_hierarchy_flattening() {
    ...
    let tags: Vec<String> = ir.namespaces[0].tags.iter()
        .map(|t| t.tag.clone())
        .collect();
...
fn test_parent_tag_extraction() {
    ...
    let stunned_tag = ir.namespaces[0].tags.iter()
    ...
    // Find the CC tag
    let cc_tag = ir.namespaces[0].tags.iter()
...
fn test_cpp_name_generation() {
    ...
    let melee_tag = ir.namespaces[0].tags.iter()
...
fn test_native_cpp_header_generation() {
    ...
    assert!(output.header.contains("MYGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Melee)"));
}
...
fn test_native_cpp_implementation_generation() {
    ...
    assert!(output.implementation.contains("#include \"GameplayTags.h\""));
...
fn test_leaf_name_extraction() {
    ...
    let tag_c = ir.namespaces[0].tags.iter()
...
fn test_all_tags_accessor() {
    ...
    let tag_strings: Vec<String> = all_tags.iter()
        .map(|t| t.tag.clone())
        .collect();
...
fn test_codegen_compiles_valid_cpp() {
    ...
    assert!(output.header.contains("MYGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Stunned)"));
    assert!(output.header.contains("MYGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Rooted)"));
    ...
    assert!(output.implementation.contains("\"Character is stunned and cannot act\""));
    assert!(output.implementation.contains("Rooted"));
    assert!(output.implementation.contains("\"Status.CC.Rooted\""));
    assert!(output.implementation.contains("\"Character is rooted in place\""));
}
...
fn test_ini_file_format() {
    ...
    assert!(output.ini_file.contains("GameplayTagList=(Tag=\"Damage.Physical\",DevComment=\"Physical damage type\")"));
    assert!(output.ini_file.contains("GameplayTagList=(Tag=\"Damage.Magical\",DevComment=\"Magical damage type\")"));
}
