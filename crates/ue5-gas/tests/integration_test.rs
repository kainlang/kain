use kain_core::{lexer::Lexer, parser::Parser, diagnostics::SpanMapper};
...
"#;
    ...
    assert!(ability_ns.tags.iter().any(|t| t.tag == "Ability.Attack.Melee"));
    assert!(ability_ns.tags.iter().any(|t| t.tag == "Ability.Attack.Melee.Sword"));
    ...
    assert!(output.header.contains("TESTGAME_API UE_DECLARE_GAMEPLAY_TAG_EXTERN"));
    ...
    assert!(output.implementation.contains("\"Ability.Attack.Melee.Sword\""));
    ...
    assert!(output.ini_file.contains("GameplayTagList=(Tag=\"Ability.Attack\")"));
    assert!(output.ini_file.contains("GameplayTagList=(Tag=\"Status.Alive\")"));
}
