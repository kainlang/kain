...
use ue5_gas::{AttributeSetIR, AttributeIR, LifecycleHooksIR, generate_attribute_set};
...
fn test_attribute_accessors_generation() {
    ...
    assert!(output.header.contains("ATTRIBUTE_ACCESSORS(UHealthSet, Health)"));
    assert!(output.header.contains("ATTRIBUTE_ACCESSORS(UHealthSet, MaxHealth)"));
}
...
fn test_class_declaration() {
    ...
    assert!(output.header.contains("class UHealthSet : public UAttributeSet"));
...
fn test_replication_setup() {
    ...
    assert!(output.source.contains("void UHealthSet::GetLifetimeReplicatedProps"));
    assert!(output.source.contains("DOREPLIFETIME_CONDITION_NOTIFY(UHealthSet, Health, COND_None, REPNOTIFY_Always)"));
    assert!(output.source.contains("DOREPLIFETIME_CONDITION_NOTIFY(UHealthSet, MaxHealth, COND_None, REPNOTIFY_Always)"));
}
...
fn test_rep_notify_functions() {
    ...
    // Header declarations
    assert!(output.header.contains("void OnRep_Health(const FGameplayAttributeData& OldValue)"));
    assert!(output.header.contains("void OnRep_MaxHealth(const FGameplayAttributeData& OldValue)"));
    
    // Source implementations
    assert!(output.source.contains("void UHealthSet::OnRep_Health"));
    assert!(output.source.contains("GAMEPLAYATTRIBUTE_REPNOTIFY(UHealthSet, Health, OldValue)"));
    assert!(output.source.contains("void UHealthSet::OnRep_MaxHealth"));
    assert!(output.source.contains("GAMEPLAYATTRIBUTE_REPNOTIFY(UHealthSet, MaxHealth, OldValue)"));
}
...
fn test_uproperty_generation() {
    ...
    assert!(output.header.contains("UPROPERTY(BlueprintReadOnly, ReplicatedUsing = OnRep_Health"));
    assert!(output.header.contains("HideFromModifiers"));
    
    // MaxHealth without HideFromModifiers
    assert!(output.header.contains("UPROPERTY(BlueprintReadOnly, ReplicatedUsing = OnRep_MaxHealth"));
...
fn test_includes() {
    ...
    assert!(output.header.contains("#include \"AbilitySystemComponent.h\""));
    ...
    assert!(output.source.contains("#include \"GameplayEffectExtension.h\""));
}
...
fn test_meta_attributes() {
    ...
    assert!(!output.source.contains("DOREPLIFETIME_CONDITION_NOTIFY(UHealthSet, Damage"));
    
    // But should still have ATTRIBUTE_ACCESSORS
    assert!(output.header.contains("ATTRIBUTE_ACCESSORS(UHealthSet, Damage)"));
...
fn test_compression_ratio() {
    ...
    println!("Compression ratio: 2 attributes → {} C++ lines (1:{})", total_lines, total_lines / 2);
}
...
fn test_lifecycle_hooks_post_gameplay_effect_execute() {
    ...
    assert!(output.header.contains("virtual void PostGameplayEffectExecute"));
    assert!(output.source.contains("void UHealthSet::PostGameplayEffectExecute"));
    assert!(output.source.contains("Super::PostGameplayEffectExecute(Data)"));
