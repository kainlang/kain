...
use kain_core::ast::{Attribute, Block, CueType, Field, Function, GameplayCueDef};
use kain_core::span::Span;
use ue5_gas::cue_ir::{CueTypeIR, GameplayCueIR};
...
fn test_tag_validation_missing_prefix() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("must start with 'GameplayCue.'"));
}
...
fn test_missing_gameplay_cue_attribute() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("must have @gameplay_cue attribute"));
}
