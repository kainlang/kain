use crate::tags_ir::{GameplayTagIR, GameplayTagsIR};
use anyhow::Result;
...
fn generate_namespace_hierarchy(
    ...
    fn emitted_leaf_symbol_name(
        tag: &GameplayTagIR,
        has_same_named_child_namespace: bool,
    ) -> String {
    ...
    let child_namespace_names: std::collections::BTreeSet<String> =
        groups.keys().filter(|k| !k.is_empty()).cloned().collect();
    ...
    for (ns_name, group_tags) in groups {
        if ns_name.is_empty() {
            ...
            for tag in group_tags {
                let symbol_name = emitted_leaf_symbol_name(
                    tag,
                    child_namespace_names.contains(&tag.leaf_name().to_string()),
                );
                if is_header {
...
mod tests {
    ...
    fn test_nested_namespaces() {
        ...
        assert!(output
            .header
            .contains("UE_DECLARE_GAMEPLAY_TAG_EXTERN(Stunned)"));
        assert!(output
            .header
            .contains("UE_DECLARE_GAMEPLAY_TAG_EXTERN(Rooted)"));
    ...
    fn test_ini_file_generation() {
        ...
        assert!(output.ini_file.contains(
