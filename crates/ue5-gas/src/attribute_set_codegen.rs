...
use crate::attribute_set_ir::{AttributeSetIR, AttributeIR, DelegateIR};
...
fn generate_header(ir: &AttributeSetIR, class_name: &str, _plugin_name: &str) -> KainResult<String> {
    ...
    for attr in &ir.attributes {
        output.push_str(&format!("\tATTRIBUTE_ACCESSORS({}, {});\n", 
            class_name, capitalize_first(&attr.name)));
    }
    output.push_str("\n");
    ...
    if !ir.delegates.is_empty() {
        ...
        for delegate in &ir.delegates {
            output.push_str(&format!("\t{} {};\n", delegate.delegate_type, capitalize_first(&delegate.name)));
        }
        output.push_str("\n");
    }
    ...
    for attr in &ir.attributes {
        if attr.rep_notify {
            ...
            output.push_str(&format!("\tvoid OnRep_{}(const FGameplayAttributeData& OldValue);\n\n", 
...
fn generate_source(ir: &AttributeSetIR, class_name: &str, _plugin_name: &str) -> KainResult<String> {
    let mut output = String::new();
...
fn generate_attribute_property(attr: &AttributeIR, _class_name: &str) -> KainResult<String> {
    ...
    if attr.replicated && attr.rep_notify {
        specifiers.push(format!("ReplicatedUsing = OnRep_{}", capitalize_first(&attr.name)));
    }
    ...
    output.push_str(&format!("\tFGameplayAttributeData {};\n\n", capitalize_first(&attr.name)));
...
fn generate_constructor(ir: &AttributeSetIR, class_name: &str) -> KainResult<String> {
    ...
    for attr in &ir.attributes {
        if let Some(ref default_val) = attr.default_value {
            output.push_str(&format!("\t{} = {}f;\n", capitalize_first(&attr.name), default_val));
        }
    }
...
fn generate_replication_function(ir: &AttributeSetIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str(&format!("void {}::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const\n", class_name));
    output.push_str("{\n");
    ...
    for attr in &ir.attributes {
        if attr.replicated {
            if attr.rep_notify {
                output.push_str(&format!("\tDOREPLIFETIME_CONDITION_NOTIFY({}, {}, COND_None, REPNOTIFY_Always);\n",
                    class_name, capitalize_first(&attr.name)));
            } else {
                output.push_str(&format!("\tDOREPLIFETIME({}, {});\n",
                    class_name, capitalize_first(&attr.name)));
            }
        }
    }
...
fn generate_rep_notify_function(attr: &AttributeIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str("{\n");
    output.push_str(&format!("\tGAMEPLAYATTRIBUTE_REPNOTIFY({}, {}, OldValue);\n", 
        class_name, attr_name));
    output.push_str("}\n");
...
fn generate_pre_gameplay_effect_execute(_ir: &AttributeSetIR, class_name: &str, _hook: &crate::attribute_set_ir::FunctionIR) -> KainResult<String> {
    let mut output = String::new();
    
    output.push_str(&format!("bool {}::PreGameplayEffectExecute(FGameplayEffectModCallbackData& Data)\n", class_name));
    output.push_str("{\n");
...
fn generate_post_gameplay_effect_execute(ir: &AttributeSetIR, class_name: &str, _hook: &crate::attribute_set_ir::FunctionIR) -> KainResult<String> {
    let mut output = String::new();
    
    output.push_str(&format!("void {}::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)\n", class_name));
    output.push_str("{\n");
    ...
    if !meta_attrs.is_empty() {
        for attr in meta_attrs {
            let attr_name = capitalize_first(&attr.name);
            output.push_str(&format!("\tif (Data.EvaluatedData.Attribute == Get{}Attribute())\n", attr_name));
            output.push_str("\t{\n");
            output.push_str(&format!("\t\t// Handle {} meta attribute\n", attr.name));
            output.push_str(&format!("\t\tconst float {}Value = Get{}();\n", attr.name, attr_name));
            output.push_str(&format!("\t\tSet{}(0.0f); // Reset meta attribute\n", attr_name));
...
fn generate_pre_attribute_change(ir: &AttributeSetIR, class_name: &str, _hook: &crate::attribute_set_ir::FunctionIR) -> KainResult<String> {
    let mut output = String::new();
    
    output.push_str(&format!("void {}::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)\n", class_name));
    output.push_str("{\n");
    ...
    for attr in &ir.attributes {
        if attr.clamp_min.is_some() || attr.clamp_max.is_some() {
            let attr_name = capitalize_first(&attr.name);
            output.push_str(&format!("\tif (Attribute == Get{}Attribute())\n", attr_name));
            output.push_str("\t{\n");
            
            if let (Some(min), Some(max)) = (attr.clamp_min, attr.clamp_max) {
                output.push_str(&format!("\t\tNewValue = FMath::Clamp(NewValue, {}f, {}f);\n", min, max));
...
fn generate_post_attribute_change(_ir: &AttributeSetIR, class_name: &str, _hook: &crate::attribute_set_ir::FunctionIR) -> KainResult<String> {
    let mut output = String::new();
