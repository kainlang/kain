...
use crate::cue_ir::{CueTypeIR, GameplayCueIR};
...
fn generate_static_cue(
    cue_ir: &GameplayCueIR,
    _plugin_name: &str,
) -> KainResult<GameplayCueOutput> {
...
fn generate_static_header(cue_ir: &GameplayCueIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str(&format!(
        "class {} : public UGameplayCueNotify_Static\n",
        class_name
    ));
    output.push_str("{\n");
...
fn generate_static_source(cue_ir: &GameplayCueIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str("{\n");
    output.push_str(&format!(
        "\tGameplayCueTag = FGameplayTag::RequestGameplayTag(FName(\"{}\"));\n",
        cue_ir.tag
    ));
    output.push_str("}\n\n");
...
fn generate_actor_header(cue_ir: &GameplayCueIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str("UCLASS()\n");
    output.push_str(&format!(
        "class {} : public AGameplayCueNotify_Actor\n",
        class_name
    ));
    output.push_str("{\n");
    output.push_str("\tGENERATED_BODY()\n\n");
