...
fn generate_header(task_ir: &AbilityTaskIR, class_name: &str) -> KainResult<String> {
    ...
    for delegate in &task_ir.delegates {
        ...
        output.push_str(&format!(
            "{}({});\n",
            delegate_macro,
            get_delegate_signature(&delegate.delegate_type)
        ));
    }
    ...
    for delegate in &task_ir.delegates {
        ...
        output.push_str(&format!(
            "\t{} {};\n\n",
            get_delegate_type_name(&delegate.delegate_type),
            delegate.name
        ));
    }
    ...
    output.push_str(&format!(
        "\tstatic {}* Create{}(UGameplayAbility* OwningAbility);\n\n",
...
fn generate_source(task_ir: &AbilityTaskIR, class_name: &str) -> KainResult<String> {
    ...
    // Static factory method
    output.push_str(&format!(
        "{}* {}::Create{}(UGameplayAbility* OwningAbility)\n",
        class_name, class_name, task_ir.name
    ));
    output.push_str("{\n");
    output.push_str(&format!(
        "\t{}* Task = NewAbilityTask<{}>(OwningAbility);\n",
        class_name, class_name
    ));
...
fn get_delegate_macro(delegate_type: &DelegateTypeIR) -> String {
    match delegate_type {
        DelegateTypeIR::AttributeChange => {
            "DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam".to_string()
        }
...
fn get_delegate_signature(delegate_type: &DelegateTypeIR) -> String {
    match delegate_type {
        ...
        DelegateTypeIR::TargetDataReady => {
            "FTargetDataDelegate, const FGameplayAbilityTargetDataHandle&, Data".to_string()
        }
        DelegateTypeIR::GameplayEvent => {
            "FGameplayEventDelegate, FGameplayTag, EventTag, const FGameplayEventData&, Payload"
                .to_string()
        }
