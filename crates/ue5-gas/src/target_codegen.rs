...
fn generate_header(target_ir: &TargetActorIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str(&format!(
        "class {} : public AGameplayAbilityTargetActor\n",
        class_name
    ));
    output.push_str("{\n");
    ...
    if target_ir.max_range.is_some() {
        output
            .push_str("\tUPROPERTY(BlueprintReadWrite, EditAnywhere, Category = \"Targeting\")\n");
...
fn generate_source(target_ir: &TargetActorIR, class_name: &str) -> KainResult<String> {
    ...
    // StartTargeting
    output.push_str(&format!(
        "void {}::StartTargeting(UGameplayAbility* Ability)\n",
        class_name
    ));
    output.push_str("{\n");
