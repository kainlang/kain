...
use crate::ability_ir::{
    GameplayAbilityIR, InstancingPolicy, NetExecutionPolicy, ReplicationPolicy,
};
...
fn generate_header(
    ...
    if ir.lifecycle_hooks.activate_ability.is_some() {
        output
            .push_str("\tvirtual void ActivateAbility(const FGameplayAbilitySpecHandle Handle,\n");
        output.push_str(
            "\t                              const FGameplayAbilityActorInfo* ActorInfo,\n",
        );
    ...
    if ir.lifecycle_hooks.input_pressed.is_some() {
        ...
        output
            .push_str("\t                           const FGameplayAbilityActorInfo* ActorInfo,\n");
...
fn generate_constructor(ir: &GameplayAbilityIR, class_name: &str) -> KainResult<String> {
    ...
    if !ir.activation_owned_tags.is_empty() {
        ...
        for tag in &ir.activation_owned_tags {
            output.push_str(&format!(
                "\tActivationOwnedTags.AddTag(FGameplayTag::RequestGameplayTag(FName(\"{}\")));\n",
                tag
            ));
        }
        output.push_str("\n");
    ...
    if let Some(ref cost) = ir.cost_effect {
        ...
        output.push_str(&format!(
            "\tCostGameplayEffectClass = U{}::StaticClass();\n\n",
            cost
        ));
    }
    ...
    if let Some(ref cooldown) = ir.cooldown_effect {
        ...
        output.push_str(&format!(
            "\tCooldownGameplayEffectClass = U{}::StaticClass();\n\n",
            cooldown
        ));
    }
...
fn generate_input_released(_ir: &GameplayAbilityIR, class_name: &str) -> KainResult<String> {
    ...
    output.push_str(&format!(
        "void {}::InputReleased(const FGameplayAbilitySpecHandle Handle,\n",
        class_name
    ));
    output.push_str("                            const FGameplayAbilityActorInfo* ActorInfo,\n");
    output.push_str(
