...
fn make_config_attribute_with_args(args: Vec<(&str, Expr)>) -> Attribute {
    Attribute {
        ...
        args: args
            .into_iter()
            .map(|(name, value)| make_named_arg(name, value))
            .collect(),
        span: Span::default(),
    }
}
...
fn test_integration_game_config_single_float_setting() {
    ...
    assert!(
        header.content.contains("UVoxelSettings"),
        "Header should contain class name"
    );
    assert!(
        header.content.contains("UDeveloperSettings"),
        "Header should inherit from UDeveloperSettings"
    );
    assert!(
        header.content.contains("UCLASS(Config=Game"),
        "Header should have Config=Game"
    );
    assert!(
        header.content.contains("float ChunkSize"),
        "Header should have ChunkSize property"
    );
    assert!(
        header.content.contains("ClampMin=\"10"),
        "Header should have ClampMin meta"
    );
    assert!(
        header.content.contains("ClampMax=\"1000"),
        "Header should have ClampMax meta"
    );
    ...
    assert!(
        source.content.contains("UVoxelSettings::UVoxelSettings()"),
        "Source should have constructor"
    );
    assert!(
        source.content.contains("ChunkSize(100.0f)"),
        "Source should initialize ChunkSize"
    );
    assert!(
        source
            .content
            .contains("const UVoxelSettings* UVoxelSettings::Get()"),
        "Source should have Get() method"
    );
}
...
fn test_integration_engine_config_multiple_settings() {
    ...
    let fields = vec![
        make_field(
            "max_lod",
            "Int",
            Some(make_int_expr(4)),
            vec![make_setting_attribute(vec![
        ...
        make_field(
            "material_name",
            "String",
            Some(make_string_expr("DefaultMaterial")),
            vec![make_setting_attribute(vec![
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("UCLASS(Config=Engine"),
        "Should use Engine config"
    );
    assert!(
        header.content.contains("int32 MaxLod"),
        "Should have Int → int32 mapping"
    );
    assert!(
        header.content.contains("bool DebugVis"),
        "Should have Bool → bool mapping"
    );
    assert!(
        header.content.contains("FString MaterialName"),
        "Should have String → FString mapping"
    );

    let source = files.iter().find(|f| f.path.ends_with(".cpp")).unwrap();
    assert!(
        source.content.contains("MaxLod(4)"),
        "Should initialize Int field"
    );
    assert!(
        source.content.contains("DebugVis(false)"),
        "Should initialize Bool field"
    );
    assert!(
        source
            .content
            .contains("MaterialName(TEXT(\"DefaultMaterial\"))"),
        "Should initialize String field"
    );
}
...
fn test_integration_editor_config() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("UCLASS(Config=Editor"),
        "Should use Editor config"
    );
}
...
fn test_integration_editor_per_project_user_settings_config() {
    ...
    assert!(
        header
            .content
            .contains("UCLASS(Config=EditorPerProjectUserSettings"),
...
fn test_integration_custom_display_name() {
    ...
    assert!(
        header
            .content
            .contains("DisplayName=\"My Custom Settings\""),
...
fn test_integration_setting_with_tooltip() {
    ...
    let field = make_field(
        "speed",
        "Float",
        Some(make_float_expr(10.0)),
        vec![make_setting_attribute(vec![(
            "tooltip",
            make_string_expr("Movement speed in units per second"),
        )])],
    );
    ...
    assert!(
        header
            .content
            .contains("ToolTip=\"Movement speed in units per second\""),
...
fn test_integration_all_numeric_constraints() {
    ...
    let fields = vec![
        make_field(
            "min_only",
            "Float",
            Some(make_float_expr(0.0)),
            vec![make_setting_attribute(vec![("min", make_float_expr(0.0))])],
        ),
        make_field(
            "max_only",
            "Float",
            ...
            vec![make_setting_attribute(vec![(
                "max",
                make_float_expr(100.0),
            )])],
        ),
        make_field(
            "both",
            "Float",
            Some(make_float_expr(50.0)),
            vec![make_setting_attribute(vec![
    ...
    assert!(
        header.content.contains("float MinOnly"),
        "Should have MinOnly field"
    );
    let min_only_section = header.content.split("float MinOnly").next().unwrap();
    assert!(
        min_only_section.contains("ClampMin=\"0"),
        "MinOnly should have ClampMin"
    );

    // Check max_only has ClampMax but not ClampMin
    assert!(
        header.content.contains("float MaxOnly"),
        "Should have MaxOnly field"
    );

    // Check both has both ClampMin and ClampMax
    assert!(
        header.content.contains("float Both"),
        "Should have Both field"
    );
    let both_section = header.content.split("float Both").next().unwrap();
    assert!(
        both_section.contains("ClampMin=\"0"),
        "Both should have ClampMin"
    );
    assert!(
        both_section.contains("ClampMax=\"100"),
        "Both should have ClampMax"
    );
}
...
fn test_integration_blueprint_accessor_generation() {
    ...
    assert!(
        header.content.contains("UFUNCTION(BlueprintCallable"),
        "Should have BlueprintCallable function"
    );
    assert!(
        header.content.contains("static float GetHealth()"),
...
fn test_integration_writable_setting() {
    ...
    assert!(
        header.content.contains("static float GetVolume()"),
        "Should have GetVolume() getter"
    );
    assert!(
        header
            .content
            .contains("static void SetVolume(float NewValue)"),
...
fn test_integration_multiple_config_structs() {
    ...
    assert!(
        files.iter().any(|f| f.path.contains("GraphicsSettings.h")),
        "Should generate GraphicsSettings.h"
    );
    assert!(
        files.iter().any(|f| f.path.contains("AudioSettings.h")),
...
fn test_integration_type_mapping_float() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("float Value"),
        "Float should map to float"
    );

    let source = files.iter().find(|f| f.path.ends_with(".cpp")).unwrap();
    assert!(
        source.content.contains("Value(1.5f)"),
        "Float literal should have f suffix"
    );
}
...
fn test_integration_type_mapping_int() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("int32 Count"),
        "Int should map to int32"
    );

    let source = files.iter().find(|f| f.path.ends_with(".cpp")).unwrap();
    assert!(
        source.content.contains("Count(42)"),
        "Int literal should be plain number"
    );
}
...
fn test_integration_type_mapping_bool() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("bool Enabled"),
        "Bool should map to bool"
    );

    let source = files.iter().find(|f| f.path.ends_with(".cpp")).unwrap();
    assert!(
        source.content.contains("Enabled(true)"),
        "Bool literal should be true/false"
    );
}
...
fn test_integration_type_mapping_string() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("FString Name"),
        "String should map to FString"
    );

    let source = files.iter().find(|f| f.path.ends_with(".cpp")).unwrap();
    assert!(
        source.content.contains("Name(TEXT(\"Default\"))"),
        "String literal should use TEXT()"
    );
}
...
fn test_integration_no_default_value() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("float Optional"),
...
fn test_integration_empty_config_struct() {
    ...
    let header = files.iter().find(|f| f.path.ends_with(".h")).unwrap();
    assert!(
        header.content.contains("UEmptySettings"),
        "Should have class name"
    );
    assert!(
        header.content.contains("UDeveloperSettings"),
        "Should inherit from UDeveloperSettings"
    );
}
...
fn test_integration_complex_scenario() {
    ...
    let fields = vec![
        make_field(
            ...
            vec![make_setting_attribute(vec![
                ...
                (
                    "tooltip",
                    make_string_expr("Size of voxel chunks in world units"),
                ),
                ("cvar", make_string_expr("voxel.ChunkSize")),
                ("blueprint", make_bool_expr(true)),
                ("min", make_float_expr(10.0)),
        ...
        make_field(
            ...
            vec![make_setting_attribute(vec![
                (
                    "display_name",
                    make_string_expr("Enable Debug Visualization"),
                ),
                ("cvar", make_string_expr("voxel.DebugVis")),
                ("blueprint", make_bool_expr(true)),
                ("writable", make_bool_expr(true)),
        ...
        make_field(
            "material_path",
            "String",
            Some(make_string_expr("/Game/Materials/VoxelMaterial")),
            vec![make_setting_attribute(vec![
                ("display_name", make_string_expr("Material Path")),
                (
                    "tooltip",
                    make_string_expr("Path to the voxel material asset"),
                ),
            ])],
        ),
    ];
    ...
    assert!(
        result.is_ok(),
        "Complex scenario should generate successfully"
    );
    ...
    assert!(
        header.content.contains("UCLASS(Config=Game"),
        "Should use Game config"
    );
    assert!(
        header
            .content
            .contains("DisplayName=\"Advanced Voxel Settings\""),
        "Should use custom display name"
    );
    assert!(
        header.content.contains("float ChunkSize"),
        "Should have Float field"
    );
    assert!(
        header.content.contains("int32 MaxLod"),
        "Should have Int field"
    );
    assert!(
        header.content.contains("bool DebugVis"),
        "Should have Bool field"
    );
    assert!(
        header.content.contains("FString MaterialPath"),
    ...
    assert!(
        header.content.contains("ClampMax=\"1000"),
        "Should have max constraint"
    );
    assert!(
        header.content.contains("ToolTip=\"Size of voxel chunks"),
        "Should have tooltip"
    );
    assert!(
        header.content.contains("static float GetChunkSize()"),
        "Should have Blueprint getter"
    );
    assert!(
        header
            .content
            .contains("static void SetDebugVis(bool NewValue)"),
        "Should have Blueprint setter for writable field"
    );

    let source = files.iter().find(|f| f.path.ends_with(".cpp")).unwrap();
    assert!(
        source.content.contains("ChunkSize(100.0f)"),
        "Should initialize Float"
    );
    assert!(
        source.content.contains("MaxLod(4)"),
        "Should initialize Int"
    );
    assert!(
        source.content.contains("DebugVis(false)"),
        "Should initialize Bool"
    );
    assert!(
        source
            .content
            .contains("MaterialPath(TEXT(\"/Game/Materials/VoxelMaterial\"))"),
        "Should initialize String"
    );
}
