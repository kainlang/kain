...
impl TSGen {
    ...
    fn gen_program(&mut self, program: &TypedProgram) -> String {
        ...
        self.writeln("function __kain_union_wrap<T extends object>(value: T, active: string): T { Object.defineProperty(value, \"__kain_union_active\", { value: active, writable: true, enumerable: false, configurable: true }); return value; }");
        self.writeln("function __kain_union_get<T>(value: any, field: string, fallback: T): T { if (value == null) { return fallback; } return value.__kain_union_active === field ? (value[field] as T) : fallback; }");
        self.writeln("function __kain_union_set<T>(value: any, field: string, next: T): T { if (value != null) { value[field] = next; Object.defineProperty(value, \"__kain_union_active\", { value: field, writable: true, enumerable: false, configurable: true }); } return next; }");
        ...
        self.writeln("function __kain_bitfield_get(value: any, field: string, unitOffset: number, bitOffset: number, width: number): number { if (value == null) { return 0; } const units = __kain_bitfield_units(value); const key = String(unitOffset); const unit = Number(units[key] ?? 0); const modulus = 2 ** Math.max(width, 0); const shifted = Math.floor(unit / (2 ** Math.max(bitOffset, 0))); const next = modulus <= 0 ? 0 : shifted % modulus; value[field] = next; return next; }");
        self.writeln("function __kain_bitfield_set(value: any, field: string, unitOffset: number, bitOffset: number, width: number, next: number): number { if (value == null) { return next; } const units = __kain_bitfield_units(value); const key = String(unitOffset); const unit = Number(units[key] ?? 0); const shift = 2 ** Math.max(bitOffset, 0); const modulus = 2 ** Math.max(width, 0); const normalized = modulus <= 0 ? 0 : ((Number(next) % modulus) + modulus) % modulus; const current = modulus <= 0 ? 0 : Math.floor(unit / shift) % modulus; units[key] = unit + ((normalized - current) * shift); value[field] = normalized; return normalized; }");
