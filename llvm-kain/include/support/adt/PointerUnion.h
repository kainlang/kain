...
struct FixedTagHelper {
  template <size_t... Is>
  static constexpr std::array<TagEntry, sizeof...(PTs)>
  fill(std::index_sequence<Is...>, uintptr_t TagMask) {
    return std::array<TagEntry, sizeof...(PTs)>{{TagEntry{uintptr_t(Is), TagMask}...}};
  }

  static constexpr std::array<TagEntry, sizeof...(PTs)> compute() {
    constexpr size_t N = sizeof...(PTs);
    constexpr uintptr_t TagMask = (uintptr_t(1) << bitsRequired(N)) - 1;
    return fill(std::make_index_sequence<N>{}, TagMask);
  }
};

template <typename... PTs>
constexpr std::array<TagEntry, sizeof...(PTs)> computeFixedTags() {
  return FixedTagHelper<PTs...>::compute();
}
...
class PointerUnion
  ...
  static constexpr bool isNullVariableImpl(uintptr_t V,
    ...
    const TagTable Table = getTagTable();
  ...
  template <typename T> static uintptr_t encode(T V) {
    const TagTable Table = getTagTable();
    const int Shift = tagShift();
    const size_t Idx = FirstIndexOfType<T, PTs...>::value;
    assert(Table[0].Value == 0 &&
...
struct CastInfo<To, PointerUnion<PTs...>>
  ...
  static inline bool isPossible(From &F) {
    const std::array<pointer_union_detail::TagEntry, sizeof...(PTs)> Table =
        From::getTagTable();
    const int Shift = From::tagShift();
    const size_t Idx = FirstIndexOfType<To, PTs...>::value;
    auto V = reinterpret_cast<uintptr_t>(F.getOpaqueValue());
    const uintptr_t TagMask = Table[Idx].Mask << Shift;
    const uintptr_t TagValue = Table[Idx].Value << Shift;
  ...
  static To doCast(From &F) {
    ...
    const int Shift = From::tagShift();
    const size_t Idx = FirstIndexOfType<To, PTs...>::value;
    const uintptr_t PtrMask = ~(uintptr_t(Table[Idx].Mask) << Shift);
