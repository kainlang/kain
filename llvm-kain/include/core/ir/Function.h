...
#include "support/adt/ilist_node.h"
...
#include "core/ir/OperandTraits.h"
#include "core/ir/SymbolTableListTraits.h"
...
struct DenormalMode;
class DISubprogram;
// LibFunc is defined in TargetLibraryInfo.inc (GET_TARGET_LIBRARY_INFO_ENUM).
// Forward declaration omitted to avoid opaque enum type identity issues.
class LLVMContext;
class Module;
class raw_ostream;
...
private:
  ...
  static constexpr unsigned UnknownLibFunc = (unsigned)-1;
  ...
  mutable LibFunc LibFuncCache = static_cast<LibFunc>(UnknownLibFunc);
