...
#include "core/profiledata/MemProfData.inc"
...
#include "core/profiledata/MIBEntryDef.inc"
#undef MIBEntryDef
  Size
};
...
// Forward decl — MemInfoBlock defined in llvm/ProfileData/MemProfData.inc
// which is not available in the stripped Kain build.
struct MemInfoBlock;
...
#undef MIBEntryDef
  ...
  // Access histogram (not in MIBEntryDef — separate array)
  uint32_t AccessHistogramSize = 0;
  uint64_t AccessHistogram[1] = {};
};
...
struct IndexedMemProfRecord {
  ...
  // profiling data.
  llvm::SmallVector<IndexedAllocationInfo, 0> AllocSites;
  ...
  llvm::SmallVector<IndexedCallSiteInfo, 0> CallSites;
...
struct MemProfRecord {
  ...
  llvm::SmallVector<AllocationInfo, 0> AllocSites;
  // Same as IndexedMemProfRecord::CallSites with frame contents inline.
  llvm::SmallVector<CallSiteInfo, 0> CallSites;
