...
/// llvm-kain: replaced LLVM libc math with standard <cmath>
#include <cfenv>
#include <cmath>
...
std::optional<APFloat> exp(const APFloat &x, RoundingMode rounding_mode,
  ...
  if (rounding_mode == APFloatBase::rmNearestTiesToEven) {
    if (APFloat::SemanticsToEnum(x.getSemantics()) ==
      ...
      float result = std::expf(x_val);
      if (std::fetestexcept(FE_INVALID)) exc |= FE_INVALID;
      if (std::fetestexcept(FE_DIVBYZERO)) exc |= FE_DIVBYZERO;
      if (std::fetestexcept(FE_OVERFLOW)) exc |= FE_OVERFLOW;
      if (std::fetestexcept(FE_UNDERFLOW)) exc |= FE_UNDERFLOW;
      if (std::fetestexcept(FE_INEXACT)) exc |= FE_INEXACT;
      if (status) {
