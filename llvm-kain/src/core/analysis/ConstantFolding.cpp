...
#include "core/ir/ConstantFold.h"
...
static Constant *ConstantFoldScalarCall1(StringRef Name,
  ...
  if (!isa<ConstantFP>(Operands[0]))
    return nullptr;
  APFloat U = cast<ConstantFP>(Operands[0])->getValueAPF();
  ...
  // f2i conversions removed (target-specific intrinsics stripped)
  return nullptr;
    ...
    const APFloat &APF = cast<ConstantFP>(Operands[0])->getValueAPF();
