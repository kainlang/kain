...
#if !defined(_WIN32)
#error "zender_vulkan currently targets the Win32 Vulkan surface path."
#endif
...
#define KQV_MAX_GLB_BYTES (8u * 1024u * 1024u)
#define KQV_MAX_GLB_JSON_BYTES (2u * 1024u * 1024u)
...
static int64_t g_glb_byte_len = 0;
static int32_t g_glb_version = 0;
static int32_t g_glb_json_chunk_len = 0;
static char g_glb_json_text[KQV_MAX_GLB_JSON_BYTES];
...
static void kqv_glb_reset(void) {
    g_glb_byte_len = 0;
    g_glb_version = 0;
    g_glb_json_chunk_len = 0;
    g_glb_json_text[0] = '\0';
}

static uint32_t kqv_read_u32_le(const uint8_t* bytes) {
    return
        (uint32_t)bytes[0] |
        ((uint32_t)bytes[1] << 8u) |
...
static int32_t kqv_probe_glb_file(const char* path) {
    FILE* file;
    long end;
    uint8_t* bytes = NULL;
    size_t read_count;
    ...
    kqv_glb_reset();
    if (!path || !path[0]) {
        return kqv_fail_text("glb-probe", "missing glb path");
    }

    file = fopen(path, "rb");
    if (!file) {
        return kqv_fail_text("glb-probe", path);
    }
    if (fseek(file, 0, SEEK_END) != 0) {
        fclose(file);
        return kqv_fail_text("glb-probe", "fseek failed");
    }
    end = ftell(file);
    if (end < 20 || (unsigned long)end > KQV_MAX_GLB_BYTES) {
        fclose(file);
        return kqv_fail_text("glb-probe", "glb size is invalid");
    }
    if (fseek(file, 0, SEEK_SET) != 0) {
        fclose(file);
        return kqv_fail_text("glb-probe", "rewind failed");
    }

    bytes = (uint8_t*)malloc((size_t)end);
    if (!bytes) {
        fclose(file);
        return kqv_fail_text("glb-probe", "glb allocation failed");
    }
    read_count = fread(bytes, 1u, (size_t)end, file);
    fclose(file);
    if (read_count != (size_t)end) {
        free(bytes);
        return kqv_fail_text("glb-probe", "glb read was truncated");
    }

    if (bytes[0] != 'g' || bytes[1] != 'l' || bytes[2] != 'T' || bytes[3] != 'F') {
        free(bytes);
        return kqv_fail_text("glb-probe", "glb header magic was invalid");
    }

    version = kqv_read_u32_le(bytes + 4);
    total_len = kqv_read_u32_le(bytes + 8);
    json_chunk_len = kqv_read_u32_le(bytes + 12);
    chunk_type = kqv_read_u32_le(bytes + 16);

    if (total_len != (uint32_t)end) {
        free(bytes);
        return kqv_fail_text("glb-probe", "glb total length did not match file size");
    }
    if (chunk_type != 0x4E4F534Au) {
        free(bytes);
        return kqv_fail_text("glb-probe", "glb first chunk was not JSON");
    }
    if (json_chunk_len == 0u || json_chunk_len + 20u > total_len) {
        free(bytes);
        return kqv_fail_text("glb-probe", "glb json chunk length was invalid");
    }
    if (json_chunk_len >= KQV_MAX_GLB_JSON_BYTES) {
        free(bytes);
        return kqv_fail_text("glb-probe", "glb json chunk exceeded bridge buffer");
    }

    memcpy(g_glb_json_text, bytes + 20u, json_chunk_len);
    g_glb_json_text[json_chunk_len] = '\0';
    g_glb_byte_len = (int64_t)end;
    g_glb_version = (int32_t)version;
    g_glb_json_chunk_len = (int32_t)json_chunk_len;
    free(bytes);
    kqv_ok();
    return 1;
}
...
static int32_t kqv_create_window(KqvApp* app, const char* title) {
    ...
    const char* class_name = "ZenderVulkanWindow";
    ...
    app->hwnd = CreateWindowExA(
        0,
        class_name,
        title && title[0] ? title : "Zender // Natural Vulkan Window",
        style,
        CW_USEDEFAULT,
        CW_USEDEFAULT,
...
static int32_t kqv_create_instance(KqvApp* app) {
    ...
    application_info.pApplicationName = "Zender Natural Vulkan";
    application_info.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
    application_info.pEngineName = "Zender";
...
static int32_t kqv_record_command_buffer(KqvApp* app, uint32_t image_index, int32_t frame_index) {
    ...
    push.orbit_speed = app->orbit_speed;
    push.chaos = app->chaos;
    push.mode = app->mode;
    push.sphere_instances = app->sphere_instances;
    push.ring_resolution = app->ring_resolution;
    push.shell_resolution = app->shell_resolution;
...
const char* zv_backend_name(void) {
    return "zender-vulkan-win32";
}

const char* zv_last_error(void) {
    return g_last_error;
}

int64_t zv_frames_presented(void) {
    return g_frames_presented;
}

int64_t zv_particles_drawn(void) {
    return g_particles_drawn;
}

int32_t zv_glb_probe_file(const char* path) {
    return kqv_probe_glb_file(path);
}

int64_t zv_glb_byte_len(void) {
    return g_glb_byte_len;
}

int32_t zv_glb_version(void) {
    return g_glb_version;
}

int32_t zv_glb_json_chunk_len(void) {
    return g_glb_json_chunk_len;
}

const char* zv_glb_json_text(void) {
    return g_glb_json_text;
}

int32_t zv_run_window(
    const char* title,
    int32_t width,
    int32_t height,
    ...
    app.mode = mode;
    app.sphere_instances = sphere_instances > 0 ? sphere_instances : 1;
    app.ring_resolution = ring_resolution > 0 ? ring_resolution : 64;
    app.shell_resolution = shell_resolution > 0 ? shell_resolution : 32;
    app.orbit_speed = orbit_speed > 0.05f ? orbit_speed : 0.84f;
    app.chaos = chaos >= 0.0f ? chaos : 0.0f;
...
int32_t zv_write_report(const char* path) {
    FILE* file;
    if (!path || !path[0]) {
    ...
    fprintf(file, "Zender Vulkan report\n");
