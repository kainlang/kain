...
Script-only `build.kn` workspace smoke for Kain's evidence DAG.

This lab exists to prove that a root `build.kn` can:
...
```powershell
python blades/build-kn-system-smoke/scripts/verify_build_kn_system_smoke.py
```
