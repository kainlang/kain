...
Kain is the first hand-owned language core in this tree. It is the pre-UE5
...
- If a topic belongs to UE5, shader codegen, or pipeline orchestration, it does
  not belong in Kain wave 1.

## Donor Matrix

| Surface | Primary donor | Secondary donor | Kain ownership |
| --- | --- | --- | --- |
...
| Comptime and runtime substrate | `src/.legacy/src/korec.kn` (historic donor name), `stdlib.kn` | `crates/kain-core/src` | Owned here |
...
Wave 1 is the smallest useful owned Kain core:

- `ast.kn`
- `language_features.kn`
- later: lexer, parser, span, diagnostic, error, effects, types, comptime,
  runtime, stdlib, low-level memory, low-level ABI, and `kainc`
