...
- `config/*.json` is the data-driven source of truth for shell surfaces, workspace modes, imported runtime packs, tool rails, command routing, resource kinds, and Fabric intent presentation.
- `session/*.kn` is the live state core that defines the canonical session document, reducer layer, derived state, registries, and intent planner between shell interactions and Fabric execution.
- `state/runtime_snapshot.json` is the projected native-runtime snapshot materialized from app config plus the latest Fabric report.


## Key Folders
- `config/` contains the app manifest, shell registries, command registry, resource kind registry, and Fabric intent registry.
- `session/` contains the canonical app state schema, reducers, derived state, and registries.
- `fabric/intents/` contains reusable Fabric subgraphs for interactive app work.
- `state/` contains the materialized runtime snapshot consumed by the native executable.
...
- `generated/` and `state/runtime_snapshot.json` are materialized outputs and can be regenerated.
