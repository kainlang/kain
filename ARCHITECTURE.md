...
- On Windows GNU builds, LLVM alone is not enough. The `x86_64-pc-windows-gnu` target also needs a MinGW GCC runtime bundle with `libgcc`, `libgcc_eh`, and a working `ar.exe`. On this VPS the successful path came from WinLibs, with `AR=ar.exe` rather than a target-prefixed archiver name.
