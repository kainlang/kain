...
- [blades/opengl](blades/opengl): raw reusable Win32/WGL compatibility blade package. Its live bridge now lives in `native/opengl_bridge.c`, while the deleted runtime-owned OpenGL lane is preserved only as salvage material under `reference/runtime_legacy/`. Validate with `blades/opengl/run.ps1`, `.kain/native/opengl_bridge.obj`, and `.kain/run/opengl_report.txt`.
