...
- `stdlib/native` is loaded for `CompileTarget::Llvm` and `CompileTarget::C`. It exposes runtime init/shutdown, actor, entangle, filesystem, input, process, diagnostics, time, collections, result/status, intent, raw native UI, and raw native graphics helpers as Kain-callable declarations/wrappers.
