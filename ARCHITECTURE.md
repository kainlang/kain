...
- `kain build ... -t llvm` now reuses native runtime objects incrementally under `<output>/.kain-runtime/<runtime-name>/` using per-object depfiles and compile fingerprints. If a warm LLVM/native build starts recompiling the whole runtime again, suspect a changed output directory, changed compile flags, missing depfiles/fingerprints, or a newer source/header dependency before blaming LLVM itself.
