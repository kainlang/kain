...
LLVM ownership lowering also has a new compiler-owned `EphemeralLocal` provenance lane for fresh single-cell helper allocations that never escape their local ownership trace. When `crates/kain-sys-codegen/src/codegen_llvm/mod.rs` can prove the cell is only used through a balanced local `collapse` / `observe` / `decay` sequence, it materializes stack-backed byte storage, erases `__kain_alloc` plus ownership runtime calls, and lowers `mem_load` / `mem_store` directly against that storage. Treat this as a strict proof-backed lane, not a heuristic: if the trace stops being fresh, single-cell, or non-escaping, the lowering must fall back to the helper-owned or imported runtime paths.
