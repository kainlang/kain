...
- [blades/kaintana](blades/kaintana): blade-owned Kain UI framework package. `src/kaintana.kn` exports window specs, theme packs, rect/split/row/column helpers, retained/immediate authoring helpers, passive session/hot-reload wrappers over `stdlib/native/ui.kn`, and backend-neutral host helpers that can route to the blade-local desktop host or the reusable `vulkain` lane without widening `runtime/native`. `native/kaintana_desktop_bridge.c` is the v1 desktop compatibility host: Win32/GDI only, rect/text command-buffer based, and screenshot/report capable. Validate with `blades/kaintana/run.ps1 -NoRun` plus the Kaintana Z3 reports under `z3/reports/*kaintana*.json`.
- [blades/kaintana-test](blades/kaintana-test): acceptance blade for the Kaintana framework. `src/main.kn` is the full desktop workbench proof, `entrypoints/vulkan.kn` is the foreign-presenter Vulkan proof over the same Kaintana surface, `run.ps1` selects `desktop`, `vulkan`, or `all`, and `.kain/run/` holds the durable artifacts: `kaintana_test_desktop.bmp`, `kaintana_test_desktop_frame.txt`, `kaintana_test_desktop_host.txt`, `kaintana_test_vulkan_frame.txt`, and `kaintana_test_vulkan_host.txt`. Imported local Kain modules that contain `world` / `entangle` are supported again in this lane after the realtime/native UI staging fix in `crates/kain-driver`; the remaining durable caveat is that consumer blades must still repeat their local `[[c_ffi.libraries]]` entries.
...
- Kaintana's remaining durable UI-native gotcha is the manifest one: `[c_ffi]` bindings are still local to the consuming blade manifest, so if a library blade such as `blades/kaintana` wraps a native bridge, consumer blades such as `blades/kaintana-test` must still declare matching `[[c_ffi.libraries]]` entries locally for `use c::...` imports to resolve. The earlier imported-`world` / `entangle` double-staging failure was fixed in `crates/kain-driver`: realtime/native UI staging must build UI from target-prepared source, not the flattened frontend import bundle, or compiler-owned intent items will be replayed twice.
