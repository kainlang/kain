...
- Kaintana adds two more durable UI-native gotchas. Imported local Kain modules that contain `world` / `entangle` currently double-stage those bindings during native LLVM artifact staging, which surfaces as `entangle endpoint '...' participates in more than one binding`; until that handoff is repaired, keep Kaintana showcase entrypoints self-contained and reserve imported modules for plain helper code. Also, `[c_ffi]` bindings are still local to the consuming blade manifest: if a library blade such as `blades/kaintana` wraps a native bridge, consumer blades such as `blades/kaintana-test` must still declare matching `[[c_ffi.libraries]]` entries locally for `use c::...` imports to resolve.
