...
- [native UI system C ABI](/M:/Code/Kain/runtime/native/include/kain_native_ui_system.h): raw single-file LLVM UI substrate exposed through [stdlib/native/ui.kn](/M:/Code/Kain/stdlib/native/ui.kn). It gives Kain source enough low-level power to build its own UI system: sessions, windows, host frame presentation, arbitrary node kind strings, stable node keys for reload, parent/rect/text/style/flag mutation, accessibility metadata, font/texture/canvas/shader resource handles, generic resource byte upload, text measurement, clipboard, IME, drag/drop, menus, dialogs, focus, hit testing, events, dirty counts, hot reload generations, and draw commands. `draw_text` now carries an explicit font handle, and the first live presenter is a non-blocking `win32-gl` adapter behind `host_attach/pump/present`; `software` remains the headless metadata backend. Keep it generic; do not add baked component kinds or a catalog to this layer.
