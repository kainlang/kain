...
- [smoketest/native-ui](/M:/Code/Kain/smoketest/native-ui): focused raw native UI proof lane. `pilot/` is the first isolated LLVM smoke for Kain-authored UI systems over `stdlib/native/ui.kn`; it now compiles a one-file Kain UI system into a real Win32/GL `.exe`, captures a BMP screenshot, and exits cleanly. Do not route this lane through the older smoketest delegate pipelines.
...
- [blades/kain-example](blades/kain-example): the canonical native LLVM proving ground blade. `src/main.kn` remains the one-file semantic stress test for low-level memory helpers, `Option`/`Result`/`Future`, compiler-owned intents, worlds/entangle, actors, filesystem, input, networking, process, native stdlib UI, native graphics, shader declarations, heap-health checkpoints, and live native LLVM lowering for enum `match`, numeric `for` over `range`, `vec!`, `format!`, and `println`. `src/ui.kn` is now the interactive runtime workbench: a Win32/GL DCC-style shell with authored tabs for actors, 3D, networking, entangle, and filesystem-backed labs. `run-ui.ps1` is the operator proof loop for that workbench and should be the first stop when an agent needs to prove the native UI/runtime lane with a real screenshot-producing `.exe`.
