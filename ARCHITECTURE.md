...
- [crates/kain-core/z3](crates/kain-core/z3): durable Z3 proof pack for compiler/frontend invariants in `kain-core`. It currently covers low-level memory layout arithmetic, signed-literal conversion bounds, diagnostics span/line-end math, parser slice/index guards, and lanes `memory`, `diagnostics`, `literals`, `parser`, `full`, plus workspace `smoke`.
...
- [blades/kaintana](blades/kaintana): blade-owned Kain UI framework core. `src/kaintana.kn` exports window specs, theme packs, rect/split/row/column helpers, retained/immediate authoring helpers, passive session/hot-reload wrappers over `stdlib/native/ui.kn`, and the default desktop host helpers over the blade-local compatibility presenter. `native/kaintana_desktop_bridge.c` is the v1 desktop host: Win32/GDI only, rect/text command-buffer based, and screenshot/report capable. The core package no longer links the Vulkan bridge by default. Validate with `blades/kaintana/run.ps1 -NoRun` plus the Kaintana Z3 reports under `z3/reports/*kaintana*.json`.
...
- [blades/kaintana-test](blades/kaintana-test): desktop acceptance blade for the Kaintana framework. `src/main.kn` is the full solar-broadcast workbench proof, `run.ps1` now builds only `kaintana-test.exe`, and `.kain/run/` holds the durable desktop artifacts: `kaintana_test_desktop.bmp`, `kaintana_test_desktop_frame.txt`, and `kaintana_test_desktop_host.txt`. This blade should stay free of `vulkain_bridge.dll` imports.
