...
`kain equip <blade>` is the human-facing resolver. `kain blades list`, `kain blades graph`, and `kain blades check` are the inspection and validation surface. `kain blades build .` and the standalone `blade build .` command are the build surface. Runtime code should call the `blade` crate or `kain-build` instead of re-scanning folders directly; future remote update/install work should extend this crate boundary rather than inventing a second registry path.
