...
- [crates/gpu/z3](crates/gpu/z3): durable Z3 proof pack for the live SPIR-V backend in `crates/gpu/src/codegen_spirv.rs`. It currently covers Vulkan/std430 wrapper layout invariants, vector-constructor component-index bounds, hoisted-local slot removal safety, and compute local-size slot mapping through `kain.gpu.proofs`, with focused `layout`, `constructors`, `control`, `full`, and workspace `smoke` lanes.
