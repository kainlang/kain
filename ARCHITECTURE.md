...
- Graphics vendor doctrine is split by role. `bgfx` is the first practical backend candidate, `bx`/`bimg` are support infrastructure for that lane, `filament-core` is a higher-level premium renderer experiment, `diligentcore` is the future explicit render-graph / compute architecture lane, and `the-forge` is a staged low-level backend identity for a future bridge-first renderer substrate. Do not wire all of them into the same `gfx.viewport` path as peers.
- The first durable smoke for this renderer seam now lives under `smoketest/3D/material_atrium_showcase`. It packages through `kain build native-ui`, binds the primitive-backed `material_atrium` scene from `kain-3D`, and advertises the current backend roadmap in a Qt-native shell instead of a legacy egui host.
- The Qt launcher for that smoke now packages `material_atrium_visual_example.png` as a host sidecar and forwards it through `KAIN_UI_NATIVE_QT_VIEWPORT_IMAGE_PATH`, which keeps the atrium preview image visible in the generated native shell instead of relying on a stale manual edit.
- That smoke now also has a deterministic artifact generator: `cargo run -p kain-3d --bin material_atrium_smoke` or the wrapper scripts under `smoketest/3D/material_atrium_showcase/` regenerate `material_atrium_visual_example.png` and a JSON matrix report from the real `material_atrium` scene. The image is a software compatibility preview with honest backend labels, not a hand-drawn concept card, and the scene now uses the authored primitive library for the atrium massing.
