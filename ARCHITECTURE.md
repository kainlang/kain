...
- Graphics vendor doctrine is split by role. `bgfx` is the first practical backend candidate, `bx`/`bimg` are support infrastructure for that lane, `filament-core` is a higher-level premium renderer experiment, and `diligentcore` is the future explicit render-graph / compute architecture lane. Do not wire all three renderers into the same `gfx.viewport` path as peers.
