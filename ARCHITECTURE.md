...
Stdlib authoring rule: prefer public root-domain imports such as `std.actor`, `std.fs`, `std.http`, `std.process`, `std.graphics`, and `std.ui` in new Kain source. `stdlib/native` remains the LLVM/direct-C profile and ABI-backed implementation layer, but all current native domains have root mirrors and clean aliases that hide the older `native_*` wrapper prefix unless a proof is intentionally testing that compatibility seam.
...
`crates/kain-driver`, `crates/kain-omni`, `crates/kain-selfhost`, `crates/kain-build`, `crates/kain-run`, and importer crates turn Kain semantic truth into emitted bundles, packaged artifacts, immediate execution plans, imported surfaces, and multi-runtime workflows.
...
`crates/kain-ui`, `crates/kain-ui-native`, and `crates/kain-3D` are the semantic UI and accelerated native presentation stack. The low-level single-file LLVM UI lane lives at the C ABI floor in `runtime/native/include/kain_native_ui_system.h` and `runtime/native/src/ui/kain_native_ui_system.c`: it exposes sessions, host frame presentation metadata, arbitrary authored node kinds, stable node keys, rects, styles, accessibility labels, text/font/resource handles, clipboard/IME/drag/menu/dialog state, events, hit testing, focus, dirty tracking, hot reload generations, and draw-command buffers without a host-owned component catalog. The compiled-bundle/runtime reload lane now sits beside it in `runtime/native/include/kain_ui_runtime.h`, `runtime/native/include/kain_ui_hot_reload.h`, `runtime/native/src/ui/kain_ui_runtime.c`, and `runtime/native/src/ui/kain_ui_hot_reload.c`: bundle reload/state transfer is cross-platform by API, file-watch live reload works everywhere the runtime can stat the bundle path, and the shared-memory control ring is hidden behind platform-specific backends instead of being baked into the UI architecture. The low-level native graphics lane lives at `runtime/native/include/kain_native_graphics_system.h` and `runtime/native/src/core/kain_native_graphics_system.c`: it exposes sessions, backend targets, SPIR-V module registration, authored buffers, meshes, pipelines, frame submission, draw-command inspection, and diagnostics without a host-owned scene or primitive catalog.
...
- `runtime/native` owns the stable ABI floor, startup, service contracts, and low-level host/runtime substrate. Native startup now lazily boots the pooled actor scheduler on first actor use instead of eagerly paying that cost for every pure compute executable.
...
- [blades/kaintana](blades/kaintana): blade-owned Kain UI framework core. `src/kaintana.kn` now exports window specs, theme packs including `oxide-dcc`, rect/split/row/column helpers plus top/bottom/grid helpers, retained/immediate authoring helpers, toolbar/button/slider/chart/primitive helpers, action/axis wrappers over `stdlib/native/input.kn`, focus/clipboard/IME/menu/dialog/popover wrappers over `stdlib/native/ui.kn`, snapshot/input-trace harness helpers, passive session/hot-reload wrappers, and the default desktop host helpers over the blade-local compatibility presenter. `native/kaintana_desktop_bridge.c` is the current desktop host: Win32/GDI only, but now client-size scaling, sized text fallback, double-buffered paint, and screenshot/report capable so long-lived proof windows do not repaint-spam or jitter. The core package no longer links the Vulkan bridge by default. Validate with `blades/kaintana/run.ps1 -NoRun` plus the Kaintana Z3 reports under `z3/reports/*kaintana*.json`.
...
- [benchmark](benchmark): multi-language benchmark lane for Kain LLVM, Rust LLVM, C++ on Clang, Erlang/OTP, and optional JavaScript/Node plus Python/CPython rows when a case declares them. `benchmarks.json` owns the case manifest, maturity labels, per-language source paths, language notes, and fairness notes; `cases/<case>/main.kn`, `main.rs`, `main.cpp`, `main.js`, and `main.py` are dependency-free case sources for normal rows, while manifest-declared language subsets, per-case Cargo manifests, Erlang OTP modules, and runner-built support artifacts cover focused Tokio/Rayon, actor mailbox, and shared-library FFI stress comparisons. `run.py` builds/checks selected languages, compiles Erlang via `erlc`, stages support DLLs beside the executables when needed, times executable/interpreter samples, writes `out/reports/latest.llm.md` plus JSON, and removes stale HTML output. The runner prefers a release-built `kain.exe`, defaults the C++ lane to the repo-bundled `toolchain/llvm/bin/clang++.exe`, resolves Erlang from the official Windows OTP `bin` directory before PATH shims, pins Kain benchmark links to `runtime/native_core_runtime.toml`, injects `KAIN_NATIVE_PROFILE=benchmark-release` with `opt-level=3`, `target-cpu=native`, and no debug info into Kain native builds, and records that tuning provenance in the report. Kain rows that rely on a case-local `KAIN.toml` plus `use c::...` must compile from the case directory so nearest-manifest resolution works; `run.py` now does that automatically for the shared-library stress lane. `benchmark/ffi_boundary/` is the dedicated ABI-tax/native-neighborhood probe for pure Kain LLVM, direct-object/shared-library C FFI, interpreter/live-bridge comparisons, and Zig `ReleaseFast` twins; it writes `ffi_boundary_latest.llm.md` plus JSON siblings under `benchmark/out/reports/` and should be used when the question is boundary overhead or whether Kain LLVM is in Zig/C territory rather than language-to-language fairness. `benchmark/blades/kain-benchmark` is the compact native UI console for running quick/full benchmark passes and previewing the latest LLM report; the built operator executable lives at `benchmark/kain-benchmark.exe`. `benchmark/assesments/` is the durable markdown research surface for full-suite summaries and root-cause analysis, while `benchmark/out/reports/` stays the generated report surface. The current cases now cover contention/collapse, entangle mirroring, converge dispatch, direct ownership memory, branch dispatch, call chains, memory streaming, allocation churn, top-level consts, recursion, string ops, array scanning, aggregate fields, tagged Option/Result values, ready-future async overhead, native TCP loopback versus Tokio, Rayon parallel reduction, SIMD-lane pressure, native map lookup, manual JSON roundtrip, filesystem streaming, process/stdout loops, local HTTP request handling, actor ask/reply fanout versus Erlang, Unicode-heavy substring search, large-object allocator churn, raw graphics submission, and shared-library FFI call stress. The sharp live suite findings now are that Kain still has a real synchronous-local-HTTP gap against Tokio even after the request-slot leak fix, while the actor ask/reply row still needs one unmeasured warmup ask per worker to measure steady-state behavior cleanly.
...
- [stdlib](/M:/Code/Kain/stdlib): runtime support and standard library data. The root profile holds general helpers including `input.kn`; `stdlib/native` is the LLVM/direct-C native profile for actor, entangle, filesystem, input, net, process, runtime, diagnostics, time, intent, raw native UI, and raw native graphics helpers; `stdlib/c` is the direct C bridge overlay; and the root `gen_server.kn` helper layers `gen_server_start`, `gen_server_call`, `gen_server_cast`, and `gen_server_info` on top of raw actor primitives. `start_link` is currently naming-only until real link semantics land
...
- [kain-input](/M:/Code/Kain/crates/kain-input): portable input crate for source provenance, typed events, frame/action reduction, text commits, axis values, binding maps, first-class `agent.intent` events, and deterministic traces/replay. Interpreter builtins in `kain-core`, `stdlib/input.kn`, `stdlib/native/input.kn`, and the native C kernel consume this shape.
- [kain-net](/M:/Code/Kain/crates/kain-net): portable networking contract crate for TCP endpoints, HTTP request/response specs, headers, server route specs, connection/listener/server/request/response handles, lifecycle state, and typed errors. Native Kain builds consume this through `stdlib/native/net.kn` plus the `kain_native_net_*`, `kain_native_tcp_*`, and `kain_native_http_*` C ABI.
- [kain-process](/M:/Code/Kain/crates/kain-process): portable child-process and PTY contract crate for executable path/argv, cwd/env overrides, stdio policy, process/PTY handles, lifecycle state, and captured output metadata. Native Kain builds consume this through `stdlib/native/process.kn` plus the `kain_native_process_*` C ABI.
...
- [native net system C ABI](/M:/Code/Kain/runtime/native/include/kain_native_net_system.h): raw LLVM/direct-C networking substrate exposed through [stdlib/native/net.kn](/M:/Code/Kain/stdlib/native/net.kn) and mirrored at the public `std.net`, `std.http`, `std.tls`, and `std.http2` entrypoints. It gives Kain source a handle-driven TCP and protocol-aware HTTP model: TCP connect/listen/accept/read/write, HTTP request/response handles, per-request protocol selection, response protocol inspection, local HTTP server listen/pump, actor route registration, request inspection, response writes, capability-state queries, local URL helpers, and status diagnostics. The current implementation lives in `runtime/native/src/core/kain_native_net_system.c`; HTTPS client requests are Windows-first through WinHTTP, HTTP/2 client intent currently negotiates only on the secure client path, and server TLS, WebSockets, and HTTP/3 are intentionally follow-up layers.
- [native process system C ABI](/M:/Code/Kain/runtime/native/include/kain_native_process_system.h): raw LLVM/direct-C child-process substrate exposed through [stdlib/native/process.kn](/M:/Code/Kain/stdlib/native/process.kn). It gives Kain source a handle-driven process model: process specs, argv/cwd/env overrides, inherit/pipe/null stdio modes, child spawn, PTY spawn, poll/wait/exit status, stdin writes, stdout/stderr capture, PTY output capture, resize, and status/error diagnostics. The current real implementation is Windows-first and lives in `runtime/native/src/core/kain_native_process_system.c`; non-Windows hosts report explicit unsupported status instead of pretending parity exists.
...
- `stdlib/native` is loaded for `CompileTarget::Llvm` and `CompileTarget::C`. It exposes runtime init/shutdown, actor, entangle, filesystem, input, net, http, tls, http2, process, diagnostics, time, collections, result/status, intent, raw native UI, and raw native graphics helpers as Kain-callable declarations/wrappers; the root stdlib mirrors the networking domains at `std.net`, `std.http`, `std.tls`, and `std.http2`.
...
- `runtime/conformance/process_runtime/run_tests.sh` is the direct C conformance lane for `kain_native_process_*`, covering argv/cwd/env wiring, stdio pipes, exit/wait, PTY attach/capture, resize, and honest unsupported-platform behavior.
- `runtime/conformance/net_runtime/run_tests.sh` is the direct C conformance lane for `kain_native_net_*`, covering TCP loopback, capability/protocol queries, HTTP server request/response, actor route registration, request-slot reuse beyond the old capacity wall, and HTTP client roundtrip.

### Networking Flow

`Kain std.net/std.http/std.tls/std.http2 -> stdlib/native/net/http/tls/http2 wrappers -> kain-net portable contracts -> runtime/native C net ABI -> raw sockets or platform HTTPS backend -> LLVM/direct-C executables`
...
`Raw host events -> target adapter -> kain-input event/session model -> stdlib/input.kn or stdlib/native/input.kn -> Kain source queries frames/actions/axes/text/trace`
...
Interpreter execution registers `kain_input_*` bridge builtins in `crates/kain-core/src/runtime.rs` and root `stdlib/input.kn` exposes the public `input_*` helpers. LLVM/direct-C native builds load `stdlib/native/input.kn`, which wraps `runtime/native/include/kain_native_input_system.h` and `runtime/native/src/core/kain_native_input_system.c`. Keep adapters thin: Win32, web DOM, UE5 Enhanced Input, UI runtime, CLI stdin, and agent tooling should translate into this model instead of defining separate app-facing input systems.
...
- Native LLVM ask/reply now has a dedicated reply-port fast path instead of routing the return leg through the generic mailbox queue. `ask` / `ask_timeout` allocate a TLS-cached reply port, handlers sending `reply_to.Reply(...)` lower to `kain_actor_reply_port_send`, typed waits use `kain_actor_reply_port_wait(...)`, and scalar `i64` waits keep `kain_actor_reply_port_wait_i64(...)`. If actor roundtrips regress, inspect the codegen/runtime seam rather than only the benchmark case.
...
- `runtime/native` owns only the passive UI/runtime substrate: `ui.bundle`, `ui.component`, `kain_native_ui_system`, hot reload, and host attachment seams. It does not own a baked widget catalog, OpenGL presenter, or compile-backed UI vendor trees.
...
- The canonical repo MCP lane is `blades/kain-mcp`, not `tools/kain-flight-control`. If an agent starts inventing `target/.../kain.exe` or stale Cargo-path invocations, point it back to `kain run blades/kain-mcp`; keep `scripts/python/launch_kain_mcp.py` for managed-sync fallback/debugging instead of as the default Codex boot path.
...
- The root `mcp.json` and repo `codex.config.toml` templates must stay repo-relative for this checkout: use `command = "kain"`, `args = ["run", "blades/kain-mcp"]`, and `cwd = "."`. Codex in this lane treats `${KAIN_REPO_ROOT}` literally instead of interpolating it, so placeholder-based repo configs can silently override the good global MCP block with a broken local one.
...
- Runtime conformance harnesses that compile `kain_runtime_services.c` or `kain_runtime_contract.c` in isolation must also compile `runtime/native/src/core/kain_runtime_core.c`, `runtime/native/src/core/kain_runtime_actor.c`, `runtime/native/src/core/kain_native_net_system.c`, and `runtime/native/src/core/kain_native_process_system.c`; the live catalog now binds the owned net/process function tables directly.
