...
- [blades/kain-labs](blades/kain-labs): native lab workspace for reference-driven experiments. Its first app, KQuantum, recreates `reference/KQuantum.tsx` as a Kain-authored native GPU particle/fluid lab with `src/main.kn`, dense native UI modules, `src/kernels.kn` SPIR-V compute kernels, data-driven mode config, and a blade-root `kain-labs.exe` proof artifact. Validate with the Bazel-backed blade compile script, `kain gpu-artifacts ...` plus `spirv-val`, Z3 index proofs for particle/fluid dispatch bounds, and screenshot/report artifacts under `blades/kain-labs/.kain/`.
- [benchmark](benchmark): paired Kain LLVM vs Rust LLVM benchmark lane. `benchmarks.json` owns the case manifest, maturity labels, and fairness notes; `cases/<case>/main.kn` and `main.rs` are dependency-free paired sources; `run.py` builds both sides, times executable samples, and writes `out/reports/latest.html` plus JSON. The runner now prefers a release-built `kain.exe`, injects `KAIN_NATIVE_PROFILE=benchmark-release` with `opt-level=3`, `target-cpu=native`, and no debug info into Kain native builds, and records that tuning provenance in the report. The current cases target contention/collapse, entangle mirroring, converge dispatch, direct ownership memory, branch dispatch, call chains, memory streaming, allocation churn, top-level consts, recursion, string ops, array scanning, aggregate fields, and tagged Option/Result values.
