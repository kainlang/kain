...
- [benchmark](benchmark): multi-language benchmark lane for Kain LLVM, Rust LLVM, C++ on Clang, JavaScript on Node, and Python on CPython. `benchmarks.json` owns the case manifest, maturity labels, per-language source paths, language notes, and fairness notes; `cases/<case>/main.kn`, `main.rs`, `main.cpp`, `main.js`, and `main.py` are dependency-free case sources for normal cases, while manifest-declared language subsets plus per-case Cargo manifests cover focused Tokio/Rayon Kain/Rust comparisons; `run.py` builds/checks selected languages, times executable/interpreter samples, writes `out/reports/latest.llm.md` plus JSON, and removes stale HTML output. The runner prefers a release-built `kain.exe`, defaults the C++ lane to the repo-bundled `toolchain/llvm/bin/clang++.exe`, pins Kain benchmark links to `runtime/native_core_runtime.toml`, injects `KAIN_NATIVE_PROFILE=benchmark-release` with `opt-level=3`, `target-cpu=native`, and no debug info into Kain native builds, and records that tuning provenance in the report. `benchmark/ffi_boundary/` is the dedicated ABI-tax/native-neighborhood probe for pure Kain LLVM, direct-object/shared-library C FFI, interpreter/live-bridge comparisons, and Zig `ReleaseFast` twins; it writes `ffi_boundary_latest.llm.md` plus JSON siblings under `benchmark/out/reports/` and should be used when the question is boundary overhead or whether Kain LLVM is in Zig/C territory rather than language-to-language fairness. `benchmark/blades/kain-benchmark` is the compact native UI console for running quick/full benchmark passes and previewing the latest LLM report; the built operator executable lives at `benchmark/kain-benchmark.exe`. `benchmark/assesments/` is the durable markdown research surface for full-suite summaries and root-cause analysis, while `benchmark/out/reports/` stays the generated report surface. The current cases target contention/collapse, entangle mirroring, converge dispatch, direct ownership memory, branch dispatch, call chains, memory streaming, allocation churn, top-level consts, recursion, string ops, array scanning, aggregate fields, tagged Option/Result values, ready-future async overhead, native TCP loopback versus Tokio, and Rayon parallel-reduction pressure.
