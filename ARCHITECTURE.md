...
LLVM ownership semantics matter here: non-extern Kain callables currently treat parameter locals as owned values and release them on scope exit. `crates/kain-sys-codegen/src/codegen_llvm/mod.rs` therefore retains borrowed `String` arguments at non-extern callsites before calling stdlib/native wrappers or authored Kain functions. Do not remove that retain contract unless parameter ownership, wrapper lowering, and scope-exit cleanup are redesigned together; the filesystem/UI/native example lane will otherwise reintroduce heap corruption.
...
- If a native LLVM regression only appears when calling stdlib/native wrappers from authored Kain, inspect `crates/kain-sys-codegen/src/codegen_llvm/mod.rs` before blaming the C runtime. Non-extern callsites must retain borrowed `String` arguments because callee parameter locals are released on scope exit; removing that retain reintroduces filesystem-driven heap corruption.
