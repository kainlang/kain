...
- [blades/kain-example](blades/kain-example): the canonical single-file native LLVM proving ground. `src/main.kn` intentionally exercises broad authored Kain surface area in one place: low-level memory helpers, `Option`/`Result`/`Future`, compiler-owned intents, worlds/entangle, actors, filesystem, input, networking, process, native stdlib UI, native graphics, shader declarations, heap-health checkpoints, and now live native LLVM lowering for enum `match`, numeric `for` over `range`, `vec!`, `format!`, and `println`. Future agents should read this file early when they need a high-signal example of what current native Kain can actually do.
