...
- `crates/kain-net` owns portable networking contracts: TCP endpoints, HTTP request/response specs, headers, route specs, listener/server/client handles, lifecycle state, and typed network errors. Native adapters implement these semantics; actor and entangle integrations layer above the transport instead of redefining sockets.
...
- [native net system C ABI](/M:/Code/Kain/runtime/native/include/kain_native_net_system.h): raw LLVM/direct-C networking substrate exposed through [stdlib/native/net.kn](/M:/Code/Kain/stdlib/native/net.kn). It gives Kain source a handle-driven TCP and HTTP/1.1 model: TCP connect/listen/accept/read/write, HTTP request/response handles, HTTP client send, local HTTP server listen/pump, actor route registration, request inspection, response writes, local URL helpers, and status diagnostics. The current implementation lives in `runtime/native/src/core/kain_native_net_system.c`; HTTPS client requests are Windows-first through WinHTTP, while server TLS, WebSockets, HTTP/2, and HTTP/3 are intentionally out of v1.
...
- `stdlib/native` is loaded for `CompileTarget::Llvm` and `CompileTarget::C`. It exposes runtime init/shutdown, actor, entangle, filesystem, input, net, process, diagnostics, time, collections, result/status, intent, raw native UI, and raw native graphics helpers as Kain-callable declarations/wrappers.
...
- `runtime/fixtures/native_net_http/main.kn` is the focused LLVM fixture proving authored Kain TCP plus HTTP/1.1 server flows, actor route registration, request inspection, response writes, and native net status/reset behavior.
...
- `runtime/conformance/net_runtime/run_tests.sh` is the direct C conformance lane for `kain_native_net_*`, covering TCP loopback, HTTP server request/response, actor route registration, and HTTP client roundtrip.
