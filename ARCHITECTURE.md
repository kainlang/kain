...
- `crates/kain-ownership` owns the portable ownership-state memory model for future `collapse`, `observe`, and `decay` integration: region kinds, legal state transitions, conservative world/entangle policy, lowering hints, and the `memory.ownership` capability name.
...
- [crates/kain-ownership/z3](crates/kain-ownership/z3): durable Z3 proof pack for the ownership-state kernel behind future `collapse`, `observe`, and `decay`. It proves observe/collapse exclusivity, decay preconditions, terminal decayed-state behavior, and conservative snapshot policy for world/entangle-backed regions.
...
- [kain-ownership](/M:/Code/Kain/crates/kain-ownership): portable ownership-state kernel for Kain memory regions. It currently provides the shared state machine and conservative region policy for future `collapse`, `observe`, and `decay` syntax/lowering work, with a local Z3 pack proving the core exclusivity and lifetime invariants.
