...
- [benchmark](benchmark): paired Kain LLVM vs Rust LLVM benchmark lane. `benchmarks.json` owns the case manifest, maturity labels, and fairness notes; `cases/<case>/main.kn` and `main.rs` are dependency-free paired sources; `run.py` builds both sides, times executable samples, and writes `out/reports/latest.html` plus JSON. The current pressure cases target contention/collapse, entangle mirroring, converge dispatch, and direct ownership memory.
